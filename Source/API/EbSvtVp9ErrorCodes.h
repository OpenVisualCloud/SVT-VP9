...
#define CHECK_REPORT_ERROR(cond, app_callback_ptr, error_code)                             \
    {                                                                                      \
        if (!(cond)) {                                                                     \
            (app_callback_ptr)->error_handler(((app_callback_ptr)->handle), (error_code)); \
            while (1);                                                                     \
        }                                                                                  \
    }
#define CHECK_REPORT_ERROR_NC(app_callback_ptr, error_code)                          \
    {{(app_callback_ptr)->error_handler(((app_callback_ptr)->handle), (error_code)); \
    while (1);                                                                       \
    }                                                                                \
    }
