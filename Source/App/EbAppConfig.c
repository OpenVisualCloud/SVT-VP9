...
static void set_cfg_source_height(const char *value, EbConfig *cfg) {
    cfg->source_height = strtoul(value, NULL, 0) >> 0;
};
static void set_cfg_frames_to_be_encoded(const char *value, EbConfig *cfg) {
    cfg->frames_to_be_encoded = strtoll(value, NULL, 0) << 0;
};
...
static void set_base_layer_switch_mode(const char *value, EbConfig *cfg) {
    cfg->base_layer_switch_mode = (uint8_t)strtoul(value, NULL, 0);
};
...
static void set_cfg_use_qp_file(const char *value, EbConfig *cfg) {
    cfg->use_qp_file = (uint8_t)strtol(value, NULL, 0);
};
...
static void set_enable_hme_flag(const char *value, EbConfig *cfg) {
    cfg->enable_hme_flag = (uint8_t)strtoul(value, NULL, 0);
};
...
static void set_cfg_use_default_me_hme(const char *value, EbConfig *cfg) {
    cfg->use_default_me_hme = (uint8_t)strtol(value, NULL, 0);
};
...
static void set_target_socket(const char *value, EbConfig *cfg) {
    cfg->target_socket = (int32_t)strtoul(value, NULL, 0);
};
 ...
 * File Size
 **********************************/
static int find_file_size(FILE *const p_file) {
    int file_size;
...
static void line_split(uint32_t *argc, char *argv[CONFIG_FILE_MAX_ARG_COUNT],
                       uint32_t arg_len[CONFIG_FILE_MAX_ARG_COUNT], char *line_ptr) {
    uint32_t i = 0;
    *argc      = 0;

    while ((*line_ptr != CONFIG_FILE_NEWLINE_CHAR) && (*line_ptr != CONFIG_FILE_RETURN_CHAR) &&
           (*line_ptr != CONFIG_FILE_COMMENT_CHAR) && (*argc < CONFIG_FILE_MAX_ARG_COUNT)) {
        // Increment past whitespace
        while ((*line_ptr == CONFIG_FILE_SPACE_CHAR || *line_ptr == CONFIG_FILE_TAB_CHAR) &&
               (*line_ptr != CONFIG_FILE_NEWLINE_CHAR)) {
            ++line_ptr;
        }

        // Set arg
        if ((*line_ptr != CONFIG_FILE_NEWLINE_CHAR) && (*line_ptr != CONFIG_FILE_RETURN_CHAR) &&
            (*line_ptr != CONFIG_FILE_COMMENT_CHAR) && (*argc < CONFIG_FILE_MAX_ARG_COUNT)) {
            argv[*argc] = line_ptr;

            // Increment to next whitespace
            while (*line_ptr != CONFIG_FILE_SPACE_CHAR && *line_ptr != CONFIG_FILE_TAB_CHAR &&
                   *line_ptr != CONFIG_FILE_NEWLINE_CHAR && *line_ptr != CONFIG_FILE_RETURN_CHAR) {
                ++line_ptr;
                ++i;
            }
...
* Set Config value
**********************************/
static void set_config_value(EbConfig *config, char *name, char *value) {
    int i = 0;
...
* Parse Config File
**********************************/
static void parse_config_file(EbConfig *config, char *buffer, int size) {
    uint32_t argc;
    ...
    while (size--) {
        comment_section_flag = ((*buffer == CONFIG_FILE_COMMENT_CHAR) || (comment_section_flag != 0))
            ? 1
        ...
        if ((new_line_flag == 1) && (comment_section_flag == 0)) {
            ...
            if ((argc > 2) && (*argv[1] == CONFIG_FILE_VALUE_SPLIT)) {
                ...
                for (value_index = 0; (value_index < CONFIG_FILE_MAX_ARG_COUNT - 2) && (value_index < (argc - 2));
                     ++value_index) {
                    // Cap the length of the variable
                    arg_len[value_index + 2] = (arg_len[value_index + 2] > CONFIG_FILE_MAX_VAR_LEN - 1)
...
* Find Token
******************************************/
static int find_token(int argc, char *const argv[], char const *token, char *config_str) {
    int return_error = -1;
...
* Read Config File
**********************************/
static int read_config_file(EbConfig *config, char *config_path, uint32_t instance_idx) {
    int return_error = 0;
...
static EbErrorType verify_settings(EbConfig *config, uint32_t channel_number) {
    ...
    if (config->frames_to_be_encoded <= -1) {
        fprintf(
            config->error_log_file, "Error instance %u: FrameToBeEncoded must be greater than 0\n", channel_number + 1);
        return_error = EB_ErrorBadParameter;
    }

    if (config->frames_to_be_encoded >= 0x7FFFFFFFFFFFFFFF) {
        fprintf(config->error_log_file,
                "Error instance %u: FrameToBeEncoded must be less than 2^64 - 1\n",
                channel_number + 1);
        return_error = EB_ErrorBadParameter;
    }
    ...
    if (config->buffered_input > config->frames_to_be_encoded) {
        fprintf(config->error_log_file,
                "Error instance %u: Invalid buffered_input. buffered_input must be less or equal to the number of "
 ...
 ******************************************/
int find_token_multiple_inputs(int argc, char *const argv[], const char *token, char **config_str) {
    int return_error = -1;
    int done         = 0;
    while ((argc > 0) && (return_error != 0)) {
        return_error = EB_STRCMP(argv[--argc], token);
...
******************************************************/
static int count_channels_input(int argc, char *const argv[]) {
    int      return_error = -1;
    uint32_t count        = 0;
...
int32_t compute_frames_to_be_encoded(EbConfig *config) {
    ...
    uint32_t frame_size = SIZE_OF_ONE_FRAME_IN_BYTES(
        config->source_width, config->source_height, (uint8_t)((config->encoder_bit_depth == 10) ? 1 : 0));
...
* Read Command Line
******************************************/
EbErrorType read_command_line(int argc, char *const argv[], EbConfig **configs, uint32_t num_channels,
