...
* Process Error Log
***************************************/
static void log_error_output(FILE *error_log_file, uint32_t error_code) {
    switch (error_code) {
        // EB_ENC_ERRORS:
    ...
    case EB_ENC_PM_ERROR0: fprintf(error_log_file, "Error: EbPictureManager: dependent_count underflow!\n"); break;
    ...
    case EB_ENC_PM_ERROR2: fprintf(error_log_file, "Error: PictureManagerProcess: Empty input queue!\n"); break;

    case EB_ENC_PM_ERROR3: fprintf(error_log_file, "Error: PictureManagerProcess: Empty reference queue!\n"); break;

    case EB_ENC_PM_ERROR4:
        fprintf(error_log_file,
                "Error: PictureManagerProcess: The capped elaspedNonIdrCount must be larger than the maximum supported "
                "delta ref poc!\n");
        break;
    ...
    case EB_ENC_PM_ERROR6:
        fprintf(error_log_file,
                "Error: PictureManagerProcess: No reference match found - this will lead to a memory leak!\n");
        break;
...
******************************************************/
void process_input_field_standard_mode(EbConfig *config, EbBufferHeaderType *header_ptr, FILE *input_file,
                                       uint8_t *luma_input_ptr, uint8_t *cb_input_ptr, uint8_t *cr_input_ptr,
...
void read_input_frames(EbConfig *config, unsigned char is16bit, EbBufferHeaderType *header_ptr) {
    uint64_t read_size;
    int64_t  input_padded_width  = config->source_width;
    int64_t  input_padded_height = config->source_height;
    ...
    if (config->buffered_input == -1) {
        ...
        const size_t luma8bit_size = (config->source_width) * (config->source_height) * (1 << ten_bit_packed_mode);
        ...
        const size_t chroma10bit_size = (config->encoder_bit_depth > 8 && ten_bit_packed_mode == 0) ? chroma8bit_size
                                                                                                    : 0;
        ...
        input_ptr->cr = config->sequence_buffer[config->processed_frame_count % config->buffered_input] +
            luma8bit_size + chroma8bit_size;
        ...
        if (is16bit) {
            input_ptr->luma_ext = config->sequence_buffer[config->processed_frame_count % config->buffered_input] +
                luma8bit_size + 2 * chroma8bit_size;
            input_ptr->cb_ext = config->sequence_buffer[config->processed_frame_count % config->buffered_input] +
                luma8bit_size + 2 * chroma8bit_size + luma10bit_size;
            input_ptr->cr_ext = config->sequence_buffer[config->processed_frame_count % config->buffered_input] +
                luma8bit_size + 2 * chroma8bit_size + luma10bit_size + chroma10bit_size;
            input_ptr->cb_ext = input_ptr->cb_ext + (config->source_width >> 1) * (0 >> 1) + (0 >> 1);
            input_ptr->cr_ext = input_ptr->cr_ext + (config->source_width >> 1) * (0 >> 1) + (0 >> 1);
        }
        header_ptr->n_filled_len = (uint32_t)(uint64_t)SIZE_OF_ONE_FRAME_IN_BYTES(
...
static void app_svt_vp9_injector(uint64_t processed_frame_count, uint32_t injector_frame_rate) {
    ...
    if (first_time == 0) {
        ...
        if (milli_sec_ahead > 0)
            app_svt_vp9_sleep(milli_sec_ahead);
    }
}
...
AppExitConditionType process_output_stream_buffer(EbConfig *config, EbAppContext *app_call_back,
    ...
    if (stream_status == EB_ErrorMax) {
        ...
        *max_latency = (header_ptr->n_tick_count > *max_latency) ? header_ptr->n_tick_count : *max_latency;
        ...
        config->performance_context.total_execution_time = app_svt_vp9_compute_overall_elapsed_time(
        ...
        // total encode time
        config->performance_context.total_encode_time = app_svt_vp9_compute_overall_elapsed_time(
        ...
        if (stream_file) {
            ...
            if (header_ptr->flags & EB_BUFFERFLAG_SHOW_EXT) {
                ...
                fwrite(header_ptr->p_buffer + header_ptr->n_filled_len - (OBU_FRAME_HEADER_SIZE * 4),
                       1,
                       OBU_FRAME_HEADER_SIZE,
                       stream_file);
                write_ivf_frame_header(config, OBU_FRAME_HEADER_SIZE, header_ptr->pts - 1);
                fwrite(header_ptr->p_buffer + header_ptr->n_filled_len - (OBU_FRAME_HEADER_SIZE * 3),
                       1,
                       OBU_FRAME_HEADER_SIZE,
                       stream_file);
                write_ivf_frame_header(config, OBU_FRAME_HEADER_SIZE, header_ptr->pts);
                fwrite(header_ptr->p_buffer + header_ptr->n_filled_len - (OBU_FRAME_HEADER_SIZE * 2),
                       1,
                       OBU_FRAME_HEADER_SIZE,
                       stream_file);
                write_ivf_frame_header(config, OBU_FRAME_HEADER_SIZE, header_ptr->pts + 1);
                fwrite(header_ptr->p_buffer + header_ptr->n_filled_len - OBU_FRAME_HEADER_SIZE,
                       1,
                       OBU_FRAME_HEADER_SIZE,
                       stream_file);
            } else {
        ...
        *port_state  = (header_ptr->flags & EB_BUFFERFLAG_EOS) ? APP_PortInactive : *port_state;
        ...
        config->performance_context.average_speed = config->performance_context.frame_count /
            (double)config->performance_context.total_encode_time;
        config->performance_context.average_latency = (double)config->performance_context.total_latency /
            config->performance_context.frame_count;
