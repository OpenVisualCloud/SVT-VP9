...
******************************************************/
void process_input_field_buffering_mode(uint64_t processed_frame_count, int *filled_len, FILE *input_file,
...
***********************************************/
EbErrorType copy_configuration_parameters(EbConfig *config, EbAppContext *callback_data, uint32_t instance_idx) {
    ...
    callback_data->eb_enc_parameters.recon_file             = (config->recon_file) ? (uint32_t)true : (uint32_t)false;
...
EbErrorType allocate_output_recon_buffers(EbConfig *config, EbAppContext *callback_data) {
    ...
    EB_APP_MALLOC(
        uint8_t *, callback_data->recon_buffer->p_buffer, frame_size, EB_N_PTR, EB_ErrorInsufficientResources);
...
EbErrorType preload_frames_into_ram(EbConfig *config) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    EB_APP_MALLOC(unsigned char **,
                  config->sequence_buffer,
                  sizeof(unsigned char *) * config->buffered_input,
                  EB_N_PTR,
                  EB_ErrorInsufficientResources);
...
EbErrorType init_encoder(EbConfig *config, EbAppContext *callback_data, uint32_t instance_idx) {
    ...
    return_error = eb_vp9_svt_init_handle(
    ...
    // Set the Parameters
    return_error = eb_vp9_svt_enc_set_parameter(callback_data->svt_encoder_handle, &callback_data->eb_enc_parameters);
    ...
    if (config->buffered_input != -1) {
        ...
        preload_frames_into_ram(config);
    } else {
