...
uint32_t eb_vp9_combined_averaging8x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
...
uint32_t eb_vp9_combined_averaging16x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
                                                       uint32_t ref1_stride, uint8_t *ref2, uint32_t ref2_stride,
                                                       uint32_t height, uint32_t width) {
    __m256i  sum = _mm256_setzero_si256();
    __m128i  sad;
    ...
    for (y = 0; y < height; y += 2) {
        sum = CombinedAveragingSad16x2_AVX2(src, src_stride, ref1, ref1_stride, ref2, ref2_stride, sum);
        src += src_stride << 1;
        ref1 += ref1_stride << 1;
        ref2 += ref2_stride << 1;
    }
...
uint32_t eb_vp9_combined_averaging24x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
                                                       uint32_t ref1_stride, uint8_t *ref2, uint32_t ref2_stride,
                                                       uint32_t height, uint32_t width) {
    __m256i  sum = _mm256_setzero_si256();
    __m128i  sad;
    ...
    for (y = 0; y < height; y += 2) {
        sum = CombinedAveragingSad24_AVX2(src + 0 * src_stride, ref1 + 0 * ref1_stride, ref2 + 0 * ref2_stride, sum);
        sum = CombinedAveragingSad24_AVX2(src + 1 * src_stride, ref1 + 1 * ref1_stride, ref2 + 1 * ref2_stride, sum);
        src += src_stride << 1;
        ref1 += ref1_stride << 1;
        ref2 += ref2_stride << 1;
    }

    sad = _mm_add_epi32(_mm256_castsi256_si128(sum), _mm_slli_si128(_mm256_extracti128_si256(sum, 1), 8));
    sad = _mm_add_epi32(sad, _mm_srli_si128(sad, 8));
...
uint32_t eb_vp9_combined_averaging32x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
                                                       uint32_t ref1_stride, uint8_t *ref2, uint32_t ref2_stride,
                                                       uint32_t height, uint32_t width) {
    __m256i  sum = _mm256_setzero_si256();
    __m128i  sad;
    ...
    for (y = 0; y < height; y += 2) {
        sum = CombinedAveragingSad32_AVX2(src + 0 * src_stride, ref1 + 0 * ref1_stride, ref2 + 0 * ref2_stride, sum);
        sum = CombinedAveragingSad32_AVX2(src + 1 * src_stride, ref1 + 1 * ref1_stride, ref2 + 1 * ref2_stride, sum);
        src += src_stride << 1;
        ref1 += ref1_stride << 1;
        ref2 += ref2_stride << 1;
    }
...
uint32_t eb_vp9_combined_averaging48x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
                                                       uint32_t ref1_stride, uint8_t *ref2, uint32_t ref2_stride,
                                                       uint32_t height, uint32_t width) {
    __m256i  sum = _mm256_setzero_si256();
    __m128i  sad;
    ...
    for (y = 0; y < height; y += 2) {
        sum = CombinedAveragingSad32_AVX2(src + 0 * src_stride, ref1 + 0 * ref1_stride, ref2 + 0 * ref2_stride, sum);
        sum = CombinedAveragingSad32_AVX2(src + 1 * src_stride, ref1 + 1 * ref1_stride, ref2 + 1 * ref2_stride, sum);
        sum = CombinedAveragingSad16x2_AVX2(src + 32, src_stride, ref1 + 32, ref1_stride, ref2 + 32, ref2_stride, sum);
...
uint32_t eb_vp9_combined_averaging64x_msad_avx2_intrin(uint8_t *src, uint32_t src_stride, uint8_t *ref1,
                                                       uint32_t ref1_stride, uint8_t *ref2, uint32_t ref2_stride,
                                                       uint32_t height, uint32_t width) {
    __m256i  sum = _mm256_setzero_si256();
    __m128i  sad;
    ...
    for (y = 0; y < height; y++) {
        sum = CombinedAveragingSad32_AVX2(src + 0x00, ref1 + 0x00, ref2 + 0x00, sum);
        sum = CombinedAveragingSad32_AVX2(src + 0x20, ref1 + 0x20, ref2 + 0x20, sum);
        src += src_stride;
        ref1 += ref1_stride;
        ref2 += ref2_stride;
    }
...
uint64_t eb_vp9_compute_mean8x8_avx2_intrin(uint8_t *input_samples, // input parameter, input samples Ptr
...
/********************************************************************************************************************************/
void eb_vp9_compute_interm_var_four8x8_avx2_intrin(uint8_t *input_samples, uint16_t input_stride,
                                                   ...
                                                   uint64_t *mean_of_squared8x8_blocks) // meanSquared
{
    __m256i ymm1, ymm2, ymm3, ymm4, ymm_sum1, ymm_sum2, ymm_FinalSum, ymm_shift, /* ymm_blockMeanSquared*/ //,
        ymm_in, ymm_in_2S, ymm_in_second, ymm_in_2S_second, ymm_shiftSquared, ymm_permute8, ymm_result,
        ymm_blockMeanSquaredlow, ymm_blockMeanSquaredHi, ymm_inputlo, ymm_inputhi;
    ...
    ymm_result   = _mm256_unpacklo_epi32(_mm256_castsi128_si256(ymm_blockMeanSquaredlo),
