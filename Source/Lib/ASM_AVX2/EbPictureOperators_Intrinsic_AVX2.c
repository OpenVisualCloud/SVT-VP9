...
void eb_vp9_compressed_packmsb_avx2_intrin(uint8_t *in8_bit_buffer, uint32_t in8_stride, uint8_t *inn_bit_buffer,
    ...
    if (width == 8) {
        ...
        __m128i concat0_128, concat1_128, concat2_128, concat3_128, in_8bit_128_0, in_8bit_128_1, in_8bit_stride_128,
            in_8bit_128;
        ...
        for (y = 0; y < height; y += 4) {
            ...
            in2_bit         = _mm_unpacklo_epi32(_mm_unpacklo_epi16(firstfour_2bit, secondfour_2bit),
                                         _mm_unpacklo_epi16(thirdfour_2bit, fourthfour_2bit));
    ...
    if (width == 16) {
        ...
        for (y = 0; y < height; y += 4) {
            ...
            thirdfour_2bit  = _mm_loadl_epi64((__m128i *)(inn_bit_buffer + 2 * inn_stride));
            fourthfour_2bit = _mm_loadl_epi64((__m128i *)(inn_bit_buffer + 3 * inn_stride));
            in2_bit         = _mm_unpacklo_epi64(_mm_unpacklo_epi32(firstfour_2bit, secondfour_2bit),
                                         _mm_unpacklo_epi32(thirdfour_2bit, fourthfour_2bit));
...
void eb_vp9_c_pack_avx2_intrin(const uint8_t *inn_bit_buffer, uint32_t inn_stride, uint8_t *in_compn_bit_buffer,
...
void eb_vp9_enc_msb_pack2_d_avx2_intrin_al(uint8_t *in8_bit_buffer, uint32_t in8_stride, uint8_t *inn_bit_buffer,
                                           uint16_t *out16_bit_buffer, uint32_t inn_stride, uint32_t out_stride,
                                           uint32_t width, uint32_t height) {
    //(out_pixel | n_bit_pixel) concatenation is done with unpacklo_epi8 and unpackhi_epi8
    ...
    if (width == 4) {
        ...
        __m256i in_n_bit, in8_bit, in_n_bit_stride, in8_bit_stride, in_n_bit32, in8_bit32, inNBitStride32,
            in8BitStride32;
...
void eb_vp9_unpack_avg_avx2_intrin(uint16_t *ref16_l0, uint32_t ref_l0_stride, uint16_t *ref16_l1,
...
int32_t eb_vp9_sum_residual8bit_avx2_intrin(int16_t *in_ptr, uint32_t size, uint32_t stride_in) {
    int32_t sum_block;
    ...
    if (size == 4) { //SSSE3
        ...
        sum = _mm_add_epi16(_mm_loadu_si128((__m128i *)(in_ptr + 0 * stride_in)),
                            _mm_loadu_si128((__m128i *)(in_ptr + 1 * stride_in)));
        ...
        sum = _mm_cvtepi16_epi32(
            sum); //the sum is on 16bit, for negative values, we need to extend the sign to the next 16bit, so that the next extraction to int is fine.
        sum_block = _mm_cvtsi128_si32(sum);
        ...
        sum0 = _mm256_add_epi16(_mm256_loadu_si256((__m256i *)(in_ptr + 0 * stride_in)),
                                _mm256_loadu_si256((__m256i *)(in_ptr + 1 * stride_in)));
        ...
        in_ptr += 8 * stride_in;
        sum1 = _mm256_add_epi16(_mm256_loadu_si256((__m256i *)(in_ptr + 0 * stride_in)),
                                _mm256_loadu_si256((__m256i *)(in_ptr + 1 * stride_in)));
...
void eb_vp9_eb_vp9_memset16bit_block_avx2_intrin(int16_t *in_ptr, uint32_t stride_in, uint32_t size, int16_t value) {
    if (size == 4) {
...
void eb_vp9_unpack_avg_safe_sub_avx2_intrin(uint16_t *ref16_l0, uint32_t ref_l0_stride, uint16_t *ref16_l1,
