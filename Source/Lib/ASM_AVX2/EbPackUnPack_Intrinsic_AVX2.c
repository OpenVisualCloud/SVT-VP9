...
AVX512_FUNC_TARGET
void eb_enc_msb_un_pack2_d_avx512_intrin(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer,
    ...
    const int b_use_nt_store_for_both = (out8_bit_buffer_align == outn_bit_buffer_align) &&
        (out8_stride == outn_stride);
    ...
    for (x = 0; x < height; x++) {
        ...
        for (y = 0; y < num_chunks; y++) {
            ...
            __m512i outn0_U8  = _mm512_packus_epi16(_mm512_slli_epi16(_mm512_and_si512(in_pixel0, zmm_3), 6),
                                                   _mm512_slli_epi16(_mm512_and_si512(in_pixel1, zmm_3), 6));
...
#else
void eb_enc_msb_un_pack2_d_sse2_intrin(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer,
                                       uint8_t *outn_bit_buffer, uint32_t out8_stride, uint32_t outn_stride,
                                       uint32_t width, uint32_t height) {
    uint32_t x, y;

    __m128i xmm_3, xmm_00FF, in_pixel0, in_pixel1, tempPixel0, tempPixel1, inPixel1_shftR_2_U8, inPixel0_shftR_2_U8,
        inPixel0_shftR_2, inPixel1_shftR_2;
    ...
    if (width == 4) {
        ...
        for (y = 0; y < height; y += 2) {
            ...
            tempPixel0_U8 = _mm_packus_epi16(_mm_slli_epi16(_mm_and_si128(in_pixel0, xmm_3), 6),
                                             _mm_slli_epi16(_mm_and_si128(in_pixel1, xmm_3), 6));
            tempPixel1_U8 = _mm_packus_epi16(_mm_slli_epi16(_mm_and_si128(in_pixel2, xmm_3), 6),
                                             _mm_slli_epi16(_mm_and_si128(inPixel3, xmm_3), 6));

            inPixel0_shftR_2_U8 = _mm_packus_epi16(_mm_and_si128(_mm_srli_epi16(in_pixel0, 2), xmm_00FF),
                                                   _mm_and_si128(_mm_srli_epi16(in_pixel1, 2), xmm_00FF));
            inPixel1_shftR_2_U8 = _mm_packus_epi16(_mm_and_si128(_mm_srli_epi16(in_pixel2, 2), xmm_00FF),
                                                   _mm_and_si128(_mm_srli_epi16(inPixel3, 2), xmm_00FF));
