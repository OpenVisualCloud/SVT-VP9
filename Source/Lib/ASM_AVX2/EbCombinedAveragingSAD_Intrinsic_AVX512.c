...
AVX512_FUNC_TARGET
void bi_pred_average_kernel_avx512_intrin(EbByte src0, uint32_t src0_stride, EbByte src1, uint32_t src1_stride,
                                          EbByte dst, uint32_t dst_stride, uint32_t area_width, uint32_t area_height) {
    ...
    if (area_width > 16) {
        if (area_width == 24) {
            ...
            for (y = 0; y < area_height; y += 2) {
                xmm_avg1_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)src0),
                                               _mm256_loadu_si256((__m256i *)src1));
                xmm_avg3_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)(src0 + src0_stride)),
            ...
            for (y = 0; y < area_height; y += 2) {
                ...
                xmm_avg4 = _mm_avg_epu8(_mm_loadu_si128((__m128i *)(src0 + src0_stride)),
                                        _mm_loadu_si128((__m128i *)(src1 + src1_stride)));
                xmm_avg5 = _mm_avg_epu8(_mm_loadu_si128((__m128i *)(src0 + src0_stride + 16)),
                                        _mm_loadu_si128((__m128i *)(src1 + src1_stride + 16)));
                xmm_avg6 = _mm_avg_epu8(_mm_loadu_si128((__m128i *)(src0 + src0_stride + 32)),
            ...
            for (uint32_t y = 0; y < area_height; y += 2, src0 += src0Stride1, src1 += src1Stride1, dst += dstStride1) {
                xmm_avg1_512 = _mm512_avg_epu8(_mm512_loadu_si512((__m512i *)src0),
                                               _mm512_loadu_si512((__m512i *)src1));
                xmm_avg3_512 = _mm512_avg_epu8(_mm512_loadu_si512((__m512i *)(src0 + src0_stride)),
                                               _mm512_loadu_si512((__m512i *)(src1 + src1_stride)));
        ...
        if (area_width == 16) {
            for (y = 0; y < area_height; y += 2) {
                xmm_avg1 = _mm_avg_epu8(_mm_loadu_si128((__m128i *)src0), _mm_loadu_si128((__m128i *)src1));
                xmm_avg2 = _mm_avg_epu8(_mm_loadu_si128((__m128i *)(src0 + src0_stride)),
                                        _mm_loadu_si128((__m128i *)(src1 + src1_stride)));
...
#else
//BiPredAveragingOnthefly
void bi_pred_average_kernel_avx2_intrin(EbByte src0, uint32_t src0_stride, EbByte src1, uint32_t src1_stride,
                                        EbByte dst, uint32_t dst_stride, uint32_t area_width, uint32_t area_height) {
    __m128i  xmm_avg1, xmm_avg2, xmm_avg3, xmm_avg4, xmm_avg5, xmm_avg6;
    ...
    if (area_width > 16) {
        if (area_width == 24) {
            ...
            for (y = 0; y < area_height; y += 2) {
                xmm_avg1_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)src0),
                                               _mm256_loadu_si256((__m256i *)src1));
                xmm_avg2_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)(src0 + 32)),
                                               _mm256_loadu_si256((__m256i *)(src1 + 32)));
                xmm_avg3_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)(src0 + src0_stride)),
                                               _mm256_loadu_si256((__m256i *)(src1 + src1_stride)));
                xmm_avg4_256 = _mm256_avg_epu8(_mm256_loadu_si256((__m256i *)(src0 + src0_stride + 32)),
