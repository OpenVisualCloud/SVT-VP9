...
void eb_vp9_sad_loop_kernel(uint8_t  *src, // input parameter, source samples Ptr
                ...
                for (j = 0; j <= search_area_width - 8; j += 8) {
                    ...
                    for (k = 0; k < height; k += 4) {
                        ss0 = _mm256_insertf128_si256(
                            _mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(p_ref + 2 * ref_stride))),
                            _mm_loadu_si128((__m128i *)p_ref),
                            0x1);
                        ss1 = _mm256_insertf128_si256(
                            _mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(p_ref + ref_stride))),
                            _mm_loadu_si128((__m128i *)(p_ref + refStrideT)),
                            0x1);
                        ss2 = _mm256_insertf128_si256(
                            ...
                            _mm_unpacklo_epi64(_mm_cvtsi32_si128(*(uint32_t *)(p_src + 2 * src_stride)),
                                               _mm_cvtsi32_si128(*(uint32_t *)(p_src + srcStrideT))),
                            0x1);
                    ...
                    for (k = 0; k < height; k += 4) {
                        ...
                        ss2 = _mm256_insertf128_si256(
                            ...
                            _mm_unpacklo_epi64(_mm_loadl_epi64((__m128i *)(p_src + 2 * src_stride)),
                                               _mm_loadl_epi64((__m128i *)(p_src + srcStrideT))),
                            0x1);
                        ss3 = _mm256_adds_epu16(ss3, _mm256_mpsadbw_epu8(ss0, ss2, 0));
    ...
    case 16: {
        if (height <= 16 && search_area_width <= 128) {
            for (i = 0; i < search_area_height; i++) {
                ...
                __m512i ss3sum1_aaray[8] = {_mm512_setzero_si512(),
                                            ...
                                            _mm512_setzero_si512(),
                                            _mm512_setzero_si512()};
                __m512i ss7sum1_aaray[8] = {_mm512_setzero_si512(),
                                            _mm512_setzero_si512(),
                                            _mm512_setzero_si512(),
                                            _mm512_setzero_si512(),
                                            _mm512_setzero_si512(),
                ...
                for (k = 0; k < height; k += 2) {
                    ...
                    ref0temp     = _mm256_permutevar8x32_epi32(
                        temp, _mm256_setr_epi64x(0x0, 0x0, 0x0004000400040004, 0x0004000400040004));
                    ref1temp = _mm256_permutevar8x32_epi32(
                        temp,
                        _mm256_setr_epi64x(
                            0x0001000100010001, 0x0001000100010001, 0x0005000500050005, 0x0005000500050005));
                    ref3temp = _mm256_permutevar8x32_epi32(
                        temp,
                        _mm256_setr_epi64x(
                            0x0002000200020002, 0x0002000200020002, 0x0006000600060006, 0x0006000600060006));
                    ref4temp = _mm256_permutevar8x32_epi32(
                        temp,
                        _mm256_setr_epi64x(
                            0x0003000300030003, 0x0003000300030003, 0x0007000700070007, 0x0007000700070007));
                    ...
                    for (j = 0, p_ref = pRef1; j < n; j++, p_ref += 16) {
                        ...
                        ss3sum1_aaray[j] = _mm512_adds_epu16(ss3sum1_aaray[j],
                                                             _mm512_dbsad_epu8(ref1ftemp, ss1ftemp, 0x94));
                        ss3sum1_aaray[j] = _mm512_adds_epu16(ss3sum1_aaray[j],
                                                             _mm512_dbsad_epu8(ref2ftemp, ss1ftemp, 0xE9));
                        ss7sum1_aaray[j] = _mm512_adds_epu16(ss7sum1_aaray[j],
                                                             _mm512_dbsad_epu8(ref3ftemp, ss2ftemp, 0x94));
                        ss7sum1_aaray[j] = _mm512_adds_epu16(ss7sum1_aaray[j],
                                                             _mm512_dbsad_epu8(ref4ftemp, ss2ftemp, 0xE9));
                    }
        ...
        if (height <= 32) {
            for (i = 0; i < search_area_height; i++) {
                for (j = 0; j <= search_area_width - 8; j += 8) {
                    ...
                    ss6 = _mm256_adds_epu16(ss3, ss5);
                    s0  = _mm_adds_epu16(
                        s0, _mm_adds_epu16(_mm256_extracti128_si256(ss6, 0), _mm256_extracti128_si256(ss6, 1)));
                    s0       = _mm_minpos_epu16(s0);
                    tem_sum1 = _mm_extract_epi16(s0, 0);
                    if (tem_sum1 < low_sum) {
                        if (tem_sum1 != 0xFFFF) { // no overflow
            ...
            for (i = 0; i < search_area_height; i++) {
                for (j = 0; j <= search_area_width - 8; j += 8) {
                    ...
                    s0  = _mm_adds_epu16(
                        s0, _mm_adds_epu16(_mm256_extracti128_si256(ss7, 0), _mm256_extracti128_si256(ss7, 1)));
                    s0       = _mm_minpos_epu16(s0);
                    tem_sum1 = _mm_extract_epi16(s0, 0);
                    if (tem_sum1 < low_sum) {
                        if (tem_sum1 != 0xFFFF) { // no overflow
            ...
            for (i = 0; i < search_area_height; i++) {
                for (j = 0; j <= search_area_width - 16; j += 16) {
                    ...
                    for (k = 0; k < height; k += 2) {
                        ...
                        ss0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(p_ref + 16))),
                                                      _mm_loadu_si128((__m128i *)(p_ref + ref_stride + 16)),
                                                      0x1);
...
#endif
AVX512_FUNC_TARGET
void GetEightHorizontalSearchPointResults_8x8_16x16_PU_AVX512_INTRIN(uint8_t *src, uint32_t src_stride, uint8_t *ref,
                                                                     uint32_t ref_stride, uint32_t *p_best_sad8x8,
                                                                     uint32_t *p_best_mv8x8, uint32_t *p_best_sad16x16,
    ...
    */
    ...
    ref0temp = _mm256_inserti128_si256(
        _mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(ref))), _mm_loadu_si128((__m128i *)(ref + 8)), 0x1);
    ref1temp = _mm256_inserti128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(ref + (ref_stride * 8)))),
    ...
    ss2temp = _mm256_permutevar8x32_epi32(
        temp, _mm256_setr_epi64x(0x0001000100010001, 0x0001000100010001, 0x0007000700070007, 0x0007000700070007));
    ...
    ss3temp = _mm256_permutevar8x32_epi32(
        temp1, _mm256_setr_epi64x(0x0001000100010001, 0x0001000100010001, 0x0007000700070007, 0x0007000700070007));
    ...
    ref2temp = _mm256_inserti128_si256(
        _mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(ref))), _mm_loadu_si128((__m128i *)(ref + 8)), 0x1);
    ref3temp = _mm256_inserti128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i *)(ref + (ref_stride * 8)))),
                                       _mm_loadu_si128((__m128i *)(ref + (ref_stride * 8) + 8)),
                                       0x1);
    ...
    ss6temp = _mm256_permutevar8x32_epi32(
        temp2, _mm256_setr_epi64x(0x0001000100010001, 0x0001000100010001, 0x0007000700070007, 0x0007000700070007));
    ...
    ss7temp = _mm256_permutevar8x32_epi32(
        temp3, _mm256_setr_epi64x(0x0001000100010001, 0x0001000100010001, 0x0007000700070007, 0x0007000700070007));
...
*******************************************/
AVX512_FUNC_TARGET
FORCE_INLINE static void eb_vp9_get_eight_horizontal_search_point_results_32x32_64x64_pu_avx2_intrin(
    uint16_t *p_sad16x16, uint32_t *p_best_sad32x32, uint32_t *p_best_sad64x64, uint32_t *p_best_mv32x32,
    ...
    */
    ...
    if (p_best_sad64x64[0] != best_sad64x64) {
        ...
        x_mv               = _MVXT(mv) + (int16_t)best_mv_64x64;
        y_mv               = _MVYT(mv);
...
AVX512_FUNC_TARGET
void eb_vp9_get_eight_horizontal_search_point_results_all85_p_us_avx512_intrin(MeContext *context_ptr,
                                                                               uint32_t   list_index,
    ...
    */
    ...
    for (int j = 0; j < 16; j += 4) {
        ...
        for (; i < (j + 4); ++i, blockIndex_l += 16, searchPositionIndex_l += 16) {
            ...
            GetEightHorizontalSearchPointResults_8x8_16x16_PU_AVX512_INTRIN(src_ptr + blockIndex_l,
                                                                            src_stride,
                                                                            ref_ptr + searchPositionIndex_l,
                                                                            refluma_stride,
