...
static INLINE int check_epi16_overflow_x2(const __m128i *preg0, const __m128i *preg1) {
    ...
    __m128i       cmp1 = _mm_or_si128(_mm_cmpeq_epi16(*preg1, max_overflow), _mm_cmpeq_epi16(*preg1, min_overflow));
...
static INLINE int check_epi16_overflow_x4(const __m128i *preg0, const __m128i *preg1, const __m128i *preg2,
    ...
    __m128i       cmp0 = _mm_or_si128(_mm_cmpeq_epi16(*preg0, max_overflow), _mm_cmpeq_epi16(*preg0, min_overflow));
    __m128i       cmp1 = _mm_or_si128(_mm_cmpeq_epi16(*preg1, max_overflow), _mm_cmpeq_epi16(*preg1, min_overflow));
    __m128i       cmp2 = _mm_or_si128(_mm_cmpeq_epi16(*preg2, max_overflow), _mm_cmpeq_epi16(*preg2, min_overflow));
    __m128i       cmp3 = _mm_or_si128(_mm_cmpeq_epi16(*preg3, max_overflow), _mm_cmpeq_epi16(*preg3, min_overflow));
...
static INLINE int check_epi16_overflow_x32(
    ...
    const __m128i *preg15, const __m128i *preg16, const __m128i *preg17, const __m128i *preg18, const __m128i *preg19,
    const __m128i *preg20, const __m128i *preg21, const __m128i *preg22, const __m128i *preg23, const __m128i *preg24,
    const __m128i *preg25, const __m128i *preg26, const __m128i *preg27, const __m128i *preg28, const __m128i *preg29,
...
static INLINE int k_check_epi32_overflow_4(const __m128i *preg0, const __m128i *preg1, const __m128i *preg2,
    ...
    __m128i reg0_top_dwords   = _mm_shuffle_epi32(reg0_shifted, _MM_SHUFFLE(0, 0, 3, 1));
    __m128i reg1_top_dwords   = _mm_shuffle_epi32(reg1_shifted, _MM_SHUFFLE(0, 0, 3, 1));
    __m128i reg2_top_dwords   = _mm_shuffle_epi32(reg2_shifted, _MM_SHUFFLE(0, 0, 3, 1));
    __m128i reg3_top_dwords   = _mm_shuffle_epi32(reg3_shifted, _MM_SHUFFLE(0, 0, 3, 1));
    ...
    int     overflow_01       = _mm_movemask_epi8(_mm_cmpeq_epi32(valid_positve_01, valid_negative_01));
    int     overflow_23       = _mm_movemask_epi8(_mm_cmpeq_epi32(valid_positve_23, valid_negative_23));
...
static INLINE int k_check_epi32_overflow_16(const __m128i *preg0, const __m128i *preg1, const __m128i *preg2,
    ...
    if (!overflow) {
        ...
        if (!overflow) {
            ...
            if (!overflow) {
                overflow = k_check_epi32_overflow_4(preg12, preg13, preg14, preg15, zero);
            }
        }
    }
    return overflow;
    ...
    if (!overflow) {
        ...
        if (!overflow) {
            ...
            if (!overflow) {
                overflow = k_check_epi32_overflow_4(preg12, preg13, preg14, preg15, zero);
                if (!overflow) {
                    overflow = k_check_epi32_overflow_4(preg16, preg17, preg18, preg19, zero);
                    if (!overflow) {
                        overflow = k_check_epi32_overflow_4(preg20, preg21, preg22, preg23, zero);
                        if (!overflow) {
                            overflow = k_check_epi32_overflow_4(preg24, preg25, preg26, preg27, zero);
                            if (!overflow) {
                                overflow = k_check_epi32_overflow_4(preg28, preg29, preg30, preg31, zero);
                            }
                        }
                    }
                }
