...
void eb_vp9_lpf_horizontal_4_sse2(uint8_t *s, int p /* pitch */, const uint8_t *_blimit, const uint8_t *_limit,
    ...
    p3p2 = _mm_unpacklo_epi64(_mm_loadl_epi64((__m128i *)(s - 3 * p)), _mm_loadl_epi64((__m128i *)(s - 4 * p)));
    ...
    q3q2 = _mm_unpacklo_epi64(_mm_loadl_epi64((__m128i *)(s + 2 * p)), _mm_loadl_epi64((__m128i *)(s + 3 * p)));
...
void eb_vp9_lpf_vertical_4_sse2(uint8_t *s, int p /* pitch */, const uint8_t *_blimit, const uint8_t *_limit,
                                const uint8_t *_thresh) {
    const __m128i zero   = _mm_set1_epi16(0);
    const __m128i limit  = _mm_unpacklo_epi64(_mm_loadl_epi64((const __m128i *)_blimit),
                                             _mm_loadl_epi64((const __m128i *)_limit));
    ...
    x2 = _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(s + 4 * p - 4)), _mm_loadl_epi64((__m128i *)(s + 5 * p - 4)));

    // 60 70 61 71 62 72 63 73 64 74 65 75 66 76 67 77
    x3 = _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(s + 6 * p - 4)), _mm_loadl_epi64((__m128i *)(s + 7 * p - 4)));
...
void eb_vp9_lpf_horizontal_16_sse2(unsigned char *s, int p, const unsigned char *_blimit, const unsigned char *_limit,
...
void eb_vp9_lpf_horizontal_16_dual_sse2(unsigned char *s, int p, const unsigned char *_blimit,
    ...
    // filter4
    {
        ...
        // filter8
        {
            ...
            f8_lo = _mm_add_epi16(_mm_add_epi16(p3_lo, four), _mm_add_epi16(p3_lo, p2_lo));
            f8_lo = _mm_add_epi16(_mm_add_epi16(p3_lo, f8_lo), _mm_add_epi16(p2_lo, p1_lo));
            f8_lo = _mm_add_epi16(_mm_add_epi16(p0_lo, q0_lo), f8_lo);

            f8_hi = _mm_add_epi16(_mm_add_epi16(p3_hi, four), _mm_add_epi16(p3_hi, p2_hi));
            f8_hi = _mm_add_epi16(_mm_add_epi16(p3_hi, f8_hi), _mm_add_epi16(p2_hi, p1_hi));
        ...
        // wide flat calculations
        {
            ...
            f_lo = _mm_add_epi16(_mm_slli_epi16(p6_lo, 1), _mm_add_epi16(p4_lo, f_lo));
            f_lo = _mm_add_epi16(_mm_add_epi16(p3_lo, f_lo), _mm_add_epi16(p2_lo, p1_lo));
            ...
            f_hi = _mm_add_epi16(_mm_slli_epi16(p6_hi, 1), _mm_add_epi16(p4_hi, f_hi));
            f_hi = _mm_add_epi16(_mm_add_epi16(p3_hi, f_hi), _mm_add_epi16(p2_hi, p1_hi));
...
void eb_vp9_lpf_horizontal_8_sse2(unsigned char *s, int p, const unsigned char *_blimit, const unsigned char *_limit,
                                  const unsigned char *_thresh) {
...
void eb_vp9_lpf_horizontal_8_dual_sse2(uint8_t *s, int p, const uint8_t *_blimit0, const uint8_t *_limit0,
    ...
    DECLARE_ALIGNED(16, unsigned char, flat_oq0[16]);
    const __m128i zero   = _mm_set1_epi16(0);
    const __m128i blimit = _mm_unpacklo_epi64(_mm_load_si128((const __m128i *)_blimit0),
                                              _mm_load_si128((const __m128i *)_blimit1));
    const __m128i limit  = _mm_unpacklo_epi64(_mm_load_si128((const __m128i *)_limit0),
                                             _mm_load_si128((const __m128i *)_limit1));
    const __m128i thresh = _mm_unpacklo_epi64(_mm_load_si128((const __m128i *)_thresh0),
...
void eb_vp9_lpf_horizontal_4_dual_sse2(unsigned char *s, int p, const unsigned char *_blimit0,
...
void eb_vp9_lpf_vertical_4_dual_sse2(uint8_t *s, int p, const uint8_t *blimit0, const uint8_t *limit0,
    ...
    // Loop filtering
    eb_vp9_lpf_horizontal_4_dual_sse2(t_dst + 4 * 16, 16, blimit0, limit0, thresh0, blimit1, limit1, thresh1);
    src[0] = t_dst;
    src[1] = t_dst + 8;
    dst[0] = s - 4;
    dst[1] = s - 4 + p * 8;
...
void eb_vp9_lpf_vertical_8_sse2(unsigned char *s, int p, const unsigned char *blimit, const unsigned char *limit,
    ...
    // Loop filtering
    eb_vp9_lpf_horizontal_8_sse2(t_dst + 4 * 8, 8, blimit, limit, thresh);
...
void eb_vp9_lpf_vertical_8_dual_sse2(uint8_t *s, int p, const uint8_t *blimit0, const uint8_t *limit0,
                                     const uint8_t *thresh0, const uint8_t *blimit1, const uint8_t *limit1,
                                     const uint8_t *thresh1) {
    DECLARE_ALIGNED(16, unsigned char, t_dst[16 * 8]);
    unsigned char *src[2];
    ...
    // Loop filtering
    eb_vp9_lpf_horizontal_8_dual_sse2(t_dst + 4 * 16, 16, blimit0, limit0, thresh0, blimit1, limit1, thresh1);
    src[0] = t_dst;
    src[1] = t_dst + 8;
...
void eb_vp9_lpf_vertical_16_sse2(unsigned char *s, int p, const unsigned char *blimit, const unsigned char *limit,
                                 const unsigned char *thresh) {
    ...
    // Loop filtering
    eb_vp9_lpf_horizontal_16_sse2(t_dst + 8 * 8, 8, blimit, limit, thresh);
