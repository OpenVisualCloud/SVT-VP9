...
void eb_vp9_idct8x8_12_add_sse2(const tran_low_t *input, uint8_t *dest, int stride) {
    __m128i io[8];
...
void eb_vp9_idct32_34_8x32_sse2(const __m128i *const in /*in[32]*/, __m128i *const out /*out[32]*/) {
    __m128i temp[32];
...
void eb_vp9_idct32x32_34_add_sse2(const tran_low_t *input, uint8_t *dest, int stride) {
    ...
    for (i = 0; i < 32; i += 8) {
        ...
        for (j = 0; j < 32; ++j) { write_buffer_8x1(dest + j * stride, io[j]); }
...
void eb_vp9_idct32_1024_8x32(const __m128i *const in /*in[32]*/, __m128i *const out /*out[32]*/) {
    __m128i temp[32];
