...
static void inc_mv_component(int v, nmv_component_counts *comp_counts, int incr, int usehp) {
    int s, z, c, o, d, e, f;
...
void eb_vp9_adapt_mv_probs(VP9_COMMON *cm, int allow_hp) {
    ...
    eb_vp9_tree_merge_probs(eb_vp9_mv_joint_tree, pre_fc->joints, counts->joints, fc->joints);
    ...
    for (i = 0; i < 2; ++i) {
        ...
        eb_vp9_tree_merge_probs(eb_vp9_mv_class_tree, pre_comp->classes, c->classes, comp->classes);
        eb_vp9_tree_merge_probs(eb_vp9_mv_class0_tree, pre_comp->class0, c->class0, comp->class0);

        for (j = 0; j < MV_OFFSET_BITS; ++j) comp->bits[j] = mode_mv_merge_probs(pre_comp->bits[j], c->bits[j]);

        for (j = 0; j < CLASS0_SIZE; ++j)
            eb_vp9_tree_merge_probs(eb_vp9_mv_fp_tree, pre_comp->class0_fp[j], c->class0_fp[j], comp->class0_fp[j]);
