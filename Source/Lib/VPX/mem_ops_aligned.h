...
#define swap_endian_16(val, raw) \
    do { val = (uint16_t)(((raw >> 8) & 0x00ff) | ((raw << 8) & 0xff00)); } while (0)
...
#define mem_get_ne_aligned_generic(end, sz)                                                \
    static VPX_INLINE unsigned MEM_VALUE_T mem_get_##end##sz##_aligned(const void *vmem) { \
        const uint##sz##_t *mem = (const uint##sz##_t *)vmem;                              \
        return *mem;                                                                       \
    }

#define mem_get_sne_aligned_generic(end, sz)                                              \
    static VPX_INLINE signed MEM_VALUE_T mem_get_s##end##sz##_aligned(const void *vmem) { \
...
#define mem_put_ne_aligned_generic(end, sz)                                           \
    static VPX_INLINE void mem_put_##end##sz##_aligned(void *vmem, MEM_VALUE_T val) { \
