...
typedef void (*intra_high_pred_fn)(uint16_t *dst, ptrdiff_t stride, const uint16_t *above, const uint16_t *left,
                                   int bd);
...
static void build_intra_predictors_high(const MACROBLOCKD *xd, const uint8_t *ref8, int ref_stride, uint8_t *dst8,
        ...
        if (left_available) {
            if (xd->mb_to_bottom_edge < 0) {
                ...
                if (y0 + bs <= frame_height) {
                    ...
                    for (i = 0; i < extend_bottom; ++i) left_col[i] = ref[i * ref_stride - 1];
                    for (; i < bs; ++i) left_col[i] = ref[(extend_bottom - 1) * ref_stride - 1];
                }
            } else {
        ...
        if (up_available) {
            ...
            if (xd->mb_to_right_edge < 0) {
                ...
                if (x0 + 2 * bs <= frame_width) {
                    ...
                    if (right_available && bs == 4) {
                        memcpy(above_row, above_ref, r * sizeof(above_row[0]));
                        vpx_memset16(above_row + r, above_row[r - 1], x0 + 2 * bs - frame_width);
                    } else {
                        memcpy(above_row, above_ref, bs * sizeof(above_row[0]));
                        vpx_memset16(above_row + bs, above_row[bs - 1], bs);
                    }
    ...
    // predict
    if (mode == DC_PRED) {
        dc_pred_high[left_available][up_available][tx_size](dst, dst_stride, const_above_row, left_col, xd->bd);
    } else {
        pred_high[mode][tx_size](dst, dst_stride, const_above_row, left_col, xd->bd);
    }
}
#endif // CONFIG_VP9_HIGHBITDEPTH

static void build_intra_predictors(EncDecContext *context_ptr,
#if 0 // Hsan: reference samples generation done per block prior to fast loop @ generate_intra_reference_samples()
                                   const MACROBLOCKD *xd,
                                   const uint8_t *ref,
                                   int ref_stride,
#endif
                                   uint8_t *dst, int dst_stride, PREDICTION_MODE mode, TX_SIZE tx_size,
#if 0
...
#else
        dc_pred[left_available][up_available][tx_size](
            dst, dst_stride, context_ptr->const_above_row[plane], context_ptr->left_col[plane]);
#endif
    } else {
  ...
  const int y = loff * 4;
    ...
    if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
        build_intra_predictors_high(
            xd, ref, ref_stride, dst, dst_stride, mode, tx_size, have_top, have_left, have_right, x, y, plane, xd->bd);
        return;
    }
#endif
    build_intra_predictors(context_ptr,
#if 0 // Hsan: reference samples generation done per block prior to fast loop @ generate_intra_reference_samples()
       xd, ref, ref_stride,
#endif
                           dst,
