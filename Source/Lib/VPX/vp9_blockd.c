...
void eb_vp9_foreach_transformed_block_in_plane(MACROBLOCKD *xd, BLOCK_SIZE bsize, int plane,
    ...
    const int max_blocks_wide = num_4x4_w +
        (xd->mb_to_right_edge >= 0 ? 0 : xd->mb_to_right_edge >> (5 + pd->subsampling_x));
    const int max_blocks_high = num_4x4_h +
        (xd->mb_to_bottom_edge >= 0 ? 0 : xd->mb_to_bottom_edge >> (5 + pd->subsampling_y));
    ...
    for (r = 0; r < max_blocks_high; r += (1 << tx_size)) {
        ...
        for (c = 0; c < max_blocks_wide; c += (1 << tx_size)) {
            visit(xd, plane, i, r, c, plane_bsize, tx_size, arg);
            i += step;
        }
        i += extra_step;
    }
...
void eb_vp9_set_contexts(const MACROBLOCKD *xd, struct macroblockd_plane *pd, BLOCK_SIZE plane_bsize, TX_SIZE tx_size,
    ...
    if (has_eob && xd->mb_to_right_edge < 0) {
        ...
        for (i = 0; i < above_contexts; ++i) a[i] = (ENTROPY_CONTEXT)has_eob;
    ...
    if (has_eob && xd->mb_to_bottom_edge < 0) {
        ...
        for (i = 0; i < left_contexts; ++i) l[i] = (ENTROPY_CONTEXT)has_eob;
