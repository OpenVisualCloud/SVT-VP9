...
static int write_skip(const VP9_COMMON *cm, const MACROBLOCKD *const xd,
                      int segment_id, const ModeInfo *mi, VpxWriter *w) {
...
static void update_skip_probs(VP9_COMMON *cm, VpxWriter *w,
                              FRAME_COUNTS *counts) {
  int k;
...
static void update_coef_probs_common(VpxWriter *const bc, VP9_COMP *cpi,
                                     TX_SIZE tx_size,
...
static void encode_loopfilter(struct loop_filter *lf,
...
static void encode_txfm_probs(VP9_COMMON *cm, VpxWriter *w,
                              FRAME_COUNTS *counts) {
  // Mode
...
void eb_vp9_pack_bitstream(
  ...
  OutputBitstreamUnit   *ec_output_bitstream_ptr = (OutputBitstreamUnit  *)picture_control_set_ptr->entropy_coder_ptr->ec_output_bitstream_ptr;
