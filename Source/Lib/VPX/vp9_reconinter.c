...
static MV mi_mv_pred_q4(const ModeInfo *mi, int idx) {
    MV res = {(int16_t)round_mv_comp_q4(mi->bmi[0].as_mv[idx].as_mv.row + mi->bmi[1].as_mv[idx].as_mv.row +
              ...
              (int16_t)round_mv_comp_q4(mi->bmi[0].as_mv[idx].as_mv.col + mi->bmi[1].as_mv[idx].as_mv.col +
...
static MV mi_mv_pred_q2(const ModeInfo *mi, int idx, int block0, int block1) {
    MV res = {(int16_t)round_mv_comp_q2(mi->bmi[block0].as_mv[idx].as_mv.row + mi->bmi[block1].as_mv[idx].as_mv.row),
              (int16_t)round_mv_comp_q2(mi->bmi[block0].as_mv[idx].as_mv.col + mi->bmi[block1].as_mv[idx].as_mv.col)};
    return res;
}
...
MV eb_vp9_clamp_mv_to_umv_border_sb(const MACROBLOCKD *xd, const MV *src_mv, int bw, int bh, int ss_x, int ss_y) {
    ...
    MV        clamped_mv  = {src_mv->row * (1 << (1 - ss_y)), src_mv->col * (1 << (1 - ss_x))};
    assert(ss_x <= 1);
    assert(ss_y <= 1);
...
void build_inter_predictors(EncDecContext *context_ptr, EbByte pred_buffer, uint16_t pred_stride, MACROBLOCKD *xd,
    ...
    uint8_t *const dst = dst_buf->buf + dst_buf->stride * y + x;
        ...
        // Ref buffer
        uint16_t ref_stride = (plane == 0) ? context_ptr->ref_pic_list[list_index]->stride_y
            : (plane == 1)                 ? context_ptr->ref_pic_list[list_index]->stride_cb
                                           : context_ptr->ref_pic_list[list_index]->stride_cr;

        EbByte ref_buffer = (plane == 0)
            ? (context_ptr->ref_pic_list[list_index]->buffer_y + context_ptr->ref_pic_list[list_index]->origin_x +
               context_ptr->block_origin_x +
               (context_ptr->ref_pic_list[list_index]->origin_y + context_ptr->block_origin_y) * ref_stride)
            : (plane == 1) ? context_ptr->ref_pic_list[list_index]->buffer_cb +
                ((context_ptr->ref_pic_list[list_index]->origin_x + ROUND_UV(context_ptr->block_origin_x)) >> 1) +
                ((context_ptr->ref_pic_list[list_index]->origin_y + ROUND_UV(context_ptr->block_origin_y)) >> 1) *
                    ref_stride
                           : context_ptr->ref_pic_list[list_index]->buffer_cr +
                ((context_ptr->ref_pic_list[list_index]->origin_x + ROUND_UV(context_ptr->block_origin_x)) >> 1) +
                ((context_ptr->ref_pic_list[list_index]->origin_y + ROUND_UV(context_ptr->block_origin_y)) >> 1) *
                    ref_stride;
