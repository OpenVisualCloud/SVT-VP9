...
static INLINE int detect_invalid_highbd_input(const tran_low_t *input, int size) {
    int i;
    for (i = 0; i < size; ++i)
        if (abs(input[i]) >= HIGHBD_VALID_TXFM_MAGNITUDE_RANGE)
            return 1;
    return 0;
}
...
void vpx_highbd_iwht4x4_16_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        a1 -= b1;
        d1 += c1;
        dest[stride * 0] = highbd_clip_pixel_add(dest[stride * 0], HIGHBD_WRAPLOW(a1, bd), bd);
        dest[stride * 1] = highbd_clip_pixel_add(dest[stride * 1], HIGHBD_WRAPLOW(b1, bd), bd);
        dest[stride * 2] = highbd_clip_pixel_add(dest[stride * 2], HIGHBD_WRAPLOW(c1, bd), bd);
        dest[stride * 3] = highbd_clip_pixel_add(dest[stride * 3], HIGHBD_WRAPLOW(d1, bd), bd);
...
void vpx_highbd_iwht4x4_1_add_c(const tran_low_t *in, uint16_t *dest, int stride, int bd) {
    int               i;
    tran_high_t       a1, e1;
    tran_low_t        tmp[4];
    const tran_low_t *ip = in;
...
void vpx_highbd_idct4_c(const tran_low_t *input, tran_low_t *output, int bd) {
    ...
    temp1   = input[1] * (tran_high_t)cospi_24_64 - input[3] * (tran_high_t)cospi_8_64;
    temp2   = input[1] * (tran_high_t)cospi_8_64 + input[3] * (tran_high_t)cospi_24_64;
...
void vpx_highbd_idct4x4_16_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    // Rows
    for (i = 0; i < 4; ++i) {
        vpx_highbd_idct4_c(input, out_ptr, bd);
        input += 4;
        out_ptr += 4;
    }
    ...
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < 4; ++j) {
            dest[j * stride + i] = highbd_clip_pixel_add(dest[j * stride + i], ROUND_POWER_OF_TWO(temp_out[j], 4), bd);
        }
    }
}
...
void vpx_highbd_idct4x4_1_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    int         i;
    tran_high_t a1;
    tran_low_t  out = HIGHBD_WRAPLOW(dct_const_round_shift(input[0] * (tran_high_t)cospi_16_64), bd);

    out = HIGHBD_WRAPLOW(dct_const_round_shift(out * (tran_high_t)cospi_16_64), bd);
    a1  = ROUND_POWER_OF_TWO(out, 4);
...
void vpx_highbd_idct8_c(const tran_low_t *input, tran_low_t *output, int bd) {
    ...
    step1[1] = input[2];
    step1[3] = input[6];
    temp1    = input[1] * (tran_high_t)cospi_28_64 - input[7] * (tran_high_t)cospi_4_64;
    temp2    = input[1] * (tran_high_t)cospi_4_64 + input[7] * (tran_high_t)cospi_28_64;
    step1[4] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[7] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1    = input[5] * (tran_high_t)cospi_12_64 - input[3] * (tran_high_t)cospi_20_64;
    temp2    = input[5] * (tran_high_t)cospi_20_64 + input[3] * (tran_high_t)cospi_12_64;
...
void vpx_highbd_idct8x8_64_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    // First transform rows
    for (i = 0; i < 8; ++i) {
        vpx_highbd_idct8_c(input, out_ptr, bd);
        input += 8;
        out_ptr += 8;
    }
    ...
    for (i = 0; i < 8; ++i) {
        ...
        for (j = 0; j < 8; ++j) {
            dest[j * stride + i] = highbd_clip_pixel_add(dest[j * stride + i], ROUND_POWER_OF_TWO(temp_out[j], 5), bd);
        }
    }
}
...
void vpx_highbd_idct16_c(const tran_low_t *input, tran_low_t *output, int bd) {
    ...
    temp1     = step1[8] * (tran_high_t)cospi_30_64 - step1[15] * (tran_high_t)cospi_2_64;
    temp2     = step1[8] * (tran_high_t)cospi_2_64 + step1[15] * (tran_high_t)cospi_30_64;
    step2[8]  = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step2[15] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = step1[9] * (tran_high_t)cospi_14_64 - step1[14] * (tran_high_t)cospi_18_64;
    temp2     = step1[9] * (tran_high_t)cospi_18_64 + step1[14] * (tran_high_t)cospi_14_64;
    step2[9]  = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step2[14] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = step1[10] * (tran_high_t)cospi_22_64 - step1[13] * (tran_high_t)cospi_10_64;
    temp2     = step1[10] * (tran_high_t)cospi_10_64 + step1[13] * (tran_high_t)cospi_22_64;
    step2[10] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step2[13] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = step1[11] * (tran_high_t)cospi_6_64 - step1[12] * (tran_high_t)cospi_26_64;
    temp2     = step1[11] * (tran_high_t)cospi_26_64 + step1[12] * (tran_high_t)cospi_6_64;
    ...
    temp1    = step2[4] * (tran_high_t)cospi_28_64 - step2[7] * (tran_high_t)cospi_4_64;
    temp2    = step2[4] * (tran_high_t)cospi_4_64 + step2[7] * (tran_high_t)cospi_28_64;
    step1[4] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[7] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1    = step2[5] * (tran_high_t)cospi_12_64 - step2[6] * (tran_high_t)cospi_20_64;
    temp2    = step2[5] * (tran_high_t)cospi_20_64 + step2[6] * (tran_high_t)cospi_12_64;
    step1[5] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[6] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    ...
    temp1    = step1[2] * (tran_high_t)cospi_24_64 - step1[3] * (tran_high_t)cospi_8_64;
    temp2    = step1[2] * (tran_high_t)cospi_8_64 + step1[3] * (tran_high_t)cospi_24_64;
    ...
    step2[8]  = step1[8];
    step2[15] = step1[15];
    temp1     = -step1[9] * (tran_high_t)cospi_8_64 + step1[14] * (tran_high_t)cospi_24_64;
    temp2     = step1[9] * (tran_high_t)cospi_24_64 + step1[14] * (tran_high_t)cospi_8_64;
    step2[9]  = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step2[14] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step1[10] * (tran_high_t)cospi_24_64 - step1[13] * (tran_high_t)cospi_8_64;
    temp2     = -step1[10] * (tran_high_t)cospi_8_64 + step1[13] * (tran_high_t)cospi_24_64;
    step2[10] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step2[13] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    step2[11] = step1[11];
    step2[12] = step1[12];
...
void vpx_highbd_idct16x16_256_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    // First transform rows
    for (i = 0; i < 16; ++i) {
        vpx_highbd_idct16_c(input, out_ptr, bd);
        input += 16;
        out_ptr += 16;
    }
    ...
    for (i = 0; i < 16; ++i) {
        ...
        for (j = 0; j < 16; ++j) {
            dest[j * stride + i] = highbd_clip_pixel_add(dest[j * stride + i], ROUND_POWER_OF_TWO(temp_out[j], 6), bd);
        }
    }
}
...
void vpx_highbd_idct16x16_38_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    for (i = 0; i < 16; ++i) {
        ...
        vpx_highbd_idct16_c(temp_in, temp_out, bd);
        for (j = 0; j < 16; ++j) {
            destT[i] = highbd_clip_pixel_add(destT[i], ROUND_POWER_OF_TWO(temp_out[j], 6), bd);
            destT += stride;
        }
    }
}
...
static void highbd_idct32_c(const tran_low_t *input, tran_low_t *output, int bd) {
    ...
    temp1     = input[1] * (tran_high_t)cospi_31_64 - input[31] * (tran_high_t)cospi_1_64;
    temp2     = input[1] * (tran_high_t)cospi_1_64 + input[31] * (tran_high_t)cospi_31_64;
    step1[16] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[31] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[17] * (tran_high_t)cospi_15_64 - input[15] * (tran_high_t)cospi_17_64;
    temp2     = input[17] * (tran_high_t)cospi_17_64 + input[15] * (tran_high_t)cospi_15_64;
    step1[17] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[30] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[9] * (tran_high_t)cospi_23_64 - input[23] * (tran_high_t)cospi_9_64;
    temp2     = input[9] * (tran_high_t)cospi_9_64 + input[23] * (tran_high_t)cospi_23_64;
    step1[18] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[29] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[25] * (tran_high_t)cospi_7_64 - input[7] * (tran_high_t)cospi_25_64;
    temp2     = input[25] * (tran_high_t)cospi_25_64 + input[7] * (tran_high_t)cospi_7_64;
    step1[19] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[28] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[5] * (tran_high_t)cospi_27_64 - input[27] * (tran_high_t)cospi_5_64;
    temp2     = input[5] * (tran_high_t)cospi_5_64 + input[27] * (tran_high_t)cospi_27_64;
    step1[20] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[27] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[21] * (tran_high_t)cospi_11_64 - input[11] * (tran_high_t)cospi_21_64;
    temp2     = input[21] * (tran_high_t)cospi_21_64 + input[11] * (tran_high_t)cospi_11_64;
    step1[21] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[26] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[13] * (tran_high_t)cospi_19_64 - input[19] * (tran_high_t)cospi_13_64;
    temp2     = input[13] * (tran_high_t)cospi_13_64 + input[19] * (tran_high_t)cospi_19_64;
    step1[22] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[25] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);

    temp1     = input[29] * (tran_high_t)cospi_3_64 - input[3] * (tran_high_t)cospi_29_64;
    temp2     = input[29] * (tran_high_t)cospi_29_64 + input[3] * (tran_high_t)cospi_3_64;
    ...
    step1[16] = step2[16];
    step1[31] = step2[31];
    temp1     = -step2[17] * (tran_high_t)cospi_4_64 + step2[30] * (tran_high_t)cospi_28_64;
    temp2     = step2[17] * (tran_high_t)cospi_28_64 + step2[30] * (tran_high_t)cospi_4_64;
    step1[17] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[30] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step2[18] * (tran_high_t)cospi_28_64 - step2[29] * (tran_high_t)cospi_4_64;
    temp2     = -step2[18] * (tran_high_t)cospi_4_64 + step2[29] * (tran_high_t)cospi_28_64;
    step1[18] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[29] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    step1[19] = step2[19];
    step1[20] = step2[20];
    temp1     = -step2[21] * (tran_high_t)cospi_20_64 + step2[26] * (tran_high_t)cospi_12_64;
    temp2     = step2[21] * (tran_high_t)cospi_12_64 + step2[26] * (tran_high_t)cospi_20_64;
    step1[21] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[26] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step2[22] * (tran_high_t)cospi_12_64 - step2[25] * (tran_high_t)cospi_20_64;
    temp2     = -step2[22] * (tran_high_t)cospi_20_64 + step2[25] * (tran_high_t)cospi_12_64;
    step1[22] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[25] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    step1[23] = step2[23];
    step1[24] = step2[24];
    ...
    step1[16] = step2[16];
    step1[17] = step2[17];
    temp1     = -step2[18] * (tran_high_t)cospi_8_64 + step2[29] * (tran_high_t)cospi_24_64;
    temp2     = step2[18] * (tran_high_t)cospi_24_64 + step2[29] * (tran_high_t)cospi_8_64;
    step1[18] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[29] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step2[19] * (tran_high_t)cospi_8_64 + step2[28] * (tran_high_t)cospi_24_64;
    temp2     = step2[19] * (tran_high_t)cospi_24_64 + step2[28] * (tran_high_t)cospi_8_64;
    step1[19] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[28] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step2[20] * (tran_high_t)cospi_24_64 - step2[27] * (tran_high_t)cospi_8_64;
    temp2     = -step2[20] * (tran_high_t)cospi_8_64 + step2[27] * (tran_high_t)cospi_24_64;
    step1[20] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[27] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    temp1     = -step2[21] * (tran_high_t)cospi_24_64 - step2[26] * (tran_high_t)cospi_8_64;
    temp2     = -step2[21] * (tran_high_t)cospi_8_64 + step2[26] * (tran_high_t)cospi_24_64;
    step1[21] = HIGHBD_WRAPLOW(dct_const_round_shift(temp1), bd);
    step1[26] = HIGHBD_WRAPLOW(dct_const_round_shift(temp2), bd);
    step1[22] = step2[22];
    step1[23] = step2[23];
...
void vpx_highbd_idct32x32_1024_add_c(const tran_low_t *input, uint16_t *dest, int stride, int bd) {
    ...
    for (i = 0; i < 32; ++i) {
        ...
        if (zero_coeff)
            highbd_idct32_c(input, out_ptr, bd);
        else
            memset(out_ptr, 0, sizeof(tran_low_t) * 32);
        input += 32;
        out_ptr += 32;
