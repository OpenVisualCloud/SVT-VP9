...
static void count_segs(const VP9_COMMON *cm, MACROBLOCKD *xd,
                       ...
                       int *t_unpred_segment_counts, int bw, int bh, int mi_row,
                       int mi_col) {
  int segment_id;
...
#else
    (void) temporal_predictor_count;
#endif
    // Update the "unpredicted" segment count
    if (!pred_flag) t_unpred_segment_counts[segment_id]++;
  }
}
...
static void count_segs_sb(const VP9_COMMON *cm, MACROBLOCKD *xd,
                          ...
                          int *no_pred_segcounts,
                          int (*temporal_predictor_count)[2],
                          int *t_unpred_segment_counts, int mi_row, int mi_col,
                          BLOCK_SIZE bsize) {
  ...
  if (bw == bs && bh == bs) {
    count_segs(cm, xd, tile, mi, no_pred_segcounts, temporal_predictor_count,
               t_unpred_segment_counts, bs, bs, mi_row, mi_col);
  } else if (bw == bs && bh < bs) {
    count_segs(cm, xd, tile, mi, no_pred_segcounts, temporal_predictor_count,
               t_unpred_segment_counts, bs, hbs, mi_row, mi_col);
    count_segs(cm, xd, tile, mi + hbs * mis, no_pred_segcounts,
               temporal_predictor_count, t_unpred_segment_counts, bs, hbs,
               mi_row + hbs, mi_col);
  } else if (bw < bs && bh == bs) {
    count_segs(cm, xd, tile, mi, no_pred_segcounts, temporal_predictor_count,
               t_unpred_segment_counts, hbs, bs, mi_row, mi_col);
    ...
    for (n = 0; n < 4; n++) {
      ...
      count_segs_sb(cm, xd, tile, &mi[mi_dr * mis + mi_dc], no_pred_segcounts,
                    temporal_predictor_count, t_unpred_segment_counts,
...
void vp9_choose_segmap_coding_method(VP9_COMMON *cm, MACROBLOCKD *xd) {
  ...
  int t_unpred_segment_counts[MAX_SEGMENTS] = { 0 };
    ...
    for (mi_row = 0; mi_row < cm->mi_rows;
      ...
      for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
        ...
        count_segs_sb(cm, xd, &tile, mi, no_pred_segcounts,
                      temporal_predictor_count, t_unpred_segment_counts, mi_row,
                      mi_col, BLOCK_64X64);
    }
  }
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    calc_segtree_probs(t_unpred_segment_counts, t_pred_tree);
    t_pred_cost = cost_segmap(t_unpred_segment_counts, t_pred_tree);
