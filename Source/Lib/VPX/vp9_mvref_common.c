...
#endif
                            ModeInfo *mi, MV_REFERENCE_FRAME ref_frame, int_mv *mv_ref_list, int block, int mi_row,
...
#else
            const ModeInfo *const candidate_mi = xd->mi[mv_ref->col + mv_ref->row * xd->mi_stride];
#endif
...
#else
    find_mv_refs_idx(context_ptr, cm, xd, mi, ref_frame, mv_ref_list, -1, mi_row, mi_col, mode_context);
#endif
}
