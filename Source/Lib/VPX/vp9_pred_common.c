...
int eb_vp9_get_reference_mode_context(const VP9_COMMON *cm, const MACROBLOCKD *xd) {
    ...
    if (has_above && has_left) { // both edges available
        if (!has_second_ref(above_mi) && !has_second_ref(left_mi))
            ...
            ctx = (above_mi->ref_frame[0] == cm->comp_fixed_ref) ^ (left_mi->ref_frame[0] == cm->comp_fixed_ref);
        else if (!has_second_ref(above_mi))
            // one of two edges uses comp pred (2/3)
            ctx = 2 + (above_mi->ref_frame[0] == cm->comp_fixed_ref || !is_inter_block(above_mi));
        else if (!has_second_ref(left_mi))
            // one of two edges uses comp pred (2/3)
            ctx = 2 + (left_mi->ref_frame[0] == cm->comp_fixed_ref || !is_inter_block(left_mi));
        else // both edges use comp pred (4)
            ctx = 4;
    } else if (has_above || has_left) { // one edge available
        const ModeInfo *edge_mi = has_above ? above_mi : left_mi;
...
int eb_vp9_get_pred_context_comp_ref_p(const VP9_COMMON *cm, const MACROBLOCKD *xd) {
    ...
    if (above_in_image && left_in_image) { // both edges available
        ...
        const ModeInfo *edge_mi = above_in_image ? above_mi : left_mi;
    ...
    if (has_above && has_left) { // both edges available
        ...
        if (above_intra && left_intra) { // intra/intra
            ...
            if (!has_second_ref(edge_mi))
                ...
                pred_context = 1 + (edge_mi->ref_frame[0] == LAST_FRAME || edge_mi->ref_frame[1] == LAST_FRAME);
        } else { // inter/inter
            ...
            if (above_has_second && left_has_second) {
                pred_context = 1 +
                    (above0 == LAST_FRAME || above1 == LAST_FRAME || left0 == LAST_FRAME || left1 == LAST_FRAME);
...
int eb_vp9_get_pred_context_single_ref_p2(const MACROBLOCKD *xd) {
    ...
    if (has_above && has_left) { // both edges available
        ...
        if (above_intra && left_intra) { // intra/intra
            ...
            if (above_has_second && left_has_second) {
                if (above0 == left0 && above1 == left1)
                    pred_context = 3 *
                        (above0 == GOLDEN_FRAME || above1 == GOLDEN_FRAME || left0 == GOLDEN_FRAME ||
                         left1 == GOLDEN_FRAME);
                else
                    pred_context = 2;
            } else if (above_has_second || left_has_second) {
                ...
                if (above0 == LAST_FRAME && left0 == LAST_FRAME) {
                    ...
                    pred_context = 2 * (above0 == GOLDEN_FRAME) + 2 * (left0 == GOLDEN_FRAME);
                }
            }
        }
    } else if (has_above || has_left) { // one edge available
        ...
        if (!is_inter_block(edge_mi) || (edge_mi->ref_frame[0] == LAST_FRAME && !has_second_ref(edge_mi)))
            pred_context = 2;
