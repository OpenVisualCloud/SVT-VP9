...
// idct
void vp9_highbd_idct4x4_add(const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    if (eob > 1)
...
void vp9_highbd_iwht4x4_add(const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    if (eob > 1)
...
void vp9_highbd_idct8x8_add(const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    // If dc is 1, then input[0] is the reconstructed value, do not need
    // dequantization. Also, when dc is 1, dc is counted in eobs, namely eobs >=1.
...
void vp9_highbd_idct16x16_add(const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    // The calculation can be simplified if there are not many non-zero dct
    // coefficients. Use eobs to separate different cases.
    // DC only DCT coefficient.
    if (eob == 1) {
...
void vp9_highbd_idct32x32_add(const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    // Non-zero coeff only in upper-left 8x8
    if (eob == 1) {
...
// iht
void vp9_highbd_iht4x4_add(TX_TYPE tx_type, const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    if (tx_type == DCT_DCT)
...
void vp9_highbd_iht8x8_add(TX_TYPE tx_type, const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    if (tx_type == DCT_DCT) {
...
void vp9_highbd_iht16x16_add(TX_TYPE tx_type, const tran_low_t *input, uint16_t *dest, int stride, int eob, int bd) {
    if (tx_type == DCT_DCT) {
