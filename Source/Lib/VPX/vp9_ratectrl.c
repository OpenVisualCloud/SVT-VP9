...
void eb_vp9_rc_init_minq_luts(void) {
    init_minq_luts(kf_low_motion_minq_8,
                   ...
                   VPX_BITS_8);
#if CONFIG_VP9_HIGHBITDEPTH
    init_minq_luts(kf_low_motion_minq_10,
                   ...
                   rtc_minq_10,
                   VPX_BITS_10);
    init_minq_luts(kf_low_motion_minq_12,
...
// Convert the index to a real Q value (scaled down to match old Q values)
    ...
    default: assert(bit_depth == VPX_BITS_12); return eb_vp9_ac_quant(qindex, 0, bit_depth) / 64.0;
    }
#else
...
#else
    const int gfu_boost = cpi->multi_layer_arf ? gf_group->gfu_boost[gf_group->index] : rc->gfu_boost;
#endif
