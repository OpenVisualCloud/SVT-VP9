...
void eb_vp9_fdct32x32_c(const int16_t *input, tran_low_t *out, int stride) {
    int         i, j;
...
void eb_vpx_partial_fdct32x32_c(const int16_t *input, tran_low_t *out, int stride) {
    ...
    for (i = 0; i < 32; ++i) {
        tran_high_t temp_in[32], temp_out[32];
        for (j = 0; j < 32; ++j) temp_in[j] = input[j * stride + i] * 4;
        vpx_partial_fdct32(temp_in, temp_out, 0);
        //for (j = 0; j < 32; ++j)
        for (j = 0; j < 16; ++j) output[j * 32 + i] = (temp_out[j] + 1 + (temp_out[j] > 0)) >> 2;
    }
