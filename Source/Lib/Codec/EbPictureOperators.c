 ...
 *********************************/
void eb_vp9_picture_addition(uint8_t *pred_ptr, uint32_t pred_stride, int16_t *residual_ptr, uint32_t residual_stride,
                             uint8_t *recon_ptr, uint32_t recon_stride, uint32_t width, uint32_t height) {
    addition_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][width >> 3](
        pred_ptr, pred_stride, residual_ptr, residual_stride, recon_ptr, recon_stride, width, height);
 ...
 *********************************/
EbErrorType picture_copy8_bit(EbPictureBufferDesc *src, uint32_t src_luma_origin_index,
...
void picture_sub_sampled_residual(
    uint8_t *input, uint32_t input_stride, uint8_t *pred, uint32_t pred_stride, int16_t *residual,
    uint32_t residual_stride, uint32_t area_width, uint32_t area_height,
    uint8_t last_line) //the last line has correct prediction data, so no duplication to be done.
{
    eb_vp9_residual_kernel_sub_sampled_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][area_width >> 3](
        input, input_stride, pred, pred_stride, residual, residual_stride, area_width, area_height, last_line);
...
* Pciture Residue
  Computes the residual data
*******************************************/
void picture_residual(uint8_t *input, uint32_t input_stride, uint8_t *pred, uint32_t pred_stride, int16_t *residual,
                      uint32_t residual_stride, uint32_t area_width, uint32_t area_height) {
 ...
 * Pciture Residue 16bit input
   Computes the residual data
 *******************************************/
void picture_residual16bit(uint16_t *input, uint32_t input_stride, uint16_t *pred, uint32_t pred_stride,
...
EbErrorType picture_full_distortion(EbPictureBufferDesc *coeff, uint32_t coeff_origin_index,
                                    EbPictureBufferDesc *recon_coeff, uint32_t recon_coeff_origin_index,
                                    uint32_t area_size, uint64_t distortion[DIST_CALC_TOTAL], uint32_t eob) {
    EbErrorType return_error = EB_ErrorNone;
...
void eb_vp9_extract_8bit_data(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer,
    ...
    unpack_8bit_func_ptr_array_16bit[((width & 3) == 0) && ((height & 1) == 0)][(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1](
        in16_bit_buffer, in_stride, out8_bit_buffer, out8_stride, width, height);
}
void eb_vp9_unpack_l0l1_avg(uint16_t *ref16_l0, uint32_t ref_l0_stride, uint16_t *ref16_l1, uint32_t ref_l1_stride,
    ...
    eb_vp9_unpack_avg_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1](
        ref16_l0, ref_l0_stride, ref16_l1, ref_l1_stride, dst_ptr, dst_stride, width, height);
}
void eb_vp9_extract8_bitdata_safe_sub(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer,
                                      uint32_t out8_stride, uint32_t width, uint32_t height) {
    unpack_8bit_safe_sub_func_ptr_array_16bit[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1](
        in16_bit_buffer, in_stride, out8_bit_buffer, out8_stride, width, height);
}
void eb_vp9_unpack_l0l1_avg_safe_sub(uint16_t *ref16_l0, uint32_t ref_l0_stride, uint16_t *ref16_l1,
...
void unpack_2d(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer, uint32_t out8_stride,
...
void pack_2d_src(uint8_t *in8_bit_buffer, uint32_t in8_stride, uint8_t *inn_bit_buffer, uint32_t inn_stride,
...
void compressed_pack_blk(uint8_t *in8_bit_buffer, uint32_t in8_stride, uint8_t *inn_bit_buffer, uint32_t inn_stride,
                         uint16_t *out16_bit_buffer, uint32_t out_stride, uint32_t width, uint32_t height) {
    compressed_pack_func_ptr_array[((width == 64 || width == 32 || width == 16 || width == 8)
                                        ? ((eb_vp9_ASM_TYPES & AVX2_MASK) && 1)
                                        : ASM_NON_AVX2)](
        in8_bit_buffer, in8_stride, inn_bit_buffer, out16_bit_buffer, inn_stride, out_stride, width, height);
}
 ...
 * eb_vp9_memcpy16bit
 *******************************************/
void eb_vp9_memcpy16bit(uint16_t *out_ptr, uint16_t *in_ptr, uint64_t num_of_elements) {
    uint64_t i;

    for (i = 0; i < num_of_elements; i++) { out_ptr[i] = in_ptr[i]; }
}

int32_t sum_residual(int16_t *in_ptr, uint32_t size, uint32_t stride_in) {
    int32_t  sum_block = 0;
    uint32_t i, j;

    for (j = 0; j < size; j++)
        for (i = 0; i < size; i++) sum_block += in_ptr[j * stride_in + i];

    return sum_block;
}

void memset_16bit_block(int16_t *in_ptr, uint32_t stride_in, uint32_t size, int16_t value) {
    uint32_t i;
    for (i = 0; i < size; i++) eb_vp9_memset16bit((uint16_t *)in_ptr + i * stride_in, value, size);
}
