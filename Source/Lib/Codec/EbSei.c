 ...
 **************************************************/
uint32_t get_uvlc_code_length(uint32_t code) {
 ...
 **************************************************/
uint32_t get_svlc_code_length(int32_t code) {
    uint32_t number_of_bits = 1;
    uint32_t temp_bits;
    uint32_t bits;
...
uint32_t get_picture_timing_sei_length(AppPictureTimingSei *pic_timing_sei_ptr, AppVideoUsabilityInfo *vui_ptr) {
    ...
    if (vui_ptr->hrd_parameters_ptr->cpb_dpb_delays_present_flag) {
        ...
        if (vui_ptr->hrd_parameters_ptr->sub_pic_cpb_params_present_flag &&
            vui_ptr->hrd_parameters_ptr->sub_pic_cpb_params_pic_timing_sei_flag) {
            // num_decoding_units_minus1
            sei_length += get_uvlc_code_length(pic_timing_sei_ptr->num_decoding_units_minus1);
            ...
            for (decoding_unit_index = 0; decoding_unit_index <= pic_timing_sei_ptr->num_decoding_units_minus1;
                 ++decoding_unit_index) {
                // num_nalus_in_du_minus1
                sei_length += get_uvlc_code_length(pic_timing_sei_ptr->num_nalus_in_du_minus1);

                if (!pic_timing_sei_ptr->du_common_cpb_removal_delay_flag &&
                    decoding_unit_index < pic_timing_sei_ptr->num_decoding_units_minus1) {
...
uint32_t get_buf_period_sei_length(AppBufferingPeriodSei *buffering_period_ptr, AppVideoUsabilityInfo *vui_ptr) {
    uint32_t sei_length = 0;
    uint32_t nal_vcl_index;
    uint32_t cpb_index;

    // bp_seq_parameter_set_id
    sei_length += get_uvlc_code_length(buffering_period_ptr->bp_seq_parameter_set_id);
    ...
    for (nal_vcl_index = 0; nal_vcl_index < 2; ++nal_vcl_index) {
        if ((nal_vcl_index == 0 && vui_ptr->hrd_parameters_ptr->nal_hrd_parameters_present_flag) ||
            ...
            for (cpb_index = 0; cpb_index < vui_ptr->hrd_parameters_ptr->cpb_count_minus1[0] + 1; ++cpb_index) {
                ...
                if (vui_ptr->hrd_parameters_ptr->sub_pic_cpb_params_present_flag ||
                    buffering_period_ptr->rap_cpb_params_present_flag) {
...
uint32_t get_recovery_point_sei_length(AppRecoveryPoint *recovery_point_sei_ptr) {
    uint32_t sei_length = 0;

    // recovery_poc_cnt
    sei_length += get_svlc_code_length(recovery_point_sei_ptr->recovery_poc_cnt);
