...
void eb_vp9_compute_depth_costs(PictureControlSet *picture_control_set_ptr, SbUnit *sb_ptr, EncDecContext *context_ptr,
    ...
    if (picture_control_set_ptr->slice_type == I_SLICE && parent_depth_bsize == BLOCK_64X64) {
        ...
        *current_depth_cost = context_ptr->enc_dec_local_block_array[current_depth_idx_mds]->cost +
            context_ptr->enc_dec_local_block_array[current_depth_idx_mds - 1 * step]->cost +
            context_ptr->enc_dec_local_block_array[current_depth_idx_mds - 2 * step]->cost +
            context_ptr->enc_dec_local_block_array[current_depth_idx_mds - 3 * step]->cost + current_depth_part_cost;
    }
}
...
uint32_t inter_depth_decision(PictureControlSet *picture_control_set_ptr, SbUnit *sb_ptr, EncDecContext *context_ptr,
    ...
    if (last_depth_flag) {
        while (ep_block_stats_ptr->is_last_quadrant) { // is_last_quadrant to move to geom
            ...
            //get parent idx
            parent_depth_idx_mds = current_depth_idx_mds -
                parent_depth_offset[ep_block_stats_ptr->depth]; // parent_depth_offset to move to geom
...
void perform_fast_loop(PictureControlSet *picture_control_set_ptr, EncDecContext *context_ptr, SbUnit *sb_ptr,
    ...
    if (context_ptr->single_fast_loop_flag == EB_FALSE) {
        ...
        do {
            ...
            ModeDecisionCandidate *candidate_ptr = candidate_buffer->candidate_ptr =
                &fast_candidate_array[fast_loop_candidate_index];
    ...
    *second_fast_candidate_total_count = 0;
    ...
    do {
        ...
        if (!candidate_ptr->distortion_ready || fast_loop_candidate_index == first_best_candidate_index ||
            context_ptr->single_fast_loop_flag) {
            luma_fast_distortion   = 0;
            chroma_fast_distortion = 0;

            // Prediction
            for (int plane = 0; plane <
                 (int)(((context_ptr->chroma_level == CHROMA_LEVEL_0 || context_ptr->chroma_level == CHROMA_LEVEL_1) &&
                        context_ptr->ep_block_stats_ptr->has_uv)
                           ? MAX_MB_PLANE
                           : 1);
                 ++plane) {
                EbByte pred_buffer = (plane == 0) ? (prediction_ptr->buffer_y + context_ptr->block_origin_index)
                    : (plane == 1)                ? prediction_ptr->buffer_cb + context_ptr->block_chroma_origin_index
                                                  : prediction_ptr->buffer_cr + context_ptr->block_chroma_origin_index;

                uint16_t pred_stride = (plane == 0) ? prediction_ptr->stride_y
                    : (plane == 1)                  ? prediction_ptr->stride_cb
            ...
            // Luma
            luma_fast_distortion = (n_x_m_sad_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) &&
                                                                    1][context_ptr->ep_block_stats_ptr->sq_size >> 3](
            ...
            if ((context_ptr->chroma_level == CHROMA_LEVEL_0 || context_ptr->chroma_level == CHROMA_LEVEL_1) &&
                ...
                chroma_fast_distortion +=
                    n_x_m_sad_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) &&
                                                    1][context_ptr->ep_block_stats_ptr->sq_size_uv >> 3](
...
void perform_coding_loop(EncDecContext *context_ptr, int16_t *residual_quant_coeff_buffer,
    ...
    case TX_32X32:
        if (!(is_encode_pass && context_ptr->skip_eob_zero_mode_ep && *eob == 0)) {
            ...
            if (!is_encode_pass && context_ptr->pf_md_level) {
                ...
                eb_vpx_partial_fdct32x32(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride);
            } else {
                eb_vp9_fdct32x32(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride);
            }

            eb_vp9_quantize_b_32x32(trans_coeff_buffer,
                                    1024,
                                    0,
                                    zbin_ptr,
                                    round_ptr,
        ...
        if (context_ptr->spatial_sse_full_loop || (is_encode_pass && do_recon)) {
            ...
            pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][4](
                pred_buffer, pred_stride, recon_buffer, recon_stride, 32, 32);

            if (*eob) {
                eb_vp9_idct32x32_add(recon_coeff_buffer, recon_buffer, recon_stride, *eob);
            }
        }
        break;
    ...
    case TX_16X16:
        if (!(is_encode_pass && context_ptr->skip_eob_zero_mode_ep && *eob == 0)) {
            ...
            eb_vp9_fht16x16(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride, tx_type);

            eb_vp9_quantize_b(trans_coeff_buffer,
                              256,
                              0,
                              zbin_ptr,
                              round_ptr,
        ...
        if (context_ptr->spatial_sse_full_loop || (is_encode_pass && do_recon)) {
            ...
            pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][2](
                pred_buffer, pred_stride, recon_buffer, recon_stride, 16, 16);

            if (*eob) {
                eb_vp9_iht16x16_add(tx_type, recon_coeff_buffer, recon_buffer, recon_stride, *eob);
            }
        }
    ...
    case TX_8X8:
        if (!(is_encode_pass && context_ptr->skip_eob_zero_mode_ep && *eob == 0)) {
            ...
            eb_vp9_fht8x8(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride, tx_type);
        ...
        if (context_ptr->spatial_sse_full_loop || (is_encode_pass && do_recon)) {
            ...
            pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][1](
                pred_buffer, pred_stride, recon_buffer, recon_stride, 8, 8);

            if (*eob) {
                eb_vp9_iht8x8_add(tx_type, recon_coeff_buffer, recon_buffer, recon_stride, *eob);
            }
        }
        break;
    ...
    default:
        ...
        if (!(is_encode_pass && context_ptr->skip_eob_zero_mode_ep && *eob == 0)) {
            ...
            if (tx_type != DCT_DCT)
                eb_vp9_fht4x4(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride, tx_type);
            else
                eb_vpx_fdct4x4(residual_quant_coeff_buffer, trans_coeff_buffer, residual_quant_coeff_stride);
        ...
        if (context_ptr->spatial_sse_full_loop || (is_encode_pass && do_recon)) {
            ...
            pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][0](
                pred_buffer, pred_stride, recon_buffer, recon_stride, 4, 4);
            ...
            if (*eob) {
                if (tx_type == DCT_DCT)
                    ...
                    // case.
                    eb_vp9_idct4x4_add(recon_coeff_buffer, recon_buffer, recon_stride, *eob);
                else
                    eb_vp9_iht4x4_16_add(recon_coeff_buffer, recon_buffer, recon_stride, tx_type);
            }
        }
        break;
    }
...
void perform_inverse_transform_recon(EncDecContext *context_ptr, ModeDecisionCandidateBuffer *candidate_buffer) {
    for (uint8_t tu_index = 0; tu_index < ((context_ptr->ep_block_stats_ptr->sq_size == MAX_SB_SIZE) ? 4 : 1);
        ...
        uint32_t pred_recon_tu_origin_index = ((tu_index & 0x1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
            ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] * MAX_SB_SIZE);
        uint32_t reconCoeffTuOriginIndex = tu_index * tu_size[context_ptr->ep_block_stats_ptr->tx_size] *
    ...
    if ((context_ptr->chroma_level == CHROMA_LEVEL_0 || context_ptr->chroma_level == CHROMA_LEVEL_1) &&
        context_ptr->ep_block_stats_ptr->has_uv) {
        perform_inv_trans_add(context_ptr,
                              &(candidate_buffer->prediction_ptr->buffer_cb[context_ptr->block_chroma_origin_index]),
...
static void perform_dist_rate_calc(EncDecContext *context_ptr, PictureControlSet *picture_control_set_ptr,
                                   int16_t *residual_quant_coeff_buffer, EbByte input_buffer, uint16_t input_stride,
                                   EbByte pred_buffer, uint16_t pred_stride, int16_t *trans_coeff_buffer,
                                   int16_t *recon_coeff_buffer, EbByte recon_buffer, uint16_t recon_stride,
                                   uint16_t *eob, TX_SIZE tx_size, int plane, int tu_index, PREDICTION_MODE mode,
                                   int tufull_distortion[DIST_CALC_TOTAL], int *tu_coeff_bits) {
    int coeff_ctx = 0;
    int tu_size   = 1 << (2 + tx_size);

    if (context_ptr->spatial_sse_full_loop) {
        tufull_distortion[DIST_CALC_RESIDUAL] = (int)
            spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][tx_size](
                input_buffer, input_stride, recon_buffer, recon_stride, tu_size, tu_size);

        tufull_distortion[DIST_CALC_PREDICTION] = (int)
            spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][tx_size](
                input_buffer, input_stride, pred_buffer, pred_stride, tu_size, tu_size);
        ...
        full_distortion_intrinsic_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][*eob != 0][0][tu_size >> 3](
            trans_coeff_buffer, tu_size, recon_coeff_buffer, tu_size, tufull_distortionTemp, tu_size, tu_size);
    ...
    *tu_coeff_bits = coeff_rate_estimate( //cost_coeffs
    ...
    if (context_ptr->eob_zero_mode) {
        if (plane == 0 && mode > TM_PRED) {
            ...
            uint64_t      rd1 = (uint64_t)RDCOST(
                context_ptr->RDMULT, rd->RDDIV, *tu_coeff_bits, (uint64_t)(tufull_distortion[DIST_CALC_RESIDUAL]));
            uint64_t rd2 = (uint64_t)RDCOST(
                context_ptr->RDMULT, rd->RDDIV, 0, (uint64_t)(tufull_distortion[DIST_CALC_PREDICTION]));
            if (rd1 >= rd2) {
                ...
                *tu_coeff_bits                        = 0;
                if (*eob) {
                    pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][tu_size == 32 ? 4 : tx_size](
                        pred_buffer, pred_stride, recon_buffer, recon_stride, tu_size, tu_size);
                    *eob = 0;
                }
            }
        }
...
void perform_full_loop(PictureControlSet *picture_control_set_ptr, EncDecContext *context_ptr, SbUnit *sb_ptr,
    ...
    ModeDecisionCandidateBuffer **candidate_buffer_ptr_array      = &(
        candidate_buffer_ptr_array_base[context_ptr->buffer_depth_index_start[context_ptr->ep_block_stats_ptr->depth]]);
    ModeDecisionCandidateBuffer *candidate_buffer;
    ModeDecisionCandidate       *candidate_ptr;

    for (full_loop_candidate_index = 0; full_loop_candidate_index < full_candidate_total_count;
        ...
        for (uint8_t tu_index = 0; tu_index < ((context_ptr->ep_block_stats_ptr->sq_size == MAX_SB_SIZE) ? 4 : 1);
            ...
            uint32_t input_tu_origin_index = ((tu_index & 0x1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] * input_picture_ptr->stride_y);
            uint32_t residual_quant_tu_origin_index = tu_index * (tu_size[context_ptr->ep_block_stats_ptr->tx_size]) *
                (tu_size[context_ptr->ep_block_stats_ptr->tx_size]);
            uint32_t pred_recon_tu_origin_index = ((tu_index & 0x1) *
                                                   tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] * MAX_SB_SIZE);
            int tu_coeff_bits = 0;
            ...
            perform_coding_loop(
                ...
                &(candidate_buffer->prediction_ptr
                      ->buffer_y[context_ptr->block_origin_index + pred_recon_tu_origin_index]),
                candidate_buffer->prediction_ptr->stride_y,
                &(((int16_t *)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_y)
                      [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(((int16_t *)candidate_buffer->recon_coeff_ptr->buffer_y)
                      [residual_quant_tu_origin_index]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(candidate_buffer->recon_ptr->buffer_y[context_ptr->block_origin_index + pred_recon_tu_origin_index]),
                candidate_buffer->recon_ptr->stride_y,
        ...
        if ((context_ptr->chroma_level == CHROMA_LEVEL_0 || context_ptr->chroma_level == CHROMA_LEVEL_1) &&
            ...
            perform_coding_loop(
                ...
                &(candidate_buffer->prediction_ptr->buffer_cb[context_ptr->block_chroma_origin_index]),
                candidate_buffer->prediction_ptr->stride_cb,
                &(((int16_t *)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_cb)
                      [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(((int16_t *)candidate_buffer->recon_coeff_ptr->buffer_cb)
                      [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(candidate_buffer->recon_ptr->buffer_cb[context_ptr->block_chroma_origin_index]),
                candidate_buffer->recon_ptr->stride_cb,
            ...
            perform_coding_loop(
                ...
                &(candidate_buffer->prediction_ptr->buffer_cr[context_ptr->block_chroma_origin_index]),
                candidate_buffer->prediction_ptr->stride_cr,
                &(((int16_t *)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_cr)
                      [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(((int16_t *)candidate_buffer->recon_coeff_ptr->buffer_cr)
                      [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                &(candidate_buffer->recon_ptr->buffer_cr[context_ptr->block_chroma_origin_index]),
                candidate_buffer->recon_ptr->stride_cr,
                quants->uv_zbin[qindex],
            ...
            if (context_ptr->eob_zero_mode) {
                if (candidate_buffer->candidate_ptr->mode_info->mode > TM_PRED) {
                    MACROBLOCKD *const xd    = context_ptr->e_mbd;
                    int                rate1 = y_coeff_bits + cb_coeff_bits + cr_coeff_bits +
                        vp9_cost_bit(vp9_get_skip_prob(&cpi->common, xd), 0);
                    int rate2       = vp9_cost_bit(vp9_get_skip_prob(&cpi->common, xd), 1);
                    int distortion1 = yfull_distortion[DIST_CALC_RESIDUAL] + cbfull_distortion[DIST_CALC_RESIDUAL] +
                        crfull_distortion[DIST_CALC_RESIDUAL];
                    int distortion2 = yfull_distortion[DIST_CALC_PREDICTION] + cbfull_distortion[DIST_CALC_PREDICTION] +
                        crfull_distortion[DIST_CALC_PREDICTION];
                    ...
                    if (rd1 >= rd2) {
                        for (uint8_t tu_index = 0;
                            ...
                            if (candidate_buffer->candidate_ptr->eob[0][tu_index]) {
                                uint32_t pred_recon_tu_origin_index =
                                    ((tu_index % 2) * tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] * MAX_SB_SIZE);
                                candidate_buffer->candidate_ptr->eob[0][tu_index] = 0;
                                int arr_index = tu_size[context_ptr->ep_block_stats_ptr->tx_size_uv] == 32
                                    ? 4
                                    : context_ptr->ep_block_stats_ptr->tx_size - 1;
                        ...
                        if (candidate_buffer->candidate_ptr->eob[1][0]) {
                            ...
                            if (arr_index >= 0 && arr_index < 9)
                                pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][arr_index](
                                    &(candidate_buffer->prediction_ptr
                                          ->buffer_cb[context_ptr->block_chroma_origin_index]),
                                    candidate_buffer->prediction_ptr->stride_cb,
                                    &(candidate_buffer->recon_ptr->buffer_cb[context_ptr->block_chroma_origin_index]),
                                    candidate_buffer->recon_ptr->stride_cb,
                        ...
                        if (candidate_buffer->candidate_ptr->eob[2][0]) {
                            ...
                            if (arr_index >= 0 && arr_index < 9)
                                pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][arr_index](
                                    &(candidate_buffer->prediction_ptr
                                          ->buffer_cr[context_ptr->block_chroma_origin_index]),
                                    candidate_buffer->prediction_ptr->stride_cr,
                                    &(candidate_buffer->recon_ptr->buffer_cr[context_ptr->block_chroma_origin_index]),
                                    candidate_buffer->recon_ptr->stride_cr,
                                    tu_size[context_ptr->ep_block_stats_ptr->tx_size_uv],
...
void generate_intra_reference_samples(SequenceControlSet *sequence_control_set_ptr, EncDecContext *context_ptr,
    ...
    const int have_top  = ((plane == 0 && context_ptr->block_origin_y > 0) ||
                          ((ROUND_UV(context_ptr->block_origin_y) >> 1) > 0));
    const int have_left = ((plane == 0 && context_ptr->block_origin_x > 0) ||
                           ((ROUND_UV(context_ptr->block_origin_x) >> 1) > 0));
        ...
        if (context_ptr->block_origin_y != 0 && context_ptr->block_origin_x != 0)
            context_ptr->intra_above_ref[0] = context_ptr->intra_left_ref[0] =
                context_ptr->luma_recon_neighbor_array
                    ->top_left_array[MAX_PICTURE_HEIGHT_SIZE + context_ptr->block_origin_x -
        ...
        if (((ROUND_UV(context_ptr->block_origin_y) >> 1) > 0) && ((ROUND_UV(context_ptr->block_origin_x) >> 1) > 0))
            context_ptr->intra_above_ref[0] = context_ptr->intra_left_ref[0] =
                context_ptr->cb_recon_neighbor_array
                    ->top_left_array[(MAX_PICTURE_HEIGHT_SIZE >> 1) + (ROUND_UV(context_ptr->block_origin_x) >> 1) -
...
void update_recon_neighbor_arrays(EbPictureBufferDesc *input_picture_ptr, EncDecContext *context_ptr, SbUnit *sb_ptr,
    ...
    if (context_ptr->intra_md_open_loop_flag) {
        // Input Samples - Luma
        eb_vp9_neighbor_array_unit_sample_write(context_ptr->luma_recon_neighbor_array,
        ...
        // Set recon buffer
        EbPictureBufferDesc *recon_buffer = (recon_location) ? context_ptr->bdp_pillar_scratch_recon_buffer
                                                             : candidate_buffer->recon_ptr;
        ...
        if ((context_ptr->chroma_level == CHROMA_LEVEL_0 || context_ptr->chroma_level == CHROMA_LEVEL_1) &&
            context_ptr->ep_block_stats_ptr->has_uv) {
            // Recon Samples - Cb
            eb_vp9_neighbor_array_unit_sample_write(context_ptr->cb_recon_neighbor_array,
            ...
            // Recon Samples - Cr
            eb_vp9_neighbor_array_unit_sample_write(context_ptr->cr_recon_neighbor_array,
...
void update_bdp_recon_neighbor_arrays(PictureControlSet   *picture_control_set_ptr,
    ...
    if (context_ptr->intra_md_open_loop_flag) {
        // Input Samples - Luma
        eb_vp9_neighbor_array_unit_sample_write(picture_control_set_ptr->md_luma_recon_neighbor_array[depth_part_stage],
                                                input_picture_ptr->buffer_y,
                                                input_picture_ptr->stride_y,
                                                input_picture_ptr->origin_x + context_ptr->block_origin_x,
                                                input_picture_ptr->origin_y + context_ptr->block_origin_y,
...
void set_sb_rate_context(PictureControlSet *picture_control_set_ptr, EncDecContext *context_ptr, MACROBLOCKD *xd) {
    ...
    for (int i = 0; i < MAX_MB_PLANE; ++i) {
        xd->above_context[i] = context_ptr->above_context +
...
void set_block_rate_context(PictureControlSet *picture_control_set_ptr, EncDecContext *context_ptr, MACROBLOCKD *xd) {
    ...
    eb_vp9_get_entropy_contexts(context_ptr->ep_block_stats_ptr->bsize,
                                context_ptr->ep_block_stats_ptr->tx_size,
...
void update_scratch_recon_buffer(EncDecContext *context_ptr, EbPictureBufferDesc *recon_src_ptr,
    ...
    if ((context_ptr->ep_block_stats_ptr->sq_size >> 3) < 9) {
        pic_copy_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) &&
                                       1][context_ptr->ep_block_stats_ptr->sq_size >> 3](
            &(recon_src_ptr->buffer_y[context_ptr->ep_block_stats_ptr->origin_x +
                                      context_ptr->ep_block_stats_ptr->origin_y * recon_src_ptr->stride_y]),
            recon_src_ptr->stride_y,
            &(recon_dst_ptr->buffer_y[context_ptr->ep_block_stats_ptr->origin_x +
                                      context_ptr->ep_block_stats_ptr->origin_y * recon_dst_ptr->stride_y]),
...
EB_BOOL check_skip_sub_blocks(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    if (sb_params_ptr->is_complete_sb && context_ptr->e_mbd->left_mi != NULL && context_ptr->e_mbd->above_mi != NULL &&
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_OPEN_LOOP_DEPTH_MODE ||
            ...
            if (picture_control_set_ptr->sb_ptr_array[sb_ptr->sb_index]->aura_status == AURA_STATUS_0 &&
                (picture_control_set_ptr->parent_pcs_ptr->sb_stat_array[sb_ptr->sb_index])
...
void next_mdc_block_index(MdcSbData *mdc_sb_data_ptr, EB_BOOL skip_sub_blocks, uint16_t *mdc_block_index) {
    ...
    if (skip_sub_blocks == EB_TRUE) {
        ...
        while (*mdc_block_index < mdc_sb_data_ptr->block_count &&
               (ep_get_block_stats((&mdc_sb_data_ptr->block_data_array[*mdc_block_index])->block_index)->origin_x <
                origin_x + size) &&
               (ep_get_block_stats((&mdc_sb_data_ptr->block_data_array[*mdc_block_index])->block_index)->origin_y <
                origin_y + size)) {
            (*mdc_block_index)++;
        }
    } else {
...
void search_uv_mode(PictureControlSet *picture_control_set_ptr, EbPictureBufferDesc *input_picture_ptr,
    ...
    for (uv_mode = DC_PRED; uv_mode <= TM_PRED; uv_mode++) {
        ...
        perform_coding_loop(
            ...
            &(((int16_t *)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_cb)
                  [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
            &(((int16_t *)context_ptr->uv_mode_search_recon_coeff_buffer->buffer_cb)
                  [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
        ...
        perform_coding_loop(
            ...
            &(((int16_t *)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_cr)
                  [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
            &(((int16_t *)context_ptr->uv_mode_search_recon_coeff_buffer->buffer_cr)
                  [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
...
void eb_vp9_mode_decision_sb(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    context_ptr->luma_recon_neighbor_array =
        picture_control_set_ptr->md_luma_recon_neighbor_array[context_ptr->depth_part_stage];
    context_ptr->cb_recon_neighbor_array =
        picture_control_set_ptr->md_cb_recon_neighbor_array[context_ptr->depth_part_stage];
    context_ptr->cr_recon_neighbor_array =
        picture_control_set_ptr->md_cr_recon_neighbor_array[context_ptr->depth_part_stage];
    ...
    while (ep_block_index < EP_BLOCK_MAX_COUNT) {
        if (ep_get_block_stats(ep_block_index)->shape == PART_N) {
            ...
            context_ptr->bmi_index          = ((context_ptr->block_origin_x >> 2) & 0x1) +
                (((context_ptr->block_origin_y >> 2) & 0x1) << 1);
            context_ptr->enc_dec_local_block_array[ep_block_index]->cost = MAX_BLOCK_COST;
            context_ptr->block_ptr->partition_context                    = partition_plane_context(
                xd, context_ptr->mi_row, context_ptr->mi_col, context_ptr->ep_block_stats_ptr->bsize);
            ...
            if (mdc_block_index < mdc_sb_data_ptr->block_count && ep_block_index == part_block_data_ptr->block_index) {
                ...
                context_ptr->input_origin_index = (context_ptr->block_origin_y + input_picture_ptr->origin_y) *
                        input_picture_ptr->stride_y +
                    (context_ptr->block_origin_x + input_picture_ptr->origin_x);
                context_ptr->input_chroma_origin_index = ((ROUND_UV(context_ptr->block_origin_y) >> 1) +
                                                          (input_picture_ptr->origin_y >> 1)) *
                        input_picture_ptr->stride_cb +
                    ((ROUND_UV(context_ptr->block_origin_x) >> 1) + (input_picture_ptr->origin_x >> 1));
                context_ptr->block_origin_index = context_ptr->ep_block_stats_ptr->origin_y * MAX_SB_SIZE +
                    context_ptr->ep_block_stats_ptr->origin_x;
                context_ptr->block_chroma_origin_index = (ROUND_UV(context_ptr->ep_block_stats_ptr->origin_y) >> 1) *
                        (MAX_SB_SIZE >> 1) +
                    (ROUND_UV(context_ptr->ep_block_stats_ptr->origin_x) >> 1);

                context_ptr->e_mbd->mb_to_top_edge = -((context_ptr->mi_row * MI_SIZE) * 8);
                context_ptr->e_mbd->mb_to_bottom_edge =
                    ((picture_control_set_ptr->parent_pcs_ptr->cpi->common.mi_rows -
                      eb_vp9_num_8x8_blocks_high_lookup[context_ptr->ep_block_stats_ptr->bsize] - context_ptr->mi_row) *
                     MI_SIZE) *
                    8;
                context_ptr->e_mbd->mb_to_left_edge = -((context_ptr->mi_col * MI_SIZE) * 8);
                context_ptr->e_mbd->mb_to_right_edge =
                    ((picture_control_set_ptr->parent_pcs_ptr->cpi->common.mi_cols -
                      eb_vp9_num_8x8_blocks_wide_lookup[context_ptr->ep_block_stats_ptr->bsize] - context_ptr->mi_col) *
                     MI_SIZE) *
                    8;

                // Set above_mi and left_mi
                context_ptr->e_mbd->above_mi = (context_ptr->mi_row > 0)
                    ? context_ptr
                          ->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride - cm->mi_stride]
                    : NULL;
                context_ptr->e_mbd->left_mi  = (context_ptr->mi_col > 0)
                     ? context_ptr->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride - 1]
                     : NULL;
                ...
                if (context_ptr->ep_block_stats_ptr->sq_size < MAX_CU_SIZE) {
                    for (int plane = 0; plane < (int)(((context_ptr->chroma_level == CHROMA_LEVEL_0 ||
                                                        context_ptr->chroma_level == CHROMA_LEVEL_1) &&
                                                       context_ptr->ep_block_stats_ptr->has_uv)
                                                          ? MAX_MB_PLANE
                                                          : 1);
                         ++plane) {
                        generate_intra_reference_samples(sequence_control_set_ptr, context_ptr, plane);
                    }
                }
                ...
                if (context_ptr->ep_block_stats_ptr->sq_size < MAX_CU_SIZE) {
                    if (context_ptr->ep_block_stats_ptr->bsize >= BLOCK_8X8) {
                        if (context_ptr->chroma_level == CHROMA_LEVEL_0) {
                            search_uv_mode(picture_control_set_ptr, input_picture_ptr, context_ptr);
                        }
                    }
                }

                prepare_fast_loop_candidates(
                    picture_control_set_ptr, context_ptr, sb_ptr, &buffer_total_count, &fast_candidate_total_count);

                // If we want to recon N candidate, we would need N+1 buffers
                max_buffers = MIN(buffer_total_count + 1,
                                  context_ptr->buffer_depth_index_width[context_ptr->ep_block_stats_ptr->depth]);

                perform_fast_loop(picture_control_set_ptr,
                                  context_ptr,
                                  sb_ptr,
                                  candidate_buffer_ptr_array_base,
                ...
                candidate_index = full_mode_decision(context_ptr,
                                                     candidate_buffer_ptr_array,
                                                     full_candidate_total_count,
                                                     context_ptr->best_candidate_index_array,
                                                     ep_block_index,
                ...
                if (sb_ptr->coded_block_array_ptr[last_block_index]->split_flag == EB_FALSE) {
                    ...
                    context_ptr->block_origin_x     = (uint16_t)sb_ptr->origin_x +
                        context_ptr->ep_block_stats_ptr->origin_x;
                    context_ptr->block_origin_y = (uint16_t)sb_ptr->origin_y +
                        context_ptr->ep_block_stats_ptr->origin_y;
                    context_ptr->mi_col = context_ptr->block_origin_x >> MI_SIZE_LOG2;
                    context_ptr->mi_row = context_ptr->block_origin_y >> MI_SIZE_LOG2;
                    ...
                    // Update Neighbor Arrays
                    {
                        ...
                        if (context_ptr->ep_block_stats_ptr->bsize < BLOCK_8X8) {
                            ...
                            context_ptr->mode_info_array[mi_index]->bmi[context_ptr->bmi_index].as_mode =
                                context_ptr->enc_dec_local_block_array[last_block_index]
                                    ->mode_info.bmi[context_ptr->bmi_index]
                                    .as_mode;

                            if (context_ptr->ep_block_stats_ptr->is_last_quadrant) {
                                context_ptr->mode_info_array[mi_index]->sb_type =
                                    context_ptr->enc_dec_local_block_array[last_block_index]->mode_info.sb_type;
                                context_ptr->mode_info_array[mi_index]->mode =
                                ...
                                context_ptr->mode_info_array[mi_index]->ref_frame[0] =
                                    context_ptr->enc_dec_local_block_array[last_block_index]->mode_info.ref_frame[0];
                                context_ptr->mode_info_array[mi_index]->ref_frame[1] =
                                    context_ptr->enc_dec_local_block_array[last_block_index]->mode_info.ref_frame[1];
                            ...
                            int mi_row_index;
                            int mi_col_index;
                            for (mi_row_index = context_ptr->mi_row; mi_row_index <
                                 context_ptr->mi_row + ((int)context_ptr->ep_block_stats_ptr->bheight >> MI_SIZE_LOG2);
                                 mi_row_index++) {
                        ...
                        if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_SB_SWITCH_DEPTH_MODE &&
                            picture_control_set_ptr->bdp_present_flag) {
                            // Update bdp pillar neighbor sample arrays
                            update_bdp_recon_neighbor_arrays(
                                picture_control_set_ptr, input_picture_ptr, context_ptr, sb_ptr, candidate_buffer, 1);
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_SB_SWITCH_DEPTH_MODE &&
        picture_control_set_ptr->bdp_present_flag) {
        update_bdp_sb_rate_context(picture_control_set_ptr, context_ptr, xd, 1);

        update_bdp_sb_rate_context(picture_control_set_ptr, context_ptr, xd, 2);
    }
}
...
void bdp_pillar_sb(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    construct_pillar_block_data_array(sequence_control_set_ptr, context_ptr, sb_ptr);
    ...
    while (ep_block_index < EP_BLOCK_MAX_COUNT) {
        if (ep_get_block_stats(ep_block_index)->shape == PART_N) {
            ...
            if (bdp_block_index < context_ptr->bdp_block_data.block_count &&
                ep_block_index == part_block_data_ptr->block_index) {
                context_ptr->block_ptr->split_flag = part_block_data_ptr->split_flag;
...
void bdp_64x64_vs_32x32_sb(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    uint16_t ep_block_index = 0;
    {
        ...
        // Set above_mi and left_mi
        context_ptr->e_mbd->above_mi = (context_ptr->mi_row > 0)
            ? context_ptr->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride - cm->mi_stride]
            : NULL;
        context_ptr->e_mbd->left_mi  = (context_ptr->mi_col > 0)
             ? context_ptr->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride - 1]
             : NULL;
        ...
        candidate_buffer_ptr_array = &(
            candidate_buffer_ptr_array_base[context_ptr
                                                ->buffer_depth_index_start[context_ptr->ep_block_stats_ptr->depth]]);
        ...
        // pre_mode_decision
        pre_mode_decision((second_fast_candidate_total_count == buffer_total_count) ? buffer_total_count : max_buffers,
                          candidate_buffer_ptr_array,
                          &full_candidate_total_count,
                          context_ptr->best_candidate_index_array,
                          (EB_BOOL)(second_fast_candidate_total_count == buffer_total_count));
        ...
        // Inter Depth Decision
        {
            ...
            depth_n_plus_one_cost = context_ptr->enc_dec_local_block_array[5]->cost +
                ...
                context_ptr->enc_dec_local_block_array[512]->cost + depth_n_plus_one_part_cost;
    ...
    while (ep_block_index < EP_BLOCK_MAX_COUNT) {
        if (sb_ptr->coded_block_array_ptr[ep_block_index]->split_flag == EB_FALSE) {
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_LIGHT_BDP_DEPTH_MODE ||
                (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_SB_SWITCH_DEPTH_MODE &&
                 (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_ptr->sb_index] ==
                  SB_LIGHT_BDP_DEPTH_MODE))) {
                cu16x16RefinementFlag = context_ptr->ep_block_stats_ptr->sq_size == 16 &&
                    context_ptr->enc_dec_local_block_array[ep_block_index]->mode_info.mode <= TM_PRED;
            } else {
                cu16x16RefinementFlag = context_ptr->ep_block_stats_ptr->sq_size == 16;
            }
            ...
            if (cu16x16RefinementFlag) {
                ...
                for (int block_index = 0; block_index < 4; block_index++) {
                    ...
                    context_ptr->block_origin_index = context_ptr->ep_block_stats_ptr->origin_y * MAX_SB_SIZE +
                        context_ptr->ep_block_stats_ptr->origin_x;
                    context_ptr->block_chroma_origin_index = (ROUND_UV(context_ptr->ep_block_stats_ptr->origin_y) >>
                                                              1) *
                            (MAX_SB_SIZE >> 1) +
                        (ROUND_UV(context_ptr->ep_block_stats_ptr->origin_x) >> 1);
                    ...
                    context_ptr->e_mbd->mb_to_bottom_edge =
                        ((picture_control_set_ptr->parent_pcs_ptr->cpi->common.mi_rows -
                          eb_vp9_num_8x8_blocks_high_lookup[context_ptr->ep_block_stats_ptr->bsize] -
                          context_ptr->mi_row) *
                         MI_SIZE) *
                        8;
                    context_ptr->e_mbd->mb_to_left_edge = -((context_ptr->mi_col * MI_SIZE) * 8);
                    context_ptr->e_mbd->mb_to_right_edge =
                        ((picture_control_set_ptr->parent_pcs_ptr->cpi->common.mi_cols -
                          eb_vp9_num_8x8_blocks_wide_lookup[context_ptr->ep_block_stats_ptr->bsize] -
                          context_ptr->mi_col) *
                         MI_SIZE) *
                        8;
                    ...
                    // Set above_mi and left_mi
                    context_ptr->e_mbd->above_mi = (context_ptr->mi_row > 0)
                        ? context_ptr->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride -
                                                       cm->mi_stride]
                        : NULL;
                    context_ptr->e_mbd->left_mi  = (context_ptr->mi_col > 0)
                         ? context_ptr->mode_info_array[context_ptr->mi_col + context_ptr->mi_row * cm->mi_stride - 1]
...

                    {
                        ...
                        // Update Neighbor Arrays
                        {
                            ...
                            // Update neighbor sample arrays
                            update_recon_neighbor_arrays(input_picture_ptr,
                                                         context_ptr,
                                                         sb_ptr,
                                                         candidate_buffer,
                                                         last_block_index == ep_block_index);
            ...
            ep_block_index +=
                (uint16_t)((ep_raster_scan_block_depth[ep_scan_to_raster_scan[ep_block_index]] < 4)
                               ? ep_inter_depth_offset
                               : ep_intra_depth_offset
                                     [ep_raster_scan_block_depth[ep_scan_to_raster_scan[ep_block_index]]]);
        }
    }
...
EB_EXTERN EbErrorType encode_pass_sb(SequenceControlSet *sequence_control_set_ptr,
    ...
    context_ptr->luma_recon_neighbor_array = context_ptr->is16bit
        ? picture_control_set_ptr->ep_luma_recon_neighbor_array_16bit
        : picture_control_set_ptr->ep_luma_recon_neighbor_array;
    context_ptr->cb_recon_neighbor_array   = context_ptr->is16bit
          ? picture_control_set_ptr->ep_cb_recon_neighbor_array_16bit
          : picture_control_set_ptr->ep_cb_recon_neighbor_array;
    context_ptr->cr_recon_neighbor_array   = context_ptr->is16bit
          ? picture_control_set_ptr->ep_cr_recon_neighbor_array_16bit
    ...
    if (sb_ptr->sb_index == 0) {
        ...
        memset(
            ...
            0,
            sizeof(*picture_control_set_ptr->ep_above_context) * MAX_MB_PLANE * 2 * mi_cols_aligned_to_sb(cm->mi_cols));

        memset(
            picture_control_set_ptr->ep_left_context,
            0,
        ...
        if (context_ptr->block_ptr->split_flag == EB_FALSE) {
                ...
                EB_MEMCPY(
                    ...
                    &picture_control_set_ptr->ep_left_context[(sb_ptr->origin_y >> 2) +
                                                              (i * sizeof(*picture_control_set_ptr->ep_left_context) *
                                                               2 * mi_cols_aligned_to_sb(cm->mi_rows))],
                    16);
            }
            ...
            for (int plane = 0; plane < (int)(context_ptr->ep_block_stats_ptr->has_uv ? MAX_MB_PLANE : 1); ++plane) {
                EbByte pred_buffer = (plane == 0)
                    ...
                    : (plane == 1) ? context_ptr->prediction_buffer->buffer_cb + context_ptr->block_chroma_origin_index
                                   : context_ptr->prediction_buffer->buffer_cr + context_ptr->block_chroma_origin_index;

                uint16_t pred_stride = (plane == 0) ? context_ptr->prediction_buffer->stride_y
                    : (plane == 1)                  ? context_ptr->prediction_buffer->stride_cb
            ...
            for (uint8_t tu_index = 0; tu_index < ((context_ptr->ep_block_stats_ptr->sq_size == MAX_SB_SIZE) ? 4 : 1);
                ...
                uint32_t input_tu_origin_index = ((tu_index % 2) * tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] * input_picture_ptr->stride_y);
                uint32_t residual_quant_tu_origin_index = tu_index *
                    (tu_size[context_ptr->ep_block_stats_ptr->tx_size]) *
                    (tu_size[context_ptr->ep_block_stats_ptr->tx_size]);
                uint32_t pred_recon_tu_origin_index = ((tu_index % 2) *
                                                       tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] *
                     context_ptr->recon_buffer->stride_y);
                uint32_t pred_tu_origin_index = ((tu_index % 2) * tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] *
                ...
                perform_coding_loop(
                    ...
                    &(((int16_t *)sb_ptr->quantized_coeff_buffer[0])[sb_ptr->quantized_coeff_buffer_block_offset[0] +
                                                                     residual_quant_tu_origin_index]),
                    context_ptr->ep_block_stats_ptr->sq_size,
                    &(input_picture_ptr->buffer_y[context_ptr->input_origin_index + input_tu_origin_index]),
                    input_picture_ptr->stride_y,
                    &((context_ptr->prediction_buffer
                           ->buffer_y)[context_ptr->block_origin_index + pred_tu_origin_index]),
                    context_ptr->prediction_buffer->stride_y,
                    &(((int16_t *)context_ptr->residual_buffer->buffer_y)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                    &(((int16_t *)context_ptr->transform_buffer->buffer_y)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                ...
                if (!(context_ptr->skip_eob_zero_mode_ep && context_ptr->block_ptr->eob[0][tu_index] == 0)) {
                    perform_dist_rate_calc(
                        ...
                        picture_control_set_ptr,
                        &(((int16_t *)
                               sb_ptr->quantized_coeff_buffer[0])[sb_ptr->quantized_coeff_buffer_block_offset[0] +
                                                                  residual_quant_tu_origin_index]),
                        &(input_picture_ptr->buffer_y[context_ptr->input_origin_index + input_tu_origin_index]),
                        input_picture_ptr->stride_y,
                        &((context_ptr->prediction_buffer
            ...
            if (context_ptr->ep_block_stats_ptr->has_uv) {
                perform_coding_loop(
                    ...
                    &(((int16_t *)context_ptr->residual_buffer->buffer_cb)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                    &(((int16_t *)context_ptr->transform_buffer->buffer_cb)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                ...
                if (!(context_ptr->skip_eob_zero_mode_ep && context_ptr->block_ptr->eob[1][0] == 0)) {
                    perform_dist_rate_calc(
                        ...
                        picture_control_set_ptr,
                        &(((int16_t *)
                               sb_ptr->quantized_coeff_buffer[1])[sb_ptr->quantized_coeff_buffer_block_offset[1]]),
                        &(input_picture_ptr->buffer_cb[context_ptr->input_chroma_origin_index]),
                        input_picture_ptr->stride_cb,
                        &((context_ptr->prediction_buffer->buffer_cb)[context_ptr->block_chroma_origin_index]),
                        context_ptr->prediction_buffer->stride_cb,
                ...
                perform_coding_loop(
                    ...
                    &((context_ptr->prediction_buffer->buffer_cr)[context_ptr->block_chroma_origin_index]),
                    context_ptr->prediction_buffer->stride_cr,
                    &(((int16_t *)context_ptr->residual_buffer->buffer_cr)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                    &(((int16_t *)context_ptr->transform_buffer->buffer_cr)
                          [0]), // Hsan - does not match block location (i.e. stride not used) - kept as is to do not change WebM kernels
                ...
                if (!(context_ptr->skip_eob_zero_mode_ep && context_ptr->block_ptr->eob[2][0] == 0)) {
                    perform_dist_rate_calc(
                        ...
                        picture_control_set_ptr,
                        &(((int16_t *)
                               sb_ptr->quantized_coeff_buffer[2])[sb_ptr->quantized_coeff_buffer_block_offset[2]]),
                        &(input_picture_ptr->buffer_cr[context_ptr->input_chroma_origin_index]),
                        input_picture_ptr->stride_cr,
                        &((context_ptr->prediction_buffer->buffer_cr)[context_ptr->block_chroma_origin_index]),
                        context_ptr->prediction_buffer->stride_cr,
                    ...
                    if (rd1 >= rd2) {
                        for (uint8_t tu_index = 0;
                            ...
                            if (context_ptr->block_ptr->eob[0][tu_index]) {
                                uint32_t pred_recon_tu_origin_index =
                                    ...
                                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] *
                                     context_ptr->recon_buffer->stride_y);
                                uint32_t pred_tu_origin_index = ((tu_index % 2) *
                                                                 tu_size[context_ptr->ep_block_stats_ptr->tx_size]) +
                                    ((tu_index > 1) * tu_size[context_ptr->ep_block_stats_ptr->tx_size] *
                                     context_ptr->prediction_buffer->stride_y);
                                context_ptr->block_ptr->eob[0][tu_index] = 0;
                                if (do_recon)
                                    pic_copy_kernel_func_ptr_array
                                        [(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1]
                                        [tu_size[context_ptr->ep_block_stats_ptr->tx_size] == 32
                                             ? 4
                                             : context_ptr->ep_block_stats_ptr->tx_size](
                                            &(context_ptr->prediction_buffer
                                                  ->buffer_y[context_ptr->block_origin_index + pred_tu_origin_index]),
                                            context_ptr->prediction_buffer->stride_y,
                                            &(context_ptr->recon_buffer
                                                  ->buffer_y[cuOriginReconIndex + pred_recon_tu_origin_index]),
                                            context_ptr->recon_buffer->stride_y,
                                            tu_size[context_ptr->ep_block_stats_ptr->tx_size],
                                            tu_size[context_ptr->ep_block_stats_ptr->tx_size]);
                            }
                        ...
                        if (context_ptr->block_ptr->eob[1][0]) {
                            ...
                            if (do_recon)
                                pic_copy_kernel_func_ptr_array
                                    [(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1]
                                    [tu_size[context_ptr->ep_block_stats_ptr->tx_size_uv] == 32
                                         ? 4
                                         : context_ptr->ep_block_stats_ptr->tx_size - 1](
                                        &((context_ptr->prediction_buffer
                                               ->buffer_cb)[context_ptr->block_chroma_origin_index]),
            ...
            sb_ptr->quantized_coeff_buffer_block_offset[0] += (context_ptr->ep_block_stats_ptr->sq_size *
                                                               context_ptr->ep_block_stats_ptr->sq_size);
            if (context_ptr->ep_block_stats_ptr->has_uv) {
                sb_ptr->quantized_coeff_buffer_block_offset[1] += (context_ptr->ep_block_stats_ptr->sq_size_uv *
                                                                   context_ptr->ep_block_stats_ptr->sq_size_uv);
                sb_ptr->quantized_coeff_buffer_block_offset[2] += (context_ptr->ep_block_stats_ptr->sq_size_uv *
                                                                   context_ptr->ep_block_stats_ptr->sq_size_uv);
            }
...
/******************************************************
         ...
         ******************************************************/
EbErrorType eb_vp9_enc_dec_context_ctor(EncDecContext **context_dbl_ptr,
    ...
    *context_dbl_ptr = context_ptr;
    ...
    // Residual scratch buffer
    {
        ...
        return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&context_ptr->transform_buffer, (EbPtr)&init_data);
        if (return_error == EB_ErrorInsufficientResources) {
            return EB_ErrorInsufficientResources;
        }
    }
    ...
    EB_MALLOC(ModeDecisionCandidate *,
              context_ptr->fast_candidate_array,
              sizeof(ModeDecisionCandidate) * MODE_DECISION_CANDIDATE_MAX_COUNT,
              EB_N_PTR);

    EB_MALLOC(ModeDecisionCandidate **,
              context_ptr->fast_candidate_ptr_array,
              sizeof(ModeDecisionCandidate *) * MODE_DECISION_CANDIDATE_MAX_COUNT,
              EB_N_PTR);
    ...
    for (uint32_t candidate_index = 0; candidate_index < MODE_DECISION_CANDIDATE_MAX_COUNT; ++candidate_index) {
        ...
        EB_MALLOC(
            ModeInfo *, context_ptr->fast_candidate_ptr_array[candidate_index]->mode_info, sizeof(ModeInfo), EB_N_PTR);
    }

    // Transform and Quantization Buffers
    EB_MALLOC(EbTransQuantBuffers *, context_ptr->trans_quant_buffers_ptr, sizeof(EbTransQuantBuffers), EB_N_PTR);

    return_error = eb_vp9_trans_quant_buffers_ctor(context_ptr->trans_quant_buffers_ptr);
    ...
    // Cost Arrays
    EB_MALLOC(uint64_t *,
              context_ptr->fast_cost_array,
              sizeof(uint64_t) * MODE_DECISION_CANDIDATE_BUFFER_MAX_COUNT,
              EB_N_PTR);

    EB_MALLOC(uint64_t *,
              context_ptr->full_cost_array,
              sizeof(uint64_t) * MODE_DECISION_CANDIDATE_BUFFER_MAX_COUNT,
              EB_N_PTR);
    ...
    EB_MALLOC(ModeDecisionCandidateBuffer **,
              ...
              sizeof(ModeDecisionCandidateBuffer *) * MODE_DECISION_CANDIDATE_BUFFER_MAX_COUNT,
              EB_N_PTR);
    ...
    EB_MALLOC(EncDecBlockUnit **,
              ...
              sizeof(EncDecBlockUnit *) * EP_BLOCK_MAX_COUNT,
              EB_N_PTR);

    for (uint16_t block_index = 0; block_index < EP_BLOCK_MAX_COUNT; ++block_index) {
        EB_MALLOC(
            EncDecBlockUnit *, context_ptr->enc_dec_local_block_array[block_index], sizeof(EncDecBlockUnit), EB_N_PTR);
    }
...
static EB_BOOL assign_enc_dec_segments(EncDecSegments *segment_ptr, uint16_t *segment_in_out_index,
    ...
    case ENCDEC_TASKS_CONTINUE:
        ...
        if (row_segment_index < segment_ptr->segment_row_count - 1 &&
            bottom_left_segment_index >= segment_ptr->row_array[row_segment_index + 1].starting_seg_index) {
        ...
        if (feedback_row_index > 0) {
            eb_vp9_get_empty_object(srm_fifo_ptr, &wrapper_ptr);
...
static void recon_output(PictureControlSet *picture_control_set_ptr, SequenceControlSet *sequence_control_set_ptr) {
    ...
    // Get Recon Buffer
    eb_vp9_get_empty_object(sequence_control_set_ptr->encode_context_ptr->recon_output_fifo_ptr,
    ...
    // Copy the Reconstructed Picture to the Output Recon Buffer
    {
        ...
        // Y Recon Samples
        sampleTotalCount = ((reconPtr->max_width - sequence_control_set_ptr->max_input_pad_right) *
                            (reconPtr->max_height - sequence_control_set_ptr->max_input_pad_bottom))
            << is16bit;
        reconReadPtr = reconPtr->buffer_y + (reconPtr->origin_y << is16bit) * reconPtr->stride_y +
            (reconPtr->origin_x << is16bit);
        reconWritePtr = &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);

        CHECK_REPORT_ERROR(outputReconPtr->n_filled_len + sampleTotalCount <= outputReconPtr->n_alloc_len,
                           encode_context_ptr->app_callback_ptr,
                           EB_ENC_ROB_OF_ERROR);

        // Initialize Y recon buffer
        eb_vp9_picture_copy_kernel(reconReadPtr,
                                   reconPtr->stride_y,
                                   reconWritePtr,
        ...
        sampleTotalCount = ((reconPtr->max_width - sequence_control_set_ptr->max_input_pad_right) *
                            ...
                            2)
            << is16bit;
        reconReadPtr = reconPtr->buffer_cb + ((reconPtr->origin_y << is16bit) >> 1) * reconPtr->stride_cb +
            ((reconPtr->origin_x << is16bit) >> 1);
        reconWritePtr = &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);
        ...
        sampleTotalCount = ((reconPtr->max_width - sequence_control_set_ptr->max_input_pad_right) *
                            ...
                            2)
            << is16bit;
        reconReadPtr = reconPtr->buffer_cr + ((reconPtr->origin_y << is16bit) >> 1) * reconPtr->stride_cr +
            ((reconPtr->origin_x << is16bit) >> 1);
        reconWritePtr = &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);
...
static void pad_ref_and_set_flags(PictureControlSet  *picture_control_set_ptr,
    ...
    if (!is16bit) {
        // Y samples
        eb_vp9_generate_padding(ref_pic_ptr->buffer_y,
        ...
        // Cb samples
        eb_vp9_generate_padding(ref_pic_ptr->buffer_cb,
        ...
        // Cr samples
        eb_vp9_generate_padding(ref_pic_ptr->buffer_cr,
    ...
    if (is16bit) {
        // Y samples
        eb_vp9_generate_padding_16bit(ref_pic16_bit_ptr->buffer_y,
        ...
        // Cb samples
        eb_vp9_generate_padding_16bit(ref_pic16_bit_ptr->buffer_cb,
        ...
        // Cr samples
        eb_vp9_generate_padding_16bit(ref_pic16_bit_ptr->buffer_cr,
...
void copy_statistics_to_ref_object(PictureControlSet *picture_control_set_ptr) {
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
        ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
            ...
            picture_control_set_ptr->parent_pcs_ptr->non_moving_index_array[sb_index];
    }

    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
        ->tmp_layer_idx = (uint8_t)picture_control_set_ptr->temporal_layer_index;
    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
        ->is_scene_change = picture_control_set_ptr->parent_pcs_ptr->scene_change_flag;
}
...
void *eb_vp9_enc_dec_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->mode_decision_input_fifo_ptr, &enc_dec_tasks_wrapper_ptr);
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag == EB_TRUE) {
            ...
            context_ptr->recon_buffer = context_ptr->is16bit ? picture_control_set_ptr->recon_picture_16bit_ptr
        ...
        while (assign_enc_dec_segments(
            ...
            eb_vp9_reset_mode_decision(context_ptr, picture_control_set_ptr, segment_index);

            // Reset EncDec Coding State
            reset_enc_dec(context_ptr, picture_control_set_ptr, sequence_control_set_ptr, segment_index);

            if (picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr != NULL) {
                ((EbReferenceObject *)
                     picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
                    ->pic_avg_variance = picture_control_set_ptr->parent_pcs_ptr->pic_avg_variance;
                ((EbReferenceObject *)
                     picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
                    ->average_intensity = picture_control_set_ptr->parent_pcs_ptr->average_intensity[0];
            }

            for (ysb_index = y_sb_start_index, sb_segment_index = sb_start_index;
                 sb_segment_index < sb_start_index + sb_segment_count;
                 ++ysb_index) {
                for (xsb_index = x_sb_start_index;
                    ...
                    context_ptr->restrict_intra_global_motion =
                        (sequence_control_set_ptr->static_config.tune == TUNE_SQ &&
                         ...
                         picture_control_set_ptr->parent_pcs_ptr->non_moving_index_array[sb_index] <
                             INTRA_GLOBAL_MOTION_NON_MOVING_INDEX_TH &&
                         picture_control_set_ptr->parent_pcs_ptr->y_mean[sb_index][PA_RASTER_SCAN_CU_INDEX_64x64] <
                    ...
                    if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_FULL85_DEPTH_MODE ||
                        ...
                        (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_SB_SWITCH_DEPTH_MODE &&
                         (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                              SB_FULL85_DEPTH_MODE ||
                          picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                              SB_FULL84_DEPTH_MODE ||
                          picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                              SB_OPEN_LOOP_DEPTH_MODE ||
                          picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                              SB_LIGHT_OPEN_LOOP_DEPTH_MODE ||
                        ...
                        context_ptr->depth_part_stage = 2;
                        bdp_8x8_refinement_sb(sequence_control_set_ptr, picture_control_set_ptr, context_ptr, sb_ptr);
        ...
        if (last_sb_flag) {
            ...
            if (sequence_control_set_ptr->static_config.loop_filter) {
                ...
                if (lf_application_enable_flag) {
                    ...
                    // Set mi_grid_visible
                    picture_control_set_ptr->parent_pcs_ptr->cpi->common.mi_grid_visible =
                        picture_control_set_ptr->mode_info_array;

                    context_ptr->e_mbd->plane[0].dst.buf =
                        &context_ptr->recon_buffer
                             ->buffer_y[context_ptr->recon_buffer->origin_x +
                                        context_ptr->recon_buffer->stride_y * context_ptr->recon_buffer->origin_y];
                    context_ptr->e_mbd->plane[0].dst.stride = context_ptr->recon_buffer->stride_y;

                    context_ptr->e_mbd->plane[1].dst.buf =
                        &context_ptr->recon_buffer->buffer_cb[(context_ptr->recon_buffer->origin_x >> 1) +
                                                              context_ptr->recon_buffer->stride_cb *
                                                                  (context_ptr->recon_buffer->origin_y >> 1)];
                    context_ptr->e_mbd->plane[1].dst.stride = context_ptr->recon_buffer->stride_cb;

                    context_ptr->e_mbd->plane[2].dst.buf =
                        &context_ptr->recon_buffer->buffer_cr[(context_ptr->recon_buffer->origin_x >> 1) +
                                                              context_ptr->recon_buffer->stride_cr *
                                                                  (context_ptr->recon_buffer->origin_y >> 1)];
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr != NULL) {
                copy_statistics_to_ref_object(picture_control_set_ptr);
            }
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag == EB_TRUE &&
                ...
                const uint32_t src_luma_off_set = input_picture_ptr->origin_x +
                    input_picture_ptr->origin_y * input_picture_ptr->stride_y;
                const uint32_t src_cb_offset = (input_picture_ptr->origin_x >> 1) +
                    (input_picture_ptr->origin_y >> 1) * input_picture_ptr->stride_cb;
                const uint32_t src_cr_offset = (input_picture_ptr->origin_x >> 1) +
                    (input_picture_ptr->origin_y >> 1) * input_picture_ptr->stride_cr;

                EbReferenceObject *reference_object = (EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
                ...
                const uint32_t ref_cb_offset    = (ref_den_pic->origin_x >> 1) +
                    (ref_den_pic->origin_y >> 1) * ref_den_pic->stride_cb;
                const uint32_t ref_cr_offset = (ref_den_pic->origin_x >> 1) +
                    (ref_den_pic->origin_y >> 1) * ref_den_pic->stride_cr;
                ...
                eb_vp9_generate_padding(ref_den_pic->buffer_cb,
                ...
                eb_vp9_generate_padding(ref_den_pic->buffer_cr,
            ...
            if (encode_context_ptr->recon_port_active) {
                recon_output(picture_control_set_ptr, sequence_control_set_ptr);
            }

            if (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) {
                // Get Empty EntropyCoding Results
                eb_vp9_get_empty_object(context_ptr->picture_demux_output_fifo_ptr, &picture_demux_results_wrapper_ptr);

                picture_demux_results_ptr = (PictureDemuxResults *)picture_demux_results_wrapper_ptr->object_ptr;
                picture_demux_results_ptr->reference_picture_wrapper_ptr =
                    picture_control_set_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr;
                picture_demux_results_ptr->sequence_control_set_wrapper_ptr =
