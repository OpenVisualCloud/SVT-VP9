...
EbErrorType eb_vp9_initial_rate_control_reorder_entry_ctor(InitialRateControlReorderEntry **entry_dbl_ptr,
...
EbErrorType eb_vp9_hl_rate_control_histogram_entry_ctor(HlRateControlHistogramEntry **entry_dbl_ptr,
                                                        uint32_t                      picture_number) {
    ...
    EB_MALLOC(
        uint16_t *, (*entry_dbl_ptr)->me_distortion_histogram, sizeof(uint16_t) * NUMBER_OF_SAD_INTERVALS, EB_N_PTR);

    EB_MALLOC(uint16_t *,
              (*entry_dbl_ptr)->ois_distortion_histogram,
