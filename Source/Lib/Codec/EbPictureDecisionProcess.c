...
#define POC_CIRCULAR_ADD(base, offset /*, bits*/)                                                                          \
    (/*(((int32_t) (base)) + ((int32_t) (offset)) > ((int32_t) (1 << (bits))))   ? ((base) + (offset) - (1 << (bits))) : \
...
#define QUEUE_GET_NEXT_SPOT(h, off)                                                                               \
    (((h + off) >= PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH) ? h + off - PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH \
                                                             : h + off)
 ...
 ************************************************/
EbErrorType eb_vp9_picture_decision_context_ctor(PictureDecisionContext **context_dbl_ptr,
    ...
    *context_dbl_ptr = context_ptr;
    ...
    EB_MALLOC(
        uint32_t **, context_ptr->ahd_running_avg_cb, sizeof(uint32_t *) * MAX_NUMBER_OF_REGIONS_IN_WIDTH, EB_N_PTR);

    EB_MALLOC(
        uint32_t **, context_ptr->ahd_running_avg_cr, sizeof(uint32_t *) * MAX_NUMBER_OF_REGIONS_IN_WIDTH, EB_N_PTR);
    ...
    for (array_index = 0; array_index < MAX_NUMBER_OF_REGIONS_IN_WIDTH; array_index++) {
        EB_MALLOC(uint32_t *,
                  context_ptr->ahd_running_avg_cb[array_index],
                  sizeof(uint32_t) * MAX_NUMBER_OF_REGIONS_IN_HEIGHT,
                  EB_N_PTR);

        EB_MALLOC(uint32_t *,
                  context_ptr->ahd_running_avg_cr[array_index],
                  sizeof(uint32_t) * MAX_NUMBER_OF_REGIONS_IN_HEIGHT,
                  EB_N_PTR);

        EB_MALLOC(uint32_t *,
                  context_ptr->ahd_running_avg[array_index],
                  sizeof(uint32_t) * MAX_NUMBER_OF_REGIONS_IN_HEIGHT,
                  EB_N_PTR);
    }
...
***************************************************************************************************/
static EbErrorType release_prev_picture_from_reorder_queue(EncodeContext *encode_context_ptr) {
    ...
    previous_entry_index     = (encode_context_ptr->picture_decision_reorder_queue_head_index == 0)
...
EbErrorType eb_vp9_generate_picture_window_split(PictureDecisionContext *context_ptr,
    ...
    while (mini_gop_index < MINI_GOP_MAX_COUNT) {
        ...
        if (eb_vp9_get_mini_gop_stats(mini_gop_index)->end_index < encode_context_ptr->pre_assignment_buffer_count &&
            ...
            context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops] =
                eb_vp9_get_mini_gop_stats(mini_gop_index)->end_index;
            context_ptr->mini_gop_length[context_ptr->total_number_of_mini_gops] =
                eb_vp9_get_mini_gop_stats(mini_gop_index)->lenght;
            context_ptr->mini_gop_hierarchical_levels[context_ptr->total_number_of_mini_gops] =
                eb_vp9_get_mini_gop_stats(mini_gop_index)->hierarchical_levels;
    ...
    if (context_ptr->total_number_of_mini_gops != 0) {
        context_ptr->mini_gop_intra_count[context_ptr->total_number_of_mini_gops - 1] =
            encode_context_ptr->pre_assignment_buffer_intra_count;
        context_ptr->mini_gop_idr_count[context_ptr->total_number_of_mini_gops - 1] =
...
EbErrorType eb_vp9_handle_incomplete_picture_window_map(PictureDecisionContext *context_ptr,
    ...
    if (context_ptr->total_number_of_mini_gops == 0) {
        ...
        context_ptr->mini_gop_length[context_ptr->total_number_of_mini_gops] =
            ...
            context_ptr->mini_gop_start_index[context_ptr->total_number_of_mini_gops];
        context_ptr->mini_gop_hierarchical_levels[context_ptr->total_number_of_mini_gops] =
            3; // MIN_HIERARCHICAL_LEVEL; // AMIR to be updated after other predictions are supported

        context_ptr->total_number_of_mini_gops++;

    } else if (context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops - 1] <
               encode_context_ptr->pre_assignment_buffer_count - 1) {
        context_ptr->mini_gop_start_index[context_ptr->total_number_of_mini_gops] =
            context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops - 1] + 1;
        context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops] =
            encode_context_ptr->pre_assignment_buffer_count - 1;
        context_ptr->mini_gop_length[context_ptr->total_number_of_mini_gops] =
            encode_context_ptr->pre_assignment_buffer_count -
...
*
***************************************************************************************************/
static EbErrorType update_base_layer_reference_queue_dependent_count(PictureDecisionContext *context_ptr,
                                                                     EncodeContext          *encode_context_ptr,
                                                                     uint32_t                mini_gop_index) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    picture_control_set_ptr = (PictureParentControlSet *)encode_context_ptr
                                  ->pre_assignment_buffer[context_ptr->mini_gop_start_index[mini_gop_index]]
                                  ->object_ptr;
    ...
    if (picture_control_set_ptr->hierarchical_layers_diff != 0) {
        ...
        while (inputQueueIndex != encode_context_ptr->picture_decision_pa_reference_queue_tail_index) {
            ...
            if (inputEntryPtr->picture_number == (picture_control_set_ptr->picture_number - 1)) {
                ...
                nextBaseLayerPredPositionPtr =
                    nextPredStructPtr->pred_struct_entry_ptr_array[nextPredStructPtr->pred_struct_entry_count - 1];
                ...
                for (depIdx = 0; depIdx < nextBaseLayerPredPositionPtr->dep_list0.list_count; ++depIdx) {
                    if (nextBaseLayerPredPositionPtr->dep_list0.list[depIdx] >= 0) {
                        inputEntryPtr->list0.list[inputEntryPtr->list0.list_count++] =
                            nextBaseLayerPredPositionPtr->dep_list0.list[depIdx];
                    }
                }
                ...
                for (depIdx = 0; depIdx < nextBaseLayerPredPositionPtr->dep_list1.list_count; ++depIdx) {
                    if (nextBaseLayerPredPositionPtr->dep_list1.list[depIdx] >= 0) {
                        inputEntryPtr->list1.list[inputEntryPtr->list1.list_count++] =
                            nextBaseLayerPredPositionPtr->dep_list1.list[depIdx];
                    }
                }

                // 3rd step: update the dependant count
                dependantListRemovedEntries = inputEntryPtr->dep_list0_count + inputEntryPtr->dep_list1_count -
                ...
                for (depIdx = 0; depIdx < depListCount; ++depIdx) {
                    ...
                    if (depPoc >= picture_control_set_ptr->picture_number && inputEntryPtr->list0.list[depIdx]) {
                        ...
                        CHECK_REPORT_ERROR(inputEntryPtr->dependent_count != ~0u,
            ...
            inputQueueIndex = (inputQueueIndex == PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH - 1)
                ? 0
                : inputQueueIndex + 1;
        }
    }
...
*
***************************************************************************************************/
static EbErrorType generate_mini_gop_rps(PictureDecisionContext *context_ptr, EncodeContext *encode_context_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
        ...
        for (picture_index = context_ptr->mini_gop_start_index[mini_gop_index];
            ...
            picture_control_set_ptr =
                (PictureParentControlSet *)encode_context_ptr->pre_assignment_buffer[picture_index]->object_ptr;
            ...
            picture_control_set_ptr->hierarchical_levels =
                (uint8_t)context_ptr->mini_gop_hierarchical_levels[mini_gop_index];
...
EbErrorType eb_vp9_signal_derivation_multi_processes_sq(SequenceControlSet      *sequence_control_set_ptr,
    ...
    if (picture_control_set_ptr->enc_mode <= ENC_MODE_1) {
        ...
        picture_control_set_ptr->cu8x8_mode = (picture_control_set_ptr->is_used_as_reference_flag) ? CU_8x8_MODE_0
                                                                                                   : CU_8x8_MODE_1;
    } else if (picture_control_set_ptr->enc_mode == ENC_MODE_7) {
        picture_control_set_ptr->cu8x8_mode = (picture_control_set_ptr->temporal_layer_index == 0) ? CU_8x8_MODE_0
                                                                                                   : CU_8x8_MODE_1;
    } else {
...
void *eb_vp9_picture_decision_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->picture_analysis_results_input_fifo_ptr, &input_results_wrapper_ptr);
        ...
        queue_entry_index =
            (int32_t)(picture_control_set_ptr->picture_number -
                      encode_context_ptr
                          ->picture_decision_reorder_queue[encode_context_ptr
                                                               ->picture_decision_reorder_queue_head_index]
                          ->picture_number);
        queue_entry_index += encode_context_ptr->picture_decision_reorder_queue_head_index;
        queue_entry_index = (queue_entry_index > PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH - 1)
            ? queue_entry_index - PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH
            : queue_entry_index;
        ...
        if (queue_entry_ptr->parent_pcs_wrapper_ptr != NULL) {
            CHECK_REPORT_ERROR_NC(encode_context_ptr->app_callback_ptr, EB_ENC_PD_ERROR7);
        } else {
        ...
        while (queue_entry_ptr->parent_pcs_wrapper_ptr != EB_NULL) {
            ...
            if (encode_context_ptr->picture_decision_reorder_queue[previous_entry_index]->parent_pcs_wrapper_ptr ==
                ...
                for (window_index = 0; window_index < FUTURE_WINDOW_WIDTH; window_index++) {
                    entry_index = QUEUE_GET_NEXT_SPOT(encode_context_ptr->picture_decision_reorder_queue_head_index,
                                                      window_index + 1);
            ...
            if (window_avail == EB_TRUE || frame_passe_thru == EB_TRUE) {
                ...
                // Setup the PCS & SCS
                picture_control_set_ptr = (PictureParentControlSet *)encode_context_ptr
                                              ->pre_assignment_buffer[encode_context_ptr->pre_assignment_buffer_count]
                                              ->object_ptr;

                // Set the POC Number
                picture_control_set_ptr->picture_number =
                    (encode_context_ptr->current_input_poc +
                     1) /*& ((1 << sequence_control_set_ptr->bitsForPictureOrderCount)-1)*/;
                ...
                release_prev_picture_from_reorder_queue(encode_context_ptr);
                ...
                if ((encode_context_ptr->pre_assignment_buffer_intra_count > 0) ||
                    ...
                    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops;
                        ...
                        for (picture_index = context_ptr->mini_gop_start_index[mini_gop_index];
                             ...
                             ++picture_index) {
                            picture_control_set_ptr = (PictureParentControlSet *)encode_context_ptr
                                                          ->pre_assignment_buffer[picture_index]
                                                          ->object_ptr;
                            sequence_control_set_ptr = (SequenceControlSet *)picture_control_set_ptr
                            ...
                            if (picture_control_set_ptr->idr_flag == EB_TRUE) {
                                ...
                                encode_context_ptr->pred_struct_position =
                                    picture_control_set_ptr->pred_struct_ptr->init_pic_index + 1;
                            }
                            ...
                            encode_context_ptr->pred_struct_position =
                                (encode_context_ptr->pred_struct_position ==
                                 picture_control_set_ptr->pred_struct_ptr->pred_struct_entry_count)
                                ? encode_context_ptr->pred_struct_position -
                                    picture_control_set_ptr->pred_struct_ptr->pred_struct_period
                                : encode_context_ptr->pred_struct_position;
                            ...
                            pred_position_ptr =
                                picture_control_set_ptr->pred_struct_ptr
                                    ->pred_struct_entry_ptr_array[encode_context_ptr->pred_struct_position];
                            ...
                            case B_SLICE:
                                ...
                                encode_context_ptr->elapsed_non_idr_count = MIN(
                                    encode_context_ptr->elapsed_non_idr_count + 1, MAX_ELAPSED_IDR_COUNT);

                                CHECK_REPORT_ERROR(picture_control_set_ptr->pred_struct_ptr->pred_struct_entry_count <
                            ...
                            default:
                                ...
                                CHECK_REPORT_ERROR_NC(encode_context_ptr->app_callback_ptr, EB_ENC_PD_ERROR2);

                                break;
                            }
                            picture_control_set_ptr->pred_struct_index = (uint8_t)
                                                                             encode_context_ptr->pred_struct_position;
                            picture_control_set_ptr->temporal_layer_index = (uint8_t)
                                                                                pred_position_ptr->temporal_layer_index;
                            ...
                            {
                                picture_control_set_ptr->decode_order = encode_context_ptr->decode_base_number +
                                    pred_position_ptr->decode_order;
                            } else {
                            ...
                            picture_control_set_ptr->cpi->common.refresh_frame_context =
                                1; /* Two state 0 = NO, 1 = YES */
                            memset(picture_control_set_ptr->cpi->common.ref_frame_sign_bias,
                                   0,
                                   MAX_REF_FRAMES * sizeof(int));
                            ...
                            if (picture_control_set_ptr->slice_type == I_SLICE) {
                                ...
                                while (input_queue_index !=
                                    ...
                                    input_entry_ptr =
                                        encode_context_ptr->picture_decision_pa_reference_queue[input_queue_index];
                                    ...
                                    for (dep_idx = 0; dep_idx < dep_list_count; ++dep_idx) {
                                        ...
                                        if (dep_poc >= picture_control_set_ptr->picture_number &&
                                            ...
                                            CHECK_REPORT_ERROR(input_entry_ptr->dependent_count != ~0u,
                                                               encode_context_ptr->app_callback_ptr,
                                                               EB_ENC_PD_ERROR3);
                                        }
                                    }
                            ...
                            input_entry_ptr = encode_context_ptr->picture_decision_pa_reference_queue
                                                  [encode_context_ptr->picture_decision_pa_reference_queue_tail_index];
                            ...
                            encode_context_ptr->picture_decision_pa_reference_queue_tail_index =
                                (encode_context_ptr->picture_decision_pa_reference_queue_tail_index ==
                                 PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH - 1)
                                ? 0
                                : encode_context_ptr->picture_decision_pa_reference_queue_tail_index + 1;
                            ...
                            picture_control_set_ptr->ref_list0_count = (picture_type == I_SLICE)
                                ? 0
                                : (uint8_t)pred_position_ptr->ref_list0.reference_list_count;
                            picture_control_set_ptr->ref_list1_count = (picture_type == I_SLICE)
                                ? 0
                                : (uint8_t)pred_position_ptr->ref_list1.reference_list_count;
...

                            {
                                ...
                                for (dep_idx = 0; dep_idx < pred_position_ptr->dep_list0.list_count; ++dep_idx) {
                                    if (pred_position_ptr->dep_list0.list[dep_idx] >= 0) {
                                        input_entry_ptr->list0.list[input_entry_ptr->list0.list_count++] =
                                ...
                                input_entry_ptr->dependent_count = input_entry_ptr->dep_list0_count +
                                    input_entry_ptr->dep_list1_count;
                            }

                            ((EbPaReferenceObject *)
                                 picture_control_set_ptr->pareference_picture_wrapper_ptr->object_ptr)
                                ->dependent_pictures_count = input_entry_ptr->dependent_count;
                        ...
                        for (picture_index = context_ptr->mini_gop_start_index[mini_gop_index];
                            ...
                            if ((picture_control_set_ptr->slice_type == P_SLICE) ||
                                ...
                                if (picture_control_set_ptr->ref_list0_count) {
                                    ...
                                    pa_reference_entry_ptr =
                                        encode_context_ptr
                                            ->picture_decision_pa_reference_queue[pa_reference_queue_index];
                                    ...
                                    picture_control_set_ptr->ref_pa_pic_ptr_array[REF_LIST_0] =
                                        pa_reference_entry_ptr->input_object_ptr;
                                    picture_control_set_ptr->ref_pic_poc_array[REF_LIST_0] = ref_poc;
                                    picture_control_set_ptr->ref_pa_pcs_array[REF_LIST_0] =
                                        pa_reference_entry_ptr->p_pcs_ptr;

                                    // Increment the PA Reference's live_count by the number of tiles in the input picture
                                    eb_vp9_object_inc_live_count(pa_reference_entry_ptr->input_object_ptr, 1);

                                    ((EbPaReferenceObject *)picture_control_set_ptr->ref_pa_pic_ptr_array[REF_LIST_0]
                                         ->object_ptr)
                                        ->p_pcs_ptr = pa_reference_entry_ptr->p_pcs_ptr;

                                    eb_vp9_object_inc_live_count(pa_reference_entry_ptr->p_pcs_ptr->p_pcs_wrapper_ptr,
                                                                 1);
                            ...
                            if (picture_control_set_ptr->slice_type == B_SLICE) {
                                if (picture_control_set_ptr->ref_list1_count) {
                                    ...
                                    -input_entry_ptr->list1_ptr->reference_list/*,
                                    sequence_control_set_ptr->bitsForPictureOrderCount*/);
                                    picture_control_set_ptr->ref_pa_pcs_array[REF_LIST_1] =
                                        pa_reference_entry_ptr->p_pcs_ptr;
                                    // Set the Reference Object
                                    picture_control_set_ptr->ref_pa_pic_ptr_array[REF_LIST_1] =
                                        pa_reference_entry_ptr->input_object_ptr;
                                    ...
                                    ((EbPaReferenceObject *)picture_control_set_ptr->ref_pa_pic_ptr_array[REF_LIST_1]
                                         ->object_ptr)
                                        ->p_pcs_ptr = pa_reference_entry_ptr->p_pcs_ptr;
                            ...
                            picture_control_set_ptr->me_segments_column_count =
                                (uint8_t)(sequence_control_set_ptr->me_segment_column_count_array
                                              [picture_control_set_ptr->temporal_layer_index]);
                            picture_control_set_ptr->me_segments_row_count =
                                (uint8_t)(sequence_control_set_ptr->me_segment_row_count_array
                                              [picture_control_set_ptr->temporal_layer_index]);
                            picture_control_set_ptr->me_segments_total_count =
                                (uint16_t)(picture_control_set_ptr->me_segments_column_count *
                                           picture_control_set_ptr->me_segments_row_count);
                            ...
                            // Post the results to the ME processes
                            {
                                ...
                                for (segment_index = 0;
                                     segment_index < picture_control_set_ptr->me_segments_total_count;
                                     ++segment_index) {
                                    // Get Empty Results Object
                                    eb_vp9_get_empty_object(context_ptr->picture_decision_results_output_fifo_ptr,
                                                            &output_results_wrapper_ptr);

                                    output_results_ptr = (PictureDecisionResults *)
                                                             output_results_wrapper_ptr->object_ptr;

                                    output_results_ptr->picture_control_set_wrapper_ptr =
                                        encode_context_ptr->pre_assignment_buffer[picture_index];
                ...
                while (input_queue_index != encode_context_ptr->picture_decision_pa_reference_queue_tail_index) {
                    ...
                    // Remove the entry
                    if ((input_entry_ptr->dependent_count == 0) && (input_entry_ptr->input_object_ptr)) {
                    ...
                    encode_context_ptr->picture_decision_pa_reference_queue_head_index =
                        (encode_context_ptr
                             ->picture_decision_pa_reference_queue[encode_context_ptr
                    ...
                    CHECK_REPORT_ERROR((encode_context_ptr->picture_decision_pa_reference_queue_head_index !=
                                        encode_context_ptr->picture_decision_pa_reference_queue_tail_index) ||
                                           (encode_context_ptr
                                                ->picture_decision_pa_reference_queue
                                                    [encode_context_ptr->picture_decision_pa_reference_queue_head_index]
                    ...
                    // Increment the input_queue_index Iterator
                    input_queue_index = (input_queue_index == PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH - 1)
                        ? 0
                        : input_queue_index + 1;
                }
