...
EbErrorType eb_vp9_input_queue_entry_ctor(InputQueueEntry **entry_dbl_ptr) {
    InputQueueEntry *entry_ptr;
    EB_MALLOC(InputQueueEntry *, entry_ptr, sizeof(InputQueueEntry), EB_N_PTR);
    *entry_dbl_ptr = entry_ptr;

    entry_ptr->input_object_ptr      = (EbObjectWrapper *)NULL;
    entry_ptr->reference_entry_index = 0;
    entry_ptr->dependent_count       = 0;

    entry_ptr->list0_ptr = (ReferenceList *)NULL;
    entry_ptr->list1_ptr = (ReferenceList *)NULL;

    return EB_ErrorNone;
}

EbErrorType eb_vp9_reference_queue_entry_ctor(ReferenceQueueEntry **entry_dbl_ptr) {
