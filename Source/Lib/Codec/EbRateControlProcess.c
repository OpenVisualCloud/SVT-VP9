...
#endif
static const double delta_rate_oq[2][6] = {{0.35, 0.70, 0.85, 1.00, 1.00, 1.00}, // 4L
                                           {0.30, 0.6, 0.8, 0.9, 1.0, 1.0}}; // 5L
static const double delta_rate_sq[6]    = {0.35, 0.50, 0.75, 1.00, 1.00, 1.00};
static const double delta_rate_vmaf[6]  = {0.50, 0.70, 0.85, 1.00, 1.00, 1.00};

// calculate the QP based on the QP scaling
static uint32_t eb_vp9_qp_scaling_calc(SequenceControlSet *sequence_control_set_ptr, EB_SLICE slice_type,
                                       uint32_t temporal_layer_index, uint32_t base_qp) {
    uint32_t scaled_qp;
    int      base_qindex;

    int          qindex = eb_vp9_quantizer_to_qindex(base_qp);
    const double q      = eb_vp9_convert_qindex_to_q(
        qindex, (vpx_bit_depth_t)sequence_control_set_ptr->static_config.encoder_bit_depth);
    RATE_CONTROL rc;
    ...
    if (slice_type == I_SLICE) {
        delta_qindex = eb_vp9_compute_qdelta(
            &rc, q, q * 0.25, (vpx_bit_depth_t)sequence_control_set_ptr->static_config.encoder_bit_depth);
...
* Internal Typedefs
*****************************/
static void rate_control_layer_reset(RateControlLayerContext *rate_control_layer_ptr,
    ...
    rate_control_layer_ptr->target_bit_rate = picture_control_set_ptr->parent_pcs_ptr->target_bit_rate *
        (uint64_t)rate_percentage_layer_array[sequence_control_set_ptr->hierarchical_levels]
        ...
        if (picture_control_set_ptr->picture_number % (sequence_control_set_ptr->intra_period + 1) == 0) {
            ...
            for (temporal_layer_index = 0; temporal_layer_index < EB_MAX_TEMPORAL_LAYERS; temporal_layer_index++) {
                rate_control_context_ptr->frames_in_interval[temporal_layer_index] =
                    picture_control_set_ptr->parent_pcs_ptr->frames_in_interval[temporal_layer_index];
            ...
            rate_control_layer_ptr->target_bit_rate = picture_control_set_ptr->parent_pcs_ptr->target_bit_rate *
                picture_control_set_ptr->parent_pcs_ptr->bits_per_sw_per_layer[rate_control_layer_ptr->temporal_index] /
                sum_bits_per_sw;
#endif
        }
    }
    ...
    rate_control_layer_ptr->channel_bit_rate = (((rate_control_layer_ptr->target_bit_rate << (2 * RC_PRECISION)) /
                                                 rate_control_layer_ptr->frame_rate) +
                                                RC_PRECISION_OFFSET) >>
        RC_PRECISION;
    rate_control_layer_ptr->channel_bit_rate  = (uint64_t)MAX((int64_t)1,
    ...
    rate_control_layer_ptr->alpha = 1 << (RC_PRECISION - 1);
    {
        if (!was_used) {
            ...
            rate_control_layer_ptr->c_coeff = (rate_control_layer_ptr->channel_bit_rate << (2 * RC_PRECISION)) /
...
static void rate_control_layer_reset_part2(RateControlContext      *context_ptr,
    ...
    rate_control_layer_ptr->max_qp = (uint32_t)CLIP3(
        ...
        63,
        (int32_t)
            context_ptr->qp_scaling_map[rate_control_layer_ptr->temporal_index][picture_control_set_ptr->picture_qp]);
...
static EbErrorType high_level_eb_vp9_rate_control_context_ctor(HighLevelRateControlContext **entry_dbl_ptr) {
...
static EbErrorType rate_control_layer_context_ctor(RateControlLayerContext **entry_dbl_ptr) {
...
static EbErrorType rate_control_interval_param_context_ctor(RateControlIntervalParamContext **entry_dbl_ptr) {
    ...
    *entry_dbl_ptr = entry_ptr;
    ...
    EB_MALLOC(RateControlLayerContext **,
              ...
              sizeof(RateControlLayerContext *) * EB_MAX_TEMPORAL_LAYERS,
              EB_N_PTR);
...
EbErrorType eb_vp9_rate_control_context_ctor(RateControlContext **context_dbl_ptr,
    ...
    *context_dbl_ptr = context_ptr;
    ...
    // High level RC
    return_error = high_level_eb_vp9_rate_control_context_ctor(&context_ptr->high_level_rate_control_ptr);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    ...
    EB_MALLOC(RateControlIntervalParamContext **,
              context_ptr->rate_control_param_queue,
              sizeof(RateControlIntervalParamContext *) * PARALLEL_GOP_MAX_NUMBER,
              EB_N_PTR);
    ...
    for (interval_index = 0; interval_index < PARALLEL_GOP_MAX_NUMBER; interval_index++) {
        return_error = rate_control_interval_param_context_ctor(&context_ptr->rate_control_param_queue[interval_index]);
        context_ptr->rate_control_param_queue[interval_index]->first_poc = (interval_index *
                                                                            (uint32_t)(intra_period + 1));
        context_ptr->rate_control_param_queue[interval_index]->last_poc  = ((interval_index + 1) *
                                                                           (uint32_t)(intra_period + 1)) -
            1;
        if (return_error == EB_ErrorInsufficientResources) {
            return EB_ErrorInsufficientResources;
    ...
    EB_MALLOC(CodedFramesStatsEntry **,
              context_ptr->coded_frames_stat_queue,
              sizeof(CodedFramesStatsEntry *) * CODED_FRAMES_STAT_QUEUE_MAX_DEPTH,
              EB_N_PTR);
    ...
    for (picture_index = 0; picture_index < CODED_FRAMES_STAT_QUEUE_MAX_DEPTH; ++picture_index) {
        return_error = eb_vp9_rate_control_coded_frames_stats_context_ctor(
            &context_ptr->coded_frames_stat_queue[picture_index], picture_index);
        if (return_error == EB_ErrorInsufficientResources) {
            return EB_ErrorInsufficientResources;
        }
    }
...
static void high_level_rc_input_picture_vbr(PictureParentControlSet *picture_control_set_ptr,
                                            SequenceControlSet      *sequence_control_set_ptr,
    ...
    if (sequence_control_set_ptr->look_ahead_distance != 0) {
        ...
        hl_rate_control_histogram_ptr_temp =
            (encode_context_ptr
                 ->hl_rate_control_historgram_queue[encode_context_ptr->hl_rate_control_historgram_queue_head_index]);
        ...
        if (picture_control_set_ptr->frames_in_sw < MIN(sequence_control_set_ptr->look_ahead_distance + 1,
            ...
            selected_ref_qp = (uint32_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                              sequence_control_set_ptr->static_config.max_qp_allowed,
                                              selected_ref_qp);

            queue_entry_index_head_temp =
                (int32_t)(picture_control_set_ptr->picture_number -
                          encode_context_ptr
            ...
            queue_entry_index_temp = queue_entry_index_head_temp;
            {
                ...
                if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE)
                    ...
                    ref_qp_index_temp =
                        context_ptr
                            ->qp_scaling_map[hl_rate_control_histogram_ptr_temp->temporal_layer_index][selected_ref_qp];

                ref_qp_index_temp = (uint32_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                                    sequence_control_set_ptr->static_config.max_qp_allowed,
                                                    ref_qp_index_temp);
                ...
                sad_bits_array_ptr =
                    rate_control_tables_ptr->sad_bits_array[hl_rate_control_histogram_ptr_temp->temporal_layer_index];
                intra_sad_bits_array_ptr =
                    rate_control_tables_ptr
                        ->intra_sad_bits_array[hl_rate_control_histogram_ptr_temp->temporal_layer_index];
                pred_bits_ref_qp = 0;
                num_of_full_sbs  = 0;
                ...
                if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE) {
                    // Loop over block in the frame and calculated the predicted bits at reg QP
                    {
                        ...
                        for (i = 0; i < NUMBER_OF_INTRA_SAD_INTERVALS; ++i) {
                            accum += (uint32_t)(hl_rate_control_histogram_ptr_temp->ois_distortion_histogram[i] *
                ...
                else {
                    {
                        ...
                        for (i = 0; i < NUMBER_OF_SAD_INTERVALS; ++i) {
                            accum += (uint32_t)(hl_rate_control_histogram_ptr_temp->me_distortion_histogram[i] *
                ...
                hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp] =
                    hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp] * (uint64_t)area_in_pixel /
                    (num_of_full_sbs << 12);

                // Store the pred_bits_ref_qp for the first frame in the window to PCS
                picture_control_set_ptr->pred_bits_ref_qp[ref_qp_index_temp] =
                    hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp];
            }
        } else {
            // Loop over the QPs and find the best QP
            min_la_bit_distance = MAX_UNSIGNED_VALUE;
            qp_search_min       = (uint8_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                           sequence_control_set_ptr->static_config.max_qp_allowed,
                                           (uint32_t)MAX((int32_t)sequence_control_set_ptr->qp - 40, 0));

            qp_search_max = (uint8_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                           sequence_control_set_ptr->static_config.max_qp_allowed,
            ...
            bit_constraint_per_sw = high_level_rate_control_ptr->bit_constraint_per_sw *
                picture_control_set_ptr->frames_in_sw / (sequence_control_set_ptr->look_ahead_distance + 1);
            ...
            previous_selected_ref_qp    = CLIP3(qp_search_min, qp_search_max, previous_selected_ref_qp);
            ...
            while (ref_qp_table_index >= qp_search_min && ref_qp_table_index <= qp_search_max && !best_qp_found) {
                ...
                while (!end_of_sequence_flag &&
                       queue_entry_index_temp <=
                           queue_entry_index_head_temp + sequence_control_set_ptr->look_ahead_distance) {
                    ...
                    if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE)
                        ...
                        ref_qp_index_temp =
                            context_ptr->qp_scaling_map[hl_rate_control_histogram_ptr_temp->temporal_layer_index]
                                                       [ref_qp_index];
                    ...
                    if (hl_rate_control_histogram_ptr_temp->is_coded) {
                        ...
                        hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp] =
                            hl_rate_control_histogram_ptr_temp->total_num_bitsCoded;
                    } else {
                        rate_control_tables_ptr = &encode_context_ptr->rate_control_tables_array[ref_qp_index_temp];
                        sad_bits_array_ptr =
                            rate_control_tables_ptr
                        ...
                        if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE) {
                            ...
                            uint32_t accum = 0;
                            for (i = 0; i < NUMBER_OF_INTRA_SAD_INTERVALS; ++i) {
                                accum += (uint32_t)((uint32_t)hl_rate_control_histogram_ptr_temp
                                                        ->ois_distortion_histogram[i] *
                    ...
                    high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] +=
                        hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp];
                ...
                if (min_la_bit_distance >=
                    (uint64_t)ABS((int64_t)high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] -
                                  (int64_t)bit_constraint_per_sw)) {
                    min_la_bit_distance = (uint64_t)ABS(
                        (int64_t)high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] -
                        (int64_t)bit_constraint_per_sw);
                    selected_ref_qp_table_index = ref_qp_table_index;
        ...
        if ((sequence_control_set_ptr->intra_period != -1) &&
            ...
            if (picture_control_set_ptr->picture_number % (sequence_control_set_ptr->intra_period + 1) == 0) {
                ...
                while (!end_of_sequence_flag &&
                    ...
                    picture_control_set_ptr->sb_total_bits_per_gop +=
                        hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp];
                ...
                if (picture_control_set_ptr->sb_total_bits_per_gop == 0) {
                    for (temporal_layer_index = 0; temporal_layer_index < EB_MAX_TEMPORAL_LAYERS;
                        ...
                        picture_control_set_ptr->bits_per_sw_per_layer[temporal_layer_index] =
                            rate_percentage_layer_array[sequence_control_set_ptr->hierarchical_levels]
                                                       [temporal_layer_index];
                    }
                }
            }
            ...
            for (temporal_layer_index = 0; temporal_layer_index < EB_MAX_TEMPORAL_LAYERS; temporal_layer_index++) {
                picture_control_set_ptr->bits_per_sw_per_layer[temporal_layer_index] =
                    rate_percentage_layer_array[sequence_control_set_ptr->hierarchical_levels][temporal_layer_index];
            }
        }
        ...
        if (picture_control_set_ptr->picture_number > max_coded_poc &&
            picture_control_set_ptr->temporal_layer_index < 2 && !picture_control_set_ptr->end_of_sequence_region) {
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE)
            ...
            picture_control_set_ptr->best_pred_qp =
                (uint8_t)context_ptr->qp_scaling_map[picture_control_set_ptr->temporal_layer_index][selected_ref_qp];

        picture_control_set_ptr->best_pred_qp = (uint8_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                                               sequence_control_set_ptr->static_config.max_qp_allowed,
                                                               picture_control_set_ptr->best_pred_qp);
...
static void frame_level_rc_input_picture_vbr(PictureControlSet               *picture_control_set_ptr,
                                             SequenceControlSet              *sequence_control_set_ptr,
                                             RateControlContext              *context_ptr,
                                             RateControlLayerContext         *rate_control_layer_ptr,
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->first_frame_in_temporal_layer ||
        (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc)) {
        if (sequence_control_set_ptr->enable_qp_scaling_flag &&
            ...
            picture_control_set_ptr->picture_qp = (uint8_t)CLIP3(
                ...
                (int32_t)(rate_control_param_ptr->intra_frames_qp +
                          context_ptr->qp_scaling_map[picture_control_set_ptr->temporal_layer_index]
                                                     [rate_control_param_ptr->intra_frames_qp_bef_scal] -
                          context_ptr->qp_scaling_map_I_SLICE[rate_control_param_ptr->intra_frames_qp_bef_scal]));
        }
                ...
                if (picture_control_set_ptr->parent_pcs_ptr->end_of_sequence_region) {
                    if ((context_ptr->extra_bits > (int64_t)(context_ptr->virtual_buffer_size << 4)) ||
                        ...
                        extra_bits_per_gop = CLIP3(-(int64_t)(context_ptr->vb_fill_threshold2 << 3),
                                                   ...
                                                   extra_bits_per_gop);
                    } else if ((context_ptr->extra_bits > (int64_t)(context_ptr->virtual_buffer_size << 3)) ||
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE) {
            ...
            if (sequence_control_set_ptr->look_ahead_distance == 0) {
                ...
                selected_ref_qp             = selected_ref_qp_table_index;
                qp_search_min               = (uint32_t)CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                                sequence_control_set_ptr->static_config.max_qp_allowed,
                                                (uint32_t)MAX((int32_t)sequence_control_set_ptr->qp - 40, 0));

                qp_search_max = CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                                      sequence_control_set_ptr->static_config.max_qp_allowed,
                                      sequence_control_set_ptr->qp + 40);

                if (!sequence_control_set_ptr->encode_context_ptr->rate_control_tables_array_updated) {
                    context_ptr->intra_coef_rate = CLIP3(
                        1, 2, (uint32_t)(rate_control_layer_ptr->frame_rate >> 16) / 4);
                } else {
                    ...
                    context_ptr->intra_coef_rate = CLIP3(1,
                ...
                for (ref_qp_table_index = qp_search_min; ref_qp_table_index < qp_search_max; ref_qp_table_index++) {
                    ...
                    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
                        ...
                        if (sb_params_ptr->is_complete_sb) {
                            ...
                            intra_sad_interval_index =
                                picture_control_set_ptr->parent_pcs_ptr->intra_sad_interval_index[sb_index];
                            picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index] +=
                                sequence_control_set_ptr->encode_context_ptr->rate_control_tables_array[ref_qp_index]
                                    .intra_sad_bits_array[picture_control_set_ptr->temporal_layer_index]
                                                         [intra_sad_interval_index];
                        }
                    }
                    ...
                    picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index] =
                        picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index] *
                        rate_control_layer_ptr->area_in_pixel / (num_of_full_sbs << 12);

                    if (min_la_bit_distance >
                        (uint64_t)ABS(
                            (int64_t)rate_control_layer_ptr->ec_bit_constraint * context_ptr->intra_coef_rate -
                            (int64_t)picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index])) {
                        min_la_bit_distance = (uint64_t)ABS(
                            (int64_t)rate_control_layer_ptr->ec_bit_constraint * context_ptr->intra_coef_rate -
                            (int64_t)picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index]);
                ...
                if (!sequence_control_set_ptr->encode_context_ptr->rate_control_tables_array_updated) {
                    ...
                    rate_control_layer_ptr->calculated_frame_qp = (uint8_t)MAX((int32_t)selected_ref_qp - (int32_t)1,
                                                                               0);
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->end_of_sequence_region) {
                if (rate_control_param_ptr->virtual_buffer_level >= context_ptr->vb_fill_threshold2 << 1) {
                    ...
                    picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                        (uint8_t)THRESHOLD1QPINCREASE;
                }
                if (rate_control_param_ptr->virtual_buffer_level <= -(context_ptr->vb_fill_threshold2 << 2))
                    picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                        (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE - (int32_t)2, 0);
                else if (rate_control_param_ptr->virtual_buffer_level <= -(context_ptr->vb_fill_threshold2 << 1))
                    picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                        (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE - (int32_t)1, 0);
                ...
                if (rate_control_param_ptr->virtual_buffer_level <= -(context_ptr->vb_fill_threshold2 << 2))
                    picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp -
                        (uint8_t)THRESHOLD2QPINCREASE - (int32_t)2;
                else if (rate_control_param_ptr->virtual_buffer_level <= -(context_ptr->vb_fill_threshold2 << 1))
                    picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp -
                        (uint8_t)THRESHOLD2QPINCREASE - (int32_t)1;
                else if (rate_control_param_ptr->virtual_buffer_level <= 0)
                    picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                        (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE, 0);
            }
            ...
            for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
                ...
                if (sb_params_ptr->is_complete_sb) {
                    picture_control_set_ptr->parent_pcs_ptr->sad_me +=
                        picture_control_set_ptr->parent_pcs_ptr->rcme_distortion[sb_index];
                }
            }

            //  tileSadMe is normalized based on the area because of the LCUs at the tile boundries
            picture_control_set_ptr->parent_pcs_ptr->sad_me = MAX(picture_control_set_ptr->parent_pcs_ptr->sad_me *
        ...
        if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc) {
            ...
            for (temporal_layer_idex = 0; temporal_layer_idex < EB_MAX_TEMPORAL_LAYERS; temporal_layer_idex++) {
                rate_control_layer_temp_ptr = rate_control_param_ptr->rate_control_layer_array[temporal_layer_idex];
                rate_control_layer_reset_part2(context_ptr, rate_control_layer_temp_ptr, picture_control_set_ptr);
            }
        }
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE) {
            ...
            if (sequence_control_set_ptr->look_ahead_distance == 0) {
                ...
                context_ptr->intra_coef_rate = CLIP3(
                    1, (uint32_t)(rate_control_layer_ptr->frame_rate >> 16) / 4, context_ptr->intra_coef_rate);
                // Loop over proper QPs and find the Predicted bits for that QP. Find the QP with the closest total predicted rate to target bits for the sliding window.
                for (ref_qp_table_index = qp_search_min; ref_qp_table_index < qp_search_max; ref_qp_table_index++) {
                    ref_qp_index                                                            = ref_qp_table_index;
                    picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index] = 0;
            ...
            if (sequence_control_set_ptr->look_ahead_distance != 0) {
                ...
                rate_control_layer_ptr->ec_bit_constraint =
                    (rate_control_layer_ptr->alpha * picture_control_set_ptr->parent_pcs_ptr->target_bits_best_pred_qp +
                     ((1 << RC_PRECISION) - rate_control_layer_ptr->alpha) *
                         picture_control_set_ptr->parent_pcs_ptr->target_bits_rc +
                     RC_PRECISION_OFFSET) >>
                    RC_PRECISION;
            ...
            rate_control_layer_ptr->total_mad = MAX(
                picture_control_set_ptr->parent_pcs_ptr->sad_me / rate_control_layer_ptr->area_in_pixel, 1);
...

            {
                ...
                qp_calc_temp2 = MAX(
                    (int64_t)(rate_control_layer_ptr->ec_bit_constraint << (2 * RC_PRECISION)) -
                        (int64_t)rate_control_layer_ptr->c_coeff * (int64_t)rate_control_layer_ptr->area_in_pixel,
                ...
                temp_qp = (uint64_t)(eb_vp9_log2f_high_precision(
                ...
                rate_control_layer_ptr->calculated_frame_qp            = (uint8_t)(CLIP3(
                    1, 63, (uint32_t)(temp_qp + RC_PRECISION_OFFSET) >> RC_PRECISION));
                picture_control_set_ptr->parent_pcs_ptr->calculated_qp = (uint8_t)(CLIP3(
                    1, 63, (uint32_t)(temp_qp + RC_PRECISION_OFFSET) >> RC_PRECISION));
            }
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->first_non_intra_frame_in_temporal_layer &&
            picture_control_set_ptr->temporal_layer_index == 0 && picture_control_set_ptr->slice_type != I_SLICE) {
            picture_control_set_ptr->picture_qp =
                (uint8_t)(rate_control_param_ptr->intra_frames_qp +
                          context_ptr->qp_scaling_map[picture_control_set_ptr->temporal_layer_index]
                                                     [rate_control_param_ptr->intra_frames_qp_bef_scal] -
                          context_ptr->qp_scaling_map_I_SLICE[rate_control_param_ptr->intra_frames_qp_bef_scal]);
        }
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->end_of_sequence_region) {
            if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold2 << 2) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 4;
            } else if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold2 << 1) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 3;
            } else if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold2) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 2;
            } else if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold1 &&
                       rate_control_param_ptr->virtual_buffer_level < context_ptr->vb_fill_threshold2) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD1QPINCREASE + 2;
            }
        } else {
            if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold2 << 2) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                ...
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 1;
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->end_of_sequence_region) {
            if (rate_control_param_ptr->virtual_buffer_level < -(context_ptr->vb_fill_threshold2 << 2))
                picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                    (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE - 2, 0);
            else if (rate_control_param_ptr->virtual_buffer_level < -(context_ptr->vb_fill_threshold2 << 1))
                picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                    (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE - 1, 0);
            else if (rate_control_param_ptr->virtual_buffer_level < 0)
                picture_control_set_ptr->picture_qp = (uint8_t)MAX(
                    (int32_t)picture_control_set_ptr->picture_qp - (int32_t)THRESHOLD2QPINCREASE, 0);
        } else {
        ...
        rate_control_layer_ptr->delta_qp_fraction = CLIP3(
            -RC_PRECISION_OFFSET,
            RC_PRECISION_OFFSET,
            -((int64_t)temp_qp - (int64_t)(picture_control_set_ptr->picture_qp << RC_PRECISION)));
...
static void frame_level_rc_feedback_picture_vbr(PictureParentControlSet *parentpicture_control_set_ptr,
                                                SequenceControlSet      *sequence_control_set_ptr,
                                                RateControlContext      *context_ptr) {
    RateControlLayerContext         *rate_control_layer_temp_ptr;
    RateControlIntervalParamContext *rate_control_param_ptr;
    ...
    rate_control_layer_ptr =
        rate_control_param_ptr->rate_control_layer_array[parentpicture_control_set_ptr->temporal_layer_index];
    ...
    if (parentpicture_control_set_ptr->slice_type != I_SLICE) {
        // Updating c_coeff
        rate_control_layer_ptr->c_coeff = (((int64_t)rate_control_layer_ptr->previous_frame_bit_actual -
        ...
        if (rate_control_layer_ptr->critical_states) {
            ...
            rate_control_layer_ptr->k_coeff =
                (rate_control_layer_ptr->coeff_averaging_weight1 * rate_control_layer_ptr->k_coeff +
                 rate_control_layer_ptr->coeff_averaging_weight2 * rate_control_layer_ptr->previous_k_coeff + 8) >>
                4;
            rate_control_layer_ptr->c_coeff =
                (rate_control_layer_ptr->coeff_averaging_weight1 * rate_control_layer_ptr->c_coeff +
                 rate_control_layer_ptr->coeff_averaging_weight2 * rate_control_layer_ptr->previous_c_coeff + 8) >>
                4;
        }
    ...
    if (sequence_control_set_ptr->look_ahead_distance != 0) {
        if (parentpicture_control_set_ptr->slice_type == I_SLICE) {
            if (parentpicture_control_set_ptr->total_num_bits < parentpicture_control_set_ptr->target_bits_best_pred_qp
                    << 1)
                context_ptr->base_layer_intra_frames_avg_qp = (3 * context_ptr->base_layer_intra_frames_avg_qp +
...

    {
        ...
        if (parentpicture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc &&
                    ...
                    (uint64_t)((int64_t)parentpicture_control_set_ptr->target_bit_rate -
                               MIN((int64_t)parentpicture_control_set_ptr->target_bit_rate * 3 / 4,
                                   (int64_t)(parentpicture_control_set_ptr->total_num_bits * context_ptr->frame_rate /
                                             (sequence_control_set_ptr->static_config.intra_period + 1)) >>
                                       RC_PRECISION)) *
                    rate_percentage_layer_array[sequence_control_set_ptr->hierarchical_levels][temporal_layer_idex] /
                    100;
            ...
            if ((int64_t)parentpicture_control_set_ptr->target_bit_rate * 3 / 4 <
                ...
                context_ptr->extra_bits_gen -= (int64_t)((parentpicture_control_set_ptr->total_num_bits *
                                                          context_ptr->frame_rate /
                                                          (sequence_control_set_ptr->static_config.intra_period + 1)) >>
                                                         RC_PRECISION) -
                    (int64_t)parentpicture_control_set_ptr->target_bit_rate * 3 / 4;
        ...
        if (previous_frame_bit_actual) {
            ...
            rate_control_layer_ptr->bit_diff = CLIP3(-(int64_t)(rate_control_layer_ptr->channel_bit_rate),
                                                     (int64_t)(rate_control_layer_ptr->channel_bit_rate >> 1),
            ...
            rate_control_layer_ptr->bit_constraint = MAX(
                (int64_t)rate_control_layer_ptr->previous_bit_constraint -
                    ((rate_control_layer_ptr->bit_diff << RC_PRECISION) / ((int64_t)bit_changes_rate)),
                1);
            ...
            if (extra_bits > 0) {
                ...
                for (interval_index_temp = 0; interval_index_temp < PARALLEL_GOP_MAX_NUMBER; interval_index_temp++) {
                    if (context_ptr->rate_control_param_queue[interval_index_temp]->in_use &&
                        context_ptr->rate_control_param_queue[interval_index_temp]->virtual_buffer_level >
                            ((int64_t)context_ptr->virtual_buffer_size >> 1)) {
                ...
                if (interval_in_use_count) {
                    ...
                    if (clip_coef1 > 0)
                        extra_bits_per_gop = CLIP3(-(int64_t)context_ptr->virtual_buffer_size >> clip_coef1,
                    ...
                    for (interval_index_temp = 0; interval_index_temp < PARALLEL_GOP_MAX_NUMBER;
                        ...
                        if (context_ptr->rate_control_param_queue[interval_index_temp]->in_use &&
                            ...
                            context_ptr->rate_control_param_queue[interval_index_temp]->virtual_buffer_level -=
                                extra_bits_per_gop;
                            context_ptr->rate_control_param_queue[interval_index_temp]->previous_virtual_buffer_level -=
                                extra_bits_per_gop;
                            context_ptr->extra_bits -= extra_bits_per_gop;
                        }
                    }
                ...
                // if no interval with more consuming was found, allocate it to ones with consuming less
                    ...
                    if (interval_in_use_count) {
                        extra_bits_per_gop = extra_bits / interval_in_use_count;
                        if (clip_coef2 > 0)
                            extra_bits_per_gop = CLIP3(-(int64_t)context_ptr->virtual_buffer_size >> clip_coef2,
...
static void high_level_rc_input_picture_cbr(PictureParentControlSet *picture_control_set_ptr,
                                            SequenceControlSet      *sequence_control_set_ptr,
                                            EncodeContext *encode_context_ptr, RateControlContext *context_ptr,
                                            HighLevelRateControlContext *high_level_rate_control_ptr) {
    bool end_of_sequence_flag = true;
    ...
    if (sequence_control_set_ptr->look_ahead_distance != 0) {
        ...
        if (picture_control_set_ptr->frames_in_sw < MIN(sequence_control_set_ptr->look_ahead_distance + 1,
            ...
            // Loop over proper QPs and find the Predicted bits for that QP. Find the QP with the closest total predicted rate to target bits for the sliding window.
            previous_selected_ref_qp    = CLIP3(qp_search_min + 1, qp_search_max - 1, previous_selected_ref_qp);
            ref_qp_table_index          = previous_selected_ref_qp;
            ref_qp_index                = ref_qp_table_index;
            selected_ref_qp_table_index = ref_qp_table_index;
            selected_ref_qp             = selected_ref_qp_table_index;
            if (sequence_control_set_ptr->intra_period != -1 &&
                ...
                if (ref_qp_index == sequence_control_set_ptr->static_config.max_qp_allowed &&
                    high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] > bit_constraint_per_sw) {
                    delta_qp = (int)((high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] -
                                      bit_constraint_per_sw) *
                                     100 /
                                     (high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index - 1] -
                                      high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index]));
...
static void frame_level_rc_input_picture_cbr(PictureControlSet               *picture_control_set_ptr,
                                             SequenceControlSet              *sequence_control_set_ptr,
                                             RateControlContext              *context_ptr,
                                             RateControlLayerContext         *rate_control_layer_ptr,
                                             RateControlIntervalParamContext *rate_control_param_ptr) {
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->first_frame_in_temporal_layer ||
        ...
        qp_search_max = (uint8_t)CLIP3(
            sequence_control_set_ptr->static_config.min_qp_allowed, MAX_REF_QP_NUM, sequence_control_set_ptr->qp + 40);
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->picture_number +
                picture_control_set_ptr->parent_pcs_ptr->frames_in_sw >
            rate_control_param_ptr->first_poc + sequence_control_set_ptr->static_config.intra_period + 1)
            bit_constraint_per_sw = high_level_rate_control_ptr->bit_constraint_per_sw;
        else
            bit_constraint_per_sw = high_level_rate_control_ptr->bit_constraint_per_sw *
                encode_context_ptr->hl_rate_control_historgram_queue[queue_entry_index_head_temp]->frames_in_sw /
                (sequence_control_set_ptr->look_ahead_distance + 1);
        ...
        while (ref_qp_table_index >= qp_search_min && ref_qp_table_index <= qp_search_max && !best_qp_found) {
            ...
            while (
                ...
                if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE)
                    ...
                    ref_qp_index_temp =
                        context_ptr
                            ->qp_scaling_map[hl_rate_control_histogram_ptr_temp->temporal_layer_index][ref_qp_index];
                ...
                if (hl_rate_control_histogram_ptr_temp->is_coded) {
                    ...
                    rate_control_tables_ptr = &encode_context_ptr->rate_control_tables_array[ref_qp_index_temp];
                    sad_bits_array_ptr      = rate_control_tables_ptr
                                             ->sad_bits_array[hl_rate_control_histogram_ptr_temp->temporal_layer_index];
                    intra_sad_bits_array_ptr = rate_control_tables_ptr->intra_sad_bits_array[0];
                    pred_bits_ref_qp         = 0;
                    num_of_full_sbs          = 0;
                    ...
                    if (hl_rate_control_histogram_ptr_temp->slice_type == I_SLICE) {
                        ...
                        for (i = 0; i < NUMBER_OF_INTRA_SAD_INTERVALS; ++i) {
                            accum +=
                                (uint32_t)((uint32_t)hl_rate_control_histogram_ptr_temp->ois_distortion_histogram[i] *
                                           (uint32_t)intra_sad_bits_array_ptr[i]);
                        }
                        ...
                        for (i = 0; i < NUMBER_OF_SAD_INTERVALS; ++i) {
                            accum +=
                                (uint32_t)((uint32_t)hl_rate_control_histogram_ptr_temp->me_distortion_histogram[i] *
                                           (uint32_t)sad_bits_array_ptr[i]);
                            accum_intra +=
                                (uint32_t)((uint32_t)hl_rate_control_histogram_ptr_temp->ois_distortion_histogram[i] *
                                           (uint32_t)intra_sad_bits_array_ptr[i]);
                    ...
                    hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp] =
                        hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp] *
                        (uint64_t)rate_control_layer_ptr->area_in_pixel / (num_of_full_sbs << 12);
                }
                high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] +=
                    hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp];
                // Store the pred_bits_ref_qp for the first frame in the window to PCS
                if (queue_entry_index_head_temp == queue_entry_index_temp2)
                    picture_control_set_ptr->parent_pcs_ptr->pred_bits_ref_qp[ref_qp_index_temp] =
                        hl_rate_control_histogram_ptr_temp->pred_bits_ref_qp[ref_qp_index_temp];
        ...
        if (ref_qp_index == sequence_control_set_ptr->static_config.max_qp_allowed &&
            high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] > bit_constraint_per_sw) {
            delta_qp =
                (int)((high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index] - bit_constraint_per_sw) * 100 /
                      (high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index - 1] -
                       high_level_rate_control_ptr->pred_bits_ref_qpPerSw[ref_qp_index]));
            delta_qp = (delta_qp + 50) / 100;
        }
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->end_of_sequence_region) {
            ...
            if (rate_control_param_ptr->virtual_buffer_level >
                context_ptr->vb_fill_threshold2 + (int64_t)(context_ptr->virtual_buffer_size * 2 / 3)) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 2;
            ...
            //else if (rate_control_param_ptr->virtual_buffer_level > context_ptr->vb_fill_threshold2 << 1){
                     ...
                     context_ptr->vb_fill_threshold2 + (int64_t)(context_ptr->virtual_buffer_size / 3)) {
                picture_control_set_ptr->picture_qp = picture_control_set_ptr->picture_qp +
                    (uint8_t)THRESHOLD2QPINCREASE + 1;
...
static void frame_level_rc_feedback_picture_cbr(PictureParentControlSet *parentpicture_control_set_ptr,
                                                SequenceControlSet      *sequence_control_set_ptr,
                                                RateControlContext      *context_ptr) {
    RateControlLayerContext         *rate_control_layer_temp_ptr;
    RateControlIntervalParamContext *rate_control_param_ptr;
...
static void high_level_rc_feed_back_picture(PictureParentControlSet *picture_control_set_ptr,
                                            SequenceControlSet      *sequence_control_set_ptr) {
    // Queue variables
    HlRateControlHistogramEntry *hl_rate_control_histogram_ptr_temp;
    uint32_t                     queue_entry_index_head_temp;
    ...
    if (sequence_control_set_ptr->look_ahead_distance != 0) {
        ...
        if (picture_control_set_ptr->picture_number >=
            sequence_control_set_ptr->encode_context_ptr
                ->hl_rate_control_historgram_queue[sequence_control_set_ptr->encode_context_ptr
                                                       ->hl_rate_control_historgram_queue_head_index]
                ->picture_number) {
            queue_entry_index_head_temp =
                (int32_t)(picture_control_set_ptr->picture_number -
                          sequence_control_set_ptr->encode_context_ptr
                              ...
                              ->picture_number);
            queue_entry_index_head_temp +=
                sequence_control_set_ptr->encode_context_ptr->hl_rate_control_historgram_queue_head_index;
            queue_entry_index_head_temp = (queue_entry_index_head_temp >
                                           HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH - 1)
                ? queue_entry_index_head_temp - HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH
                : queue_entry_index_head_temp;

            hl_rate_control_histogram_ptr_temp = (sequence_control_set_ptr->encode_context_ptr
...
static void rate_control_refinement(PictureControlSet               *picture_control_set_ptr,
                                    SequenceControlSet              *sequence_control_set_ptr,
                                    RateControlIntervalParamContext *rate_control_param_ptr,
    ...
    if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc &&
        picture_control_set_ptr->picture_number != 0 && !prev_gop_rate_control_param_ptr->scene_change_in_gop) {
        int16_t deltaApQp = (int16_t)prev_gop_rate_control_param_ptr->first_pic_actual_qp -
    ...
    if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc &&
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->intra_selected_org_qp + 10 <=
            prev_gop_rate_control_param_ptr->first_pic_actual_qp) {
            qpDecAllowed = (uint8_t)(prev_gop_rate_control_param_ptr->first_pic_actual_qp -
                                     picture_control_set_ptr->parent_pcs_ptr->intra_selected_org_qp) >>
                1;
        }

        if (picture_control_set_ptr->parent_pcs_ptr->intra_selected_org_qp >=
            prev_gop_rate_control_param_ptr->first_pic_actual_qp + 10) {
            qpIncAllowed = (uint8_t)(picture_control_set_ptr->parent_pcs_ptr->intra_selected_org_qp -
                                     prev_gop_rate_control_param_ptr->first_pic_actual_qp) *
                2 / 3;
    ...
    if (sequence_control_set_ptr->intra_period != -1 &&
        picture_control_set_ptr->parent_pcs_ptr->hierarchical_levels < 2 &&
        (int32_t)picture_control_set_ptr->temporal_layer_index == 0 && picture_control_set_ptr->slice_type != I_SLICE) {
        if (next_gop_rate_control_param_ptr->first_pic_actual_qp_assigned ||
...
static void eb_vp9_init_rc(RateControlContext *context_ptr, PictureControlSet *picture_control_set_ptr,
                           SequenceControlSet *sequence_control_set_ptr) {
    ...
    if (sequence_control_set_ptr->static_config.rate_control_mode == 1) { // VBR
        context_ptr->virtual_buffer_size = (((uint64_t)sequence_control_set_ptr->static_config.target_bit_rate * 3)
                                            << RC_PRECISION) /
            (context_ptr->frame_rate);
        ...
        context_ptr->virtual_buffer_size =
            ((uint64_t)sequence_control_set_ptr->static_config.target_bit_rate); // vbv_buf_size);
        context_ptr->rate_average_periodin_frames = (uint64_t)sequence_control_set_ptr->static_config.intra_period + 1;
        context_ptr->virtual_buffer_level_initial_value = context_ptr->virtual_buffer_size >> 1;
        context_ptr->virtual_buffer_level               = context_ptr->virtual_buffer_size >> 1;
        context_ptr->previous_virtual_buffer_level      = context_ptr->virtual_buffer_size >> 1;
    ...
    for (uint32_t base_qp = 0; base_qp < MAX_REF_QP_NUM; base_qp++) {
        if (base_qp < 64) {
            context_ptr->qp_scaling_map_I_SLICE[base_qp] = eb_vp9_qp_scaling_calc(
                sequence_control_set_ptr, I_SLICE, 0, base_qp);
        } else {
        ...
        for (uint32_t temporal_layer_index = 0; temporal_layer_index < 4; temporal_layer_index++) {
            if (base_qp < 64) {
                context_ptr->qp_scaling_map[temporal_layer_index][base_qp] = eb_vp9_qp_scaling_calc(
                    sequence_control_set_ptr, 0, temporal_layer_index, base_qp);
            } else {
                context_ptr->qp_scaling_map[temporal_layer_index][base_qp] = (uint32_t)CLIP3(
                    0, 63, (int)base_qp - (63 - (int)context_ptr->qp_scaling_map[temporal_layer_index][63]));
            }
        }
    }
}

static uint64_t predictBits(SequenceControlSet *sequence_control_set_ptr, EncodeContext *encode_context_ptr,
                            HlRateControlHistogramEntry *hlRateControl_histogram_ptr_temp, uint32_t qp) {
    ...
    if (hlRateControl_histogram_ptr_temp->is_coded) {
        ...
        EbBitNumber       *sad_bits_array_ptr =
            rate_control_tables_ptr->sad_bits_array[hlRateControl_histogram_ptr_temp->temporal_layer_index];
        ...
        if (hlRateControl_histogram_ptr_temp->slice_type == EB_I_PICTURE) {
            ...
            int32_t  accum = 0;
            for (i = 0; i < NUMBER_OF_INTRA_SAD_INTERVALS; ++i) {
                accum += (uint32_t)(hlRateControl_histogram_ptr_temp->ois_distortion_histogram[i] *
                                    intra_sad_bits_array_ptr[i]);
            }
            ...
            uint32_t accum_intra = 0;
            for (i = 0; i < NUMBER_OF_SAD_INTERVALS; ++i) {
                accum += (uint32_t)(hlRateControl_histogram_ptr_temp->me_distortion_histogram[i] *
                                    sad_bits_array_ptr[i]);
                accum_intra += (uint32_t)(hlRateControl_histogram_ptr_temp->ois_distortion_histogram[i] *
                                          intra_sad_bits_array_ptr[i]);
            }
            if (accum > accum_intra * 3)
                pred_bits_ref_qp = accum_intra;
...
static uint8_t Vbv_Buf_Calc(PictureControlSet *picture_control_set_ptr, SequenceControlSet *sequence_control_set_ptr,
    ...
    * is in a reasonable state by the end of the lookahead. */
    ...
    int32_t current_ind    = (queue_entry_index_temp > HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH - 1)
           ? queue_entry_index_temp - HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH
           : queue_entry_index_temp;
    ...
    for (int32_t iterations = 0; iterations < 1000 && loop_terminate != 3; iterations++) {
        ...
        double cur_bits                  = (double)predictBits(
            sequence_control_set_ptr, encode_context_ptr, hlRateControl_histogram_ptr_temp, q);
        ...
        while (buffer_fill_cur >= 0) {
            queue_entry_index_temp2 = (queue_entry_index_temp > HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH - 1)
                ? queue_entry_index_temp - HIGH_LEVEL_RATE_CONTROL_HISTOGRAM_QUEUE_MAX_DEPTH
                : queue_entry_index_temp;
            hlRateControl_histogram_ptr_temp =
                (encode_context_ptr->hl_rate_control_historgram_queue[queue_entry_index_temp2]);
        ...
        target_fill = MIN(encode_context_ptr->buffer_fill + total_duration * encode_context_ptr->vbv_max_rate * 0.5,
                          encode_context_ptr->vbv_buf_size * (1 - 0.5));
        if (buffer_fill_cur < target_fill) {
            q++;
            q = CLIP3(sequence_control_set_ptr->static_config.min_qp_allowed,
                      sequence_control_set_ptr->static_config.max_qp_allowed,
                      q);
            loop_terminate |= 1;
            continue;
        ...
        target_fill = CLIP3(encode_context_ptr->vbv_buf_size * (1 - 0.05),
                            encode_context_ptr->vbv_buf_size,
                            encode_context_ptr->buffer_fill - total_duration * encode_context_ptr->vbv_max_rate * 0.5);
...
void *eb_vp9_rate_control_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->rate_control_input_tasks_fifo_ptr, &rate_control_tasks_wrapper_ptr);
        ...
        case RC_PICTURE_MANAGER_RESULT:
            ...
            picture_control_set_ptr = (PictureControlSet *)
                                          rate_control_tasks_ptr->picture_control_set_wrapper_ptr->object_ptr;
            ...
            if (sequence_control_set_ptr->static_config.rate_control_mode) {
                if (picture_control_set_ptr->picture_number == 0) {
                    ...
                    eb_vp9_init_rc(context_ptr, picture_control_set_ptr, sequence_control_set_ptr);
                    encode_context_ptr->buffer_fill  = (uint64_t)(sequence_control_set_ptr->static_config.vbv_buf_size *
                                                                 0.9);
                ...
                if (sequence_control_set_ptr->static_config.rate_control_mode == 1)
                    high_level_rc_input_picture_vbr(picture_control_set_ptr->parent_pcs_ptr,
                                                    sequence_control_set_ptr,
                                                    sequence_control_set_ptr->encode_context_ptr,
                                                    context_ptr,
                                                    context_ptr->high_level_rate_control_ptr);
                else
                    high_level_rc_input_picture_cbr(picture_control_set_ptr->parent_pcs_ptr,
                                                    sequence_control_set_ptr,
                                                    sequence_control_set_ptr->encode_context_ptr,
                                                    context_ptr,
                                                    context_ptr->high_level_rate_control_ptr);
            ...
            rate_control_layer_ptr =
                rate_control_param_ptr->rate_control_layer_array[picture_control_set_ptr->temporal_layer_index];
            ...
            if (sequence_control_set_ptr->static_config.rate_control_mode == 0) {
                ...
                if (sequence_control_set_ptr->enable_qp_scaling_flag &&
                    ...
                    if (picture_control_set_ptr->parent_pcs_ptr->qp_scaling_mode == QP_SCALING_MODE_1) {
                        ...
                        if (picture_control_set_ptr->slice_type == I_SLICE) {
                            ...
                            cpi->rc.kf_boost = (((NON_MOVING_SCORE_3 -
                                                  picture_control_set_ptr->parent_pcs_ptr->non_moving_average_score) *
                                                 (kf_high - kf_low)) /
                                                NON_MOVING_SCORE_3) +
                                kf_low;
                            ...
                            active_best_quality = VPXMIN(active_worst_quality, VPXMAX(1, active_best_quality));
                            ...
                            q_adj_factor += 0.05 -
                                (0.001 * (double)picture_control_set_ptr->parent_pcs_ptr->kf_zeromotion_pct);

                            // Convert the adjustment factor to a qindex delta
                            // on active_best_quality.
                            q_val = eb_vp9_convert_qindex_to_q(active_best_quality, cm->bit_depth);
                            active_best_quality += eb_vp9_compute_qdelta(
                                &rc, q_val, q_val * q_adj_factor, cm->bit_depth);
                            ...
                            cpi->rc.gfu_boost = (((NON_MOVING_SCORE_3 -
                                                   picture_control_set_ptr->parent_pcs_ptr->non_moving_average_score) *
                                                  (gf_high - gf_low)) /
                                                 NON_MOVING_SCORE_3) +
                                gf_low;
                        ...
                        picture_control_set_ptr->parent_pcs_ptr->cpi->common.base_qindex =
                            picture_control_set_ptr->base_qindex = q;
                    ...
                    picture_control_set_ptr->parent_pcs_ptr->cpi->common.base_qindex =
                        picture_control_set_ptr->base_qindex = eb_vp9_quantizer_to_qindex(
                            picture_control_set_ptr->picture_qp);
                }
                ...
                // ***Rate Control***
                if (sequence_control_set_ptr->static_config.rate_control_mode == 1) {
                    frame_level_rc_input_picture_vbr(picture_control_set_ptr,
                                                     sequence_control_set_ptr,
                                                     context_ptr,
                                                     rate_control_layer_ptr,
                    ...
                    // rate control QP refinement
                    rate_control_refinement(picture_control_set_ptr,
                                            sequence_control_set_ptr,
                                            rate_control_param_ptr,
                                            prev_gop_rate_control_param_ptr,
                                            next_gop_rate_control_param_ptr);
                } else {
                    frame_level_rc_input_picture_cbr(picture_control_set_ptr,
                                                     sequence_control_set_ptr,
                                                     context_ptr,
                                                     rate_control_layer_ptr,
                                                     rate_control_param_ptr);
            ...
            if (encode_context_ptr->vbv_buf_size && encode_context_ptr->vbv_max_rate) {
                ...
                picture_control_set_ptr->picture_qp = (uint8_t)Vbv_Buf_Calc(
                    picture_control_set_ptr, sequence_control_set_ptr, encode_context_ptr);
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->temporal_layer_index == 0 &&
                sequence_control_set_ptr->look_ahead_distance != 0) {
                context_ptr->base_layer_frames_avg_qp = (3 * context_ptr->base_layer_frames_avg_qp +
                                                         picture_control_set_ptr->picture_qp + 2) >>
                    2;
            }
            if (picture_control_set_ptr->slice_type == I_SLICE) {
                if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc) {
                    rate_control_param_ptr->first_pic_pred_qp =
                        (uint16_t)picture_control_set_ptr->parent_pcs_ptr->best_pred_qp;
                ...
                }
                {
                    if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc) {
                        if (sequence_control_set_ptr->look_ahead_distance != 0) {
                            context_ptr->base_layer_intra_frames_avg_qp =
                                (3 * context_ptr->base_layer_intra_frames_avg_qp + picture_control_set_ptr->picture_qp +
                                 2) >>
                                2;
                        }
                    }
                    ...
                    if (picture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc) {
                        ...
                        rate_control_param_ptr->intra_frames_qp_bef_scal =
                            (uint8_t)sequence_control_set_ptr->static_config.max_qp_allowed;
                        for (uint32_t qindex = sequence_control_set_ptr->static_config.min_qp_allowed;
                             qindex <= sequence_control_set_ptr->static_config.max_qp_allowed;
                             qindex++) {
            ...
            eb_vp9_get_empty_object(context_ptr->rate_control_output_results_fifo_ptr,
                                    &rate_control_results_wrapper_ptr);
            rate_control_results_ptr = (RateControlResults *)rate_control_results_wrapper_ptr->object_ptr;
            rate_control_results_ptr->picture_control_set_wrapper_ptr =
                rate_control_tasks_ptr->picture_control_set_wrapper_ptr;
        ...
        case RC_PACKETIZATION_FEEDBACK_RESULT:
            ...
            parentpicture_control_set_ptr = (PictureParentControlSet *)
                                                rate_control_tasks_ptr->picture_control_set_wrapper_ptr->object_ptr;
            sequence_control_set_ptr = (SequenceControlSet *)
                                           parentpicture_control_set_ptr->sequence_control_set_wrapper_ptr->object_ptr;
                ...
                do {
                    ...
                    reference_queue_index = (reference_queue_index == REFERENCE_QUEUE_MAX_DEPTH - 1)
                        ...
                        : reference_queue_index + 1;
                         ...
                         (reference_entry_ptr->picture_number != parentpicture_control_set_ptr->picture_number));
            }
            // Frame level RC
            if (sequence_control_set_ptr->intra_period == -1 ||
                sequence_control_set_ptr->static_config.rate_control_mode == 0) {
                ...
                if (parentpicture_control_set_ptr->slice_type == I_SLICE) {
                    if (parentpicture_control_set_ptr->total_num_bits > MAX_BITS_PER_FRAME) {
                        context_ptr->max_rate_adjust_delta_qp++;
                               ...
                               parentpicture_control_set_ptr->total_num_bits < MAX_BITS_PER_FRAME * 85 / 100) {
                        context_ptr->max_rate_adjust_delta_qp--;
                    }
            ...
            if (sequence_control_set_ptr->static_config.rate_control_mode != 0) {
                ...
                context_ptr->virtual_buffer_level = (int64_t)context_ptr->previous_virtual_buffer_level +
                    (int64_t)parentpicture_control_set_ptr->total_num_bits -
                    (int64_t)context_ptr->high_level_rate_control_ptr->channel_bit_rate_per_frame;

                high_level_rc_feed_back_picture(parentpicture_control_set_ptr, sequence_control_set_ptr);
                if (sequence_control_set_ptr->static_config.rate_control_mode == 1)
                    frame_level_rc_feedback_picture_vbr(
                        parentpicture_control_set_ptr, sequence_control_set_ptr, context_ptr);
                else
                ...
                if (parentpicture_control_set_ptr->picture_number == rate_control_param_ptr->first_poc) {
                    ...
                    {
                        int16_t deltaApQp = (int16_t)rate_control_param_ptr->first_pic_actual_qp -
                            (int16_t)rate_control_param_ptr->first_pic_pred_qp;
                        rate_control_param_ptr->extra_ap_bit_ratio_i = (rate_control_param_ptr->first_pic_pred_bits !=
                                                                        0)
            ...
            if (sequence_control_set_ptr->intra_period != -1) {
                ...
                queue_entry_index =
                    (int32_t)(parentpicture_control_set_ptr->picture_number -
                ...
                context_ptr->rate_average_periodin_frames =
                    (uint64_t)sequence_control_set_ptr->static_config.intra_period + 1;
                ...
                while (move_slide_wondow_flag) {
                    ...
                    while (move_slide_wondow_flag && !end_of_sequence_flag &&
                        ...
                        queue_entry_index_temp2 = (queue_entry_index_temp > CODED_FRAMES_STAT_QUEUE_MAX_DEPTH - 1)
                    ...
                    if (move_slide_wondow_flag) {
                        ...
                        while (!end_of_sequence_flag &&
                            ...
                            context_ptr->total_bit_actual_per_sw +=
                                context_ptr->coded_frames_stat_queue[queue_entry_index_temp2]->frame_total_bit_actual;
                            end_of_sequence_flag =
                                context_ptr->coded_frames_stat_queue[queue_entry_index_temp2]->end_of_sequence_flag;
                        ...
                        if (frames_in_sw == (uint32_t)sequence_control_set_ptr->intra_period + 1) {
                            context_ptr->max_bit_actual_per_sw = MAX(
                                ...
                                context_ptr->total_bit_actual_per_sw *
                                    (sequence_control_set_ptr->frame_rate >> RC_PRECISION) / frames_in_sw / 1000);
                                ...
                                if (1) {
                                    ...
                                    SVT_LOG(
                                        ...
                                        (double)((int64_t)context_ptr->total_bit_actual_per_sw *
                                                 (sequence_control_set_ptr->frame_rate >> RC_PRECISION) / frames_in_sw /
                                                 1000),
                                        (double)(100 * (double)context_ptr->total_bit_actual_per_sw *
                                                 (sequence_control_set_ptr->frame_rate >> RC_PRECISION) / frames_in_sw /
                                                 (double)sequence_control_set_ptr->static_config.target_bit_rate) -
                                            100);
                                }
                            }
                        }
                        ...
                        context_ptr->coded_frames_stat_queue_head_index =
                            (context_ptr->coded_frames_stat_queue_head_index == CODED_FRAMES_STAT_QUEUE_MAX_DEPTH - 1)
                            ? 0
