...
static const uint32_t partition_height[MAX_ME_PU_COUNT] = {
    64, 32, 32, 32, 32, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 8, 8, 8, 8, 8, 8, 8, 8,
...
static const uint32_t pu_search_index_map[MAX_ME_PU_COUNT][2] = {
    ...
    {0, 0},   {16, 0},  {32, 0},  {48, 0},  {0, 16},  {16, 16}, {32, 16}, {48, 16}, {0, 32},  {16, 32}, {32, 32},
    {48, 32}, {0, 48},  {16, 48}, {32, 48}, {48, 48},

    {0, 0},   {8, 0},   {16, 0},  {24, 0},  {32, 0},  {40, 0},  {48, 0},  {56, 0},  {0, 8},   {8, 8},   {16, 8},
    {24, 8},  {32, 8},  {40, 8},  {48, 8},  {56, 8},  {0, 16},  {8, 16},  {16, 16}, {24, 16}, {32, 16}, {40, 16},
    {48, 16}, {56, 16}, {0, 24},  {8, 24},  {16, 24}, {24, 24}, {32, 24}, {40, 24}, {48, 24}, {56, 24}, {0, 32},
    {8, 32},  {16, 32}, {24, 32}, {32, 32}, {40, 32}, {48, 32}, {56, 32}, {0, 40},  {8, 40},  {16, 40}, {24, 40},
    {32, 40}, {40, 40}, {48, 40}, {56, 40}, {0, 48},  {8, 48},  {16, 48}, {24, 48}, {32, 48}, {40, 48}, {48, 48},
    {56, 48}, {0, 56},  {8, 56},  {16, 56}, {24, 56}, {32, 56}, {40, 56}, {48, 56}, {56, 56}};
...
*******************************************/
static void eb_vp9_get_eight_horizontal_search_point_results_all85_p_us_c(MeContext *context_ptr, uint32_t list_index,
                                                                          uint32_t search_region_index,
                                                                          uint32_t x_search_index,
                                                                          uint32_t y_search_index) {
    uint8_t *src_ptr = context_ptr->sb_src_ptr;

    uint8_t *ref_ptr = context_ptr->integer_buffer_ptr[list_index][0] + (ME_FILTER_TAP >> 1) +
        ((ME_FILTER_TAP >> 1) * context_ptr->interpolated_full_stride[list_index][0]);
    ...
    */
    ...
    eb_vp9_get_eight_horizontal_search_point_results_8x8_16x16_pu(src_ptr + block_index,
    ...
    eb_vp9_get_eight_horizontal_search_point_results_32x32_64x64(
        p_sad16x16, p_best_sad32x32, p_best_sad64x64, p_best_mv32x32, p_best_mv64x64, curr_mv);
}
...
* GetEightHorizontalSearchPointResultsAll85CUs
*******************************************/
static void eb_vp9_get_eight_horizontal_search_point_results_all85_p_us(MeContext *context_ptr, uint32_t list_index,
                                                                        uint32_t search_region_index,
                                                                        uint32_t x_search_index,
                                                                        uint32_t y_search_index) {
    uint8_t *src_ptr = context_ptr->sb_src_ptr;
    ...
    */
    ...
    block_index           = 0;
    search_position_index = search_position_tl_index;
    eb_vp9_get_eight_horizontal_search_point_results_8x8_16x16_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1](
        src_ptr + block_index,
        context_ptr->sb_src_stride,
        ref_ptr + search_position_index,
        refluma_stride,
    ...
    //32x32 and 64x64
    eb_vp9_get_eight_horizontal_search_point_results_32x32_64x64_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1](
        p_sad16x16, p_best_sad32x32, p_best_sad64x64, p_best_mv32x32, p_best_mv64x64, curr_mv);
}
#endif

typedef void (*EbFpSearchFunc)(MeContext *context_ptr, uint32_t list_index, uint32_t search_region_index,
                               uint32_t x_search_index, uint32_t y_search_index);

/*****************************
* Function Tables
*****************************/
static EbFpSearchFunc FUNC_TABLE
    eb_vp9_get_eight_horizontal_search_point_results_all85_p_us_func_ptr_array[ASM_TYPE_TOTAL] = {
    ...
    uint32_t y_search_index) // input parameter, search region position in the vertical direction, used to derive y_mv
{
    ...
    block_index           = 0;
    search_position_index = search_position_tl_index;
    eb_vp9_sad_calculation_8x8_16x16_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1](
        src_ptr + block_index,
        src_stride,
        ref_ptr + search_position_index,
        refluma_stride,
    ...
    eb_vp9_sad_calculation_32x32_64x64_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1](
        p_sad16x16, p_best_sad32x32, p_best_sad64x64, p_best_mv32x32, p_best_mv64x64, curr_mv);
}
 ...
 * full_pel_search_sb
 *******************************************/
static void full_pel_search_sb(MeContext *context_ptr, uint32_t list_index, int16_t x_search_area_origin,
                               int16_t y_search_area_origin, uint32_t search_area_width, uint32_t search_area_height) {
    ...
    for (y_search_index = 0; y_search_index < search_area_height; y_search_index++) {
        for (x_search_index = 0; x_search_index < search_area_width_mult8; x_search_index += 8) {
            ...
            eb_vp9_get_eight_horizontal_search_point_results_all85_p_us_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) &&
                                                                                       1](
                context_ptr,
                list_index,
...
static void pu_half_pel_refinement(
    ...
    uint32_t *p_best_sad, uint32_t *p_best_mv, uint8_t *psub_pel_direction) {
    ...
    if (context_ptr->fractional_search_method == SSD_SEARCH) {
        *p_best_ssd = (uint32_t)
            spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][Log2f(pu_width) - 2](
...

    {
        ...
        distortion_left_position = (context_ptr->fractional_search_method == SSD_SEARCH)
            ? spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][Log2f(pu_width) - 2](
                  &(context_ptr->sb_src_ptr[pu_sb_buffer_index]),
                  context_ptr->sb_src_stride,
                  &(posb_buffer[search_region_index]),
                  context_ptr->interpolated_stride,
                  pu_height,
                  pu_width)
            : (context_ptr->fractional_search_method == SUB_SAD_SEARCH)
            ? (n_x_m_sad_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1][pu_width >> 3](
                  &(context_ptr->sb_src_ptr[pu_sb_buffer_index]),
        ...
        distortion_top_position = (context_ptr->fractional_search_method == SSD_SEARCH)
            ? spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][Log2f(pu_width) - 2](
                  &(context_ptr->sb_src_ptr[pu_sb_buffer_index]),
                  context_ptr->sb_src_stride,
                  &(posh_buffer[search_region_index]),
                  context_ptr->interpolated_stride,
                  pu_height,
                  pu_width)
            : (context_ptr->fractional_search_method == SUB_SAD_SEARCH)
        ...
        distortion_top_left_position = (context_ptr->fractional_search_method == SSD_SEARCH)
            ? spatialfull_distortion_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][Log2f(pu_width) - 2](
                  &(context_ptr->sb_src_ptr[pu_sb_buffer_index]),
                  context_ptr->sb_src_stride,
                  &(posj_buffer[search_region_index]),
                  context_ptr->interpolated_stride,
                  pu_height,
                  pu_width)
            : (context_ptr->fractional_search_method == SUB_SAD_SEARCH)
        ...
        if (context_ptr->fractional_search_method == SSD_SEARCH) {
            if (distortion_bottom_left_position < *p_best_ssd) {
                *p_best_sad =
                    (uint32_t)(n_x_m_sad_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & AVX2_MASK) && 1][pu_width >> 3](
                        &(context_ptr->sb_src_ptr[pu_sb_buffer_index]),
                        context_ptr->sb_src_stride,
                        &(posj_buffer[search_region_index]),
                        context_ptr->interpolated_stride,
    ...
    best_half_sad = MIN(
        ...
        MIN(distortion_right_position,
            MIN(distortion_top_position,
                MIN(distortion_bottom_position,
                    MIN(distortion_top_left_position,
                        MIN(distortion_top_right_position,
                            MIN(distortion_bottom_left_position, distortion_bottom_right_position)))))));
...
void half_pel_search_sb(
    ...
    if (context_ptr->fractional_search64x64) {
        pu_half_pel_refinement(sequence_control_set_ptr,
                               context_ptr,
                               &(ref_buffer[0]),
                               ref_stride,
...
static void set_quarter_pel_refinement_inputs_on_the_fly(uint8_t  *pos_full, //[IN] points to A
    ...
    case EB_QUARTER_IN_FULL:
        ...
        /*c=b+A*/ buf1[0] = pos_b;
        buf1_stride[0]    = stride;
        buf2[0]           = pos_full;
        buf2_stride[0]    = full_stride;
        /*a=A+b*/ buf1[1] = pos_full;
        buf1_stride[1]    = full_stride;
        buf2[1]           = pos_b + 1;
        buf2_stride[1]    = stride;
        /*n=h+A*/ buf1[2] = pos_h;
        buf1_stride[2]    = stride;
        buf2[2]           = pos_full;
        buf2_stride[2]    = full_stride;
        /*d=A+h*/ buf1[3] = pos_full;
        buf1_stride[3]    = full_stride;
        buf2[3]           = pos_h + stride;
        buf2_stride[3]    = stride;
        /*r=b+h*/ buf1[4] = pos_b;
        buf1_stride[4]    = stride;
        buf2[4]           = pos_h;
        buf2_stride[4]    = stride;
        /*p=h+b*/ buf1[5] = pos_h;
        buf1_stride[5]    = stride;
        buf2[5]           = pos_b + 1;
        buf2_stride[5]    = stride;
        /*e=h+b*/ buf1[6] = pos_h + stride;
        buf1_stride[6]    = stride;
        buf2[6]           = pos_b + 1;
        buf2_stride[6]    = stride;
        /*g=b+h*/ buf1[7] = pos_b;
        buf1_stride[7]    = stride;
        buf2[7]           = pos_h + stride;
        buf2_stride[7]    = stride;
    ...
    case EB_QUARTER_IN_HALF_HORIZONTAL:
        ...
        buf1_stride[0]    = full_stride;
        buf2[0]           = pos_b;
        buf2_stride[0]    = stride;
        /*c=b+A*/ buf1[1] = pos_b;
        buf1_stride[1]    = stride;
        buf2[1]           = pos_full;
        buf2_stride[1]    = full_stride;
        /*q=j+b*/ buf1[2] = pos_j;
        buf1_stride[2]    = stride;
        buf2[2]           = pos_b;
        buf2_stride[2]    = stride;
        /*f=b+j*/ buf1[3] = pos_b;
        buf1_stride[3]    = stride;
        buf2[3]           = pos_j + stride;
        buf2_stride[3]    = stride;
    ...
    case EB_QUARTER_IN_HALF_VERTICAL:
        ...
        buf2[1]           = pos_j + 1;
        buf2_stride[1]    = stride;
        /*d=A+h*/ buf1[2] = pos_full - full_stride;
        buf1_stride[2]    = full_stride;
        buf2[2]           = pos_h;
        buf2_stride[2]    = stride;
        /*n=h+A*/ buf1[3] = pos_h;
        buf1_stride[3]    = stride;
        buf2[3]           = pos_full;
        buf2_stride[3]    = full_stride;
...
static void quarter_pel_search_sb(
    ...
    if (enable_quarter_pel && enable_half_pel32x32) {
        ...
        for (pu_index = 0; pu_index < 4; ++pu_index) {
            ...
            buf1[0] = buf1[0] + pu_shift_x_index + pu_shift_y_index * buf1_stride[0];
            buf2[0] = buf2[0] + pu_shift_x_index + pu_shift_y_index * buf2_stride[0];
            buf1[1] = buf1[1] + pu_shift_x_index + pu_shift_y_index * buf1_stride[1];
            buf2[1] = buf2[1] + pu_shift_x_index + pu_shift_y_index * buf2_stride[1];
            buf1[2] = buf1[2] + pu_shift_x_index + pu_shift_y_index * buf1_stride[2];
            buf2[2] = buf2[2] + pu_shift_x_index + pu_shift_y_index * buf2_stride[2];
            buf1[3] = buf1[3] + pu_shift_x_index + pu_shift_y_index * buf1_stride[3];
            buf2[3] = buf2[3] + pu_shift_x_index + pu_shift_y_index * buf2_stride[3];
            buf1[4] = buf1[4] + pu_shift_x_index + pu_shift_y_index * buf1_stride[4];
            buf2[4] = buf2[4] + pu_shift_x_index + pu_shift_y_index * buf2_stride[4];
            buf1[5] = buf1[5] + pu_shift_x_index + pu_shift_y_index * buf1_stride[5];
            buf2[5] = buf2[5] + pu_shift_x_index + pu_shift_y_index * buf2_stride[5];
            buf1[6] = buf1[6] + pu_shift_x_index + pu_shift_y_index * buf1_stride[6];
            buf2[6] = buf2[6] + pu_shift_x_index + pu_shift_y_index * buf2_stride[6];
            buf1[7] = buf1[7] + pu_shift_x_index + pu_shift_y_index * buf1_stride[7];
            buf2[7] = buf2[7] + pu_shift_x_index + pu_shift_y_index * buf2_stride[7];
...
void single_hme_quadrant_level0(
    ...
    uint64_t *
        level0_best_sad, // output parameter, Level0 SAD at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        x_level0_search_center, // output parameter, Level0 x_mv at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        y_level0_search_center, // output parameter, Level0 y_mv at (search_region_number_in_width, search_region_number_in_height)
    uint32_t search_area_multiplier_x, uint32_t search_area_multiplier_y) {
    ...
    int16_t search_area_width = (int16_t)((context_ptr->hme_level0_total_search_area_width * search_area_multiplier_x) /
                                          100);
    int16_t search_area_height =
        (int16_t)((context_ptr->hme_level0_total_search_area_height * search_area_multiplier_y) / 100);
    ...
    search_area_width = ((origin_x + x_search_area_origin + search_area_width) > (int16_t)sixteenth_ref_pic_ptr->width)
        ? MAX(1,
              search_area_width -
                  ((origin_x + x_search_area_origin + search_area_width) - (int16_t)sixteenth_ref_pic_ptr->width))
        : search_area_width;

    // Correct the top edge of the Search Area if it is not on the reference Picture
    y_search_area_origin = ((origin_y + y_search_area_origin) < -pad_height) ? -pad_height - origin_y
                                                                             : y_search_area_origin;
    ...
    search_area_height = (origin_y + y_search_area_origin + search_area_height > (int16_t)sixteenth_ref_pic_ptr->height)
        ? MAX(1,
              search_area_height -
                  ((origin_y + y_search_area_origin + search_area_height) - (int16_t)sixteenth_ref_pic_ptr->height))
        : search_area_height;
...

    {
        ...
        if (((search_area_width & 15) == 0) && ((eb_vp9_ASM_TYPES & AVX2_MASK) && 1)) {
#ifndef DISABLE_AVX512
            eb_vp9_sad_loop_kernel_avx512_hme_l0_intrin(&context_ptr->sixteenth_sb_buffer[0],
                                                        ...
                                                        search_area_height);
#else
            eb_vp9_sad_loop_kernel_avx2_hme_l0_intrin(&context_ptr->sixteenth_sb_buffer[0],
                                                      context_ptr->sixteenth_sb_buffer_stride,
                                                      &sixteenth_ref_pic_ptr->buffer_y[search_region_index],
                                                      sixteenth_ref_pic_ptr->stride_y * 2,
                                                      sb_height >> 1,
        ...
        } else {
            {
                ...
                eb_vp9_sad_loop_kernel(&context_ptr->sixteenth_sb_buffer[0],
                                       context_ptr->sixteenth_sb_buffer_stride,
                                       &sixteenth_ref_pic_ptr->buffer_y[search_region_index],
                                       sixteenth_ref_pic_ptr->stride_y * 2,
                                       sb_height >> 1,
...
void hme_level0(
    ...
    int16_t search_area_width =
        (int16_t)((context_ptr->hme_level0_search_area_in_width_array[search_region_number_in_width] *
                   search_area_multiplier_x) /
                  100);
    int16_t search_area_height =
        (int16_t)((context_ptr->hme_level0_search_area_in_height_array[search_region_number_in_height] *
                   search_area_multiplier_y) /
                  100);
    ...
    x_search_area_origin =
        -(int16_t)(((context_ptr->hme_level0_total_search_area_width * search_area_multiplier_x) / 100) >> 1) +
        x_search_region_distance;
    y_search_area_origin =
        -(int16_t)(((context_ptr->hme_level0_total_search_area_height * search_area_multiplier_y) / 100) >> 1) +
        y_search_region_distance;
...
void hme_level1(
    ...
    int16_t
        x_level0_search_center, // input parameter, best Level0 x_mv at (search_region_number_in_width, search_region_number_in_height)
    int16_t
        y_level0_search_center, // input parameter, best Level0 y_mv at (search_region_number_in_width, search_region_number_in_height)
    uint64_t *
        level1_best_sad, // output parameter, Level1 SAD at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        x_level1_search_center, // output parameter, Level1 x_mv at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        y_level1_search_center) // output parameter, Level1 y_mv at (search_region_number_in_width, search_region_number_in_height)
{
    int16_t  x_top_left_search_region;
    int16_t  y_top_left_search_region;
    uint32_t search_region_index;
    ...
    int16_t search_area_width  = (hme_level1_search_area_in_width < 8) ? 8
         : (hme_level1_search_area_in_width & 7)                       ? hme_level1_search_area_in_width +
            (hme_level1_search_area_in_width - ((hme_level1_search_area_in_width >> 3) << 3))
    ...
    search_area_width = ((origin_x + x_search_area_origin + search_area_width) > (int16_t)quarter_ref_pic_ptr->width)
        ? MAX(1,
              search_area_width -
                  ((origin_x + x_search_area_origin + search_area_width) - (int16_t)quarter_ref_pic_ptr->width))
        : search_area_width;
    ...
    search_area_height = (origin_y + y_search_area_origin + search_area_height > (int16_t)quarter_ref_pic_ptr->height)
        ? MAX(1,
              search_area_height -
                  ((origin_y + y_search_area_origin + search_area_height) - (int16_t)quarter_ref_pic_ptr->height))
        : search_area_height;
    ...
    if (((sb_width & 7) == 0) || (sb_width == 4)) {
        ...
        eb_vp9_sad_loop_kernel(&context_ptr->quarter_sb_buffer[0],
                               context_ptr->quarter_sb_buffer_stride * 2,
                               &quarter_ref_pic_ptr->buffer_y[search_region_index],
                               quarter_ref_pic_ptr->stride_y * 2,
                               sb_height >> 1,
...
void hme_level2(
    ...
    uint32_t search_region_number_in_height, // input parameter, search region number in the vertical direction
    int16_t
        x_level1_search_center, // input parameter, best Level1 x_mv at(search_region_number_in_width, search_region_number_in_height)
    int16_t
        y_level1_search_center, // input parameter, best Level1 y_mv at(search_region_number_in_width, search_region_number_in_height)
    uint64_t *
        level2_best_sad, // output parameter, Level2 SAD at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        x_level2_search_center, // output parameter, Level2 x_mv at (search_region_number_in_width, search_region_number_in_height)
    int16_t *
        y_level2_search_center) // output parameter, Level2 y_mv at (search_region_number_in_width, search_region_number_in_height)
{
    int16_t  x_top_left_search_region;
    int16_t  y_top_left_search_region;
    uint32_t search_region_index;
    ...
    int16_t search_area_width = (hme_level2_search_area_in_width < 8) ? 8
        : (hme_level2_search_area_in_width & 7)                       ? hme_level2_search_area_in_width +
            (hme_level2_search_area_in_width - ((hme_level2_search_area_in_width >> 3) << 3))
                                                : hme_level2_search_area_in_width;
    int16_t search_area_height =
        (int16_t)context_ptr->hme_level2_search_area_in_height_array[search_region_number_in_height];
    ...
    if (((sb_width & 7) == 0) && (sb_width != 40) && (sb_width != 56)) {
        ...
        // Put the first search location into level0 results
        eb_vp9_sad_loop_kernel(context_ptr->sb_src_ptr,
                               context_ptr->sb_src_stride * 2,
                               &ref_pic_ptr->buffer_y[search_region_index],
                               ref_pic_ptr->stride_y * 2,
                               sb_height >> 1,
...
static void select_buffer(uint32_t  pu_index, //[IN]
                                     ...
                                     uint32_t dst_stride) //[IN]
{
    ...
    picture_average_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1](
        buf1, ref_stride1, buf2, ref_stride2, dst, dst_stride, pu_width, pu_height);
...
uint32_t bi_pred_averging(MeContext *context_ptr, MePredUnit *me_candidate, uint32_t pu_index, uint8_t *source_pic,
    ...
    me_candidate->distortion = (context_ptr->fractional_search_method == SUB_SAD_SEARCH)
        ? nx_m_sad_averaging_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][pu_width >> 3](
            ...
            << 1
        : nx_m_sad_averaging_kernel_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1][pu_width >> 3](
              source_pic, luma_stride, ptr_list0, ptr_list0_stride, ptr_list1, ptr_list1_stride, pu_height, pu_width);
...
EbErrorType bi_prediction_compensation(MeContext *context_ptr, uint32_t pu_index, MePredUnit *me_candidate,
    ...
    first_ref_pos_x = _MVXT(first_ref_mv), first_ref_pos_y = _MVYT(first_ref_mv), me_candidate->Mv[0] = first_ref_mv;
    ...
    first_search_region_index_pos_integ = (int32_t)(xfirst_search_index + (ME_FILTER_TAP >> 1)) +
        (int32_t)context_ptr->interpolated_full_stride[first_list][0] *
            (int32_t)(yfirst_search_index + (ME_FILTER_TAP >> 1));
    first_search_region_index_posb = (int32_t)(xfirst_search_index + (ME_FILTER_TAP >> 1) - 1) +
        (int32_t)context_ptr->interpolated_stride * (int32_t)(yfirst_search_index + (ME_FILTER_TAP >> 1));
    first_search_region_index_posh = (int32_t)(xfirst_search_index + (ME_FILTER_TAP >> 1) - 1) +
        (int32_t)context_ptr->interpolated_stride * (int32_t)(yfirst_search_index + (ME_FILTER_TAP >> 1) - 1);
    first_search_region_index_posj = (int32_t)(xfirst_search_index + (ME_FILTER_TAP >> 1) - 1) +
        (int32_t)context_ptr->interpolated_stride * (int32_t)(yfirst_search_index + (ME_FILTER_TAP >> 1) - 1);

    // Second reference

    // Set Candidate information
    second_ref_pos_x = _MVXT(second_ref_mv), second_ref_pos_y = _MVYT(second_ref_mv),
    ...
    second_search_region_index_pos_integ = (int32_t)(xsecond_search_index + (ME_FILTER_TAP >> 1)) +
        (int32_t)context_ptr->interpolated_full_stride[second_list][0] *
            (int32_t)(ysecond_search_index + (ME_FILTER_TAP >> 1));
    second_search_region_index_posb = (int32_t)(xsecond_search_index + (ME_FILTER_TAP >> 1) - 1) +
        (int32_t)context_ptr->interpolated_stride * (int32_t)(ysecond_search_index + (ME_FILTER_TAP >> 1));
    second_search_region_index_posh = (int32_t)(xsecond_search_index + (ME_FILTER_TAP >> 1) - 1) +
...
EbErrorType su_pel_enable(MeContext *context_ptr, PictureParentControlSet *picture_control_set_ptr, uint32_t list_index,
    ...
    avg_mvx32x32 = (_MVXT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_32x32_0]) +
        ...
        2;
    avg_mvy32x32 = (_MVYT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_32x32_0]) +
    ...
    avg_mvx16x16 = (_MVXT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_16x16_0]) +
        ...
        4;
    avg_mvy16x16 = (_MVYT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_16x16_0]) +
    ...
    avg_mvx8x8 = (_MVXT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_8x8_0]) +
        ...
        6;
    avg_mvy8x8 = (_MVYT(context_ptr->p_sb_best_mv[list_index][ref_pic_index][ME_TIER_ZERO_PU_8x8_0]) +
    ...
    avg_sad32x32 = (context_ptr->p_sb_best_sad[list_index][ref_pic_index][ME_TIER_ZERO_PU_32x32_0] +
        ...
        2;
    avg_sad16x16 = (context_ptr->p_sb_best_sad[list_index][ref_pic_index][ME_TIER_ZERO_PU_16x16_0] +
        ...
        4;
    avg_sad8x8 = (context_ptr->p_sb_best_sad[list_index][ref_pic_index][ME_TIER_ZERO_PU_8x8_0] +
...
static void test_search_area_bounds(EbPictureBufferDesc *ref_pic_ptr, MeContext *context_ptr, int16_t *xsc,
                                    int16_t *ysc, uint32_t list_index, int16_t origin_x, int16_t origin_y,
    ...
    EbPictureBufferDesc *input_ptr) // input parameter, source Picture Ptr
{
    ...
    int16_t  picture_width =
        (int16_t)((SequenceControlSet *)picture_control_set_ptr->sequence_control_set_wrapper_ptr->object_ptr)
            ->luma_width;
    int16_t picture_height =
        (int16_t)((SequenceControlSet *)picture_control_set_ptr->sequence_control_set_wrapper_ptr->object_ptr)
            ->luma_height;
    uint32_t sb_width   = (input_ptr->width - sb_origin_x) < MAX_SB_SIZE ? input_ptr->width - sb_origin_x : MAX_SB_SIZE;
    uint32_t sb_height  = (input_ptr->height - sb_origin_y) < MAX_SB_SIZE ? input_ptr->height - sb_origin_y
                                                                          : MAX_SB_SIZE;
    int16_t  pad_width  = (int16_t)MAX_SB_SIZE - 1;
    int16_t  pad_height = (int16_t)MAX_SB_SIZE - 1;
    int16_t  search_area_width;
    ...
    for (list_index = REF_LIST_0; list_index <= num_of_list_to_search; ++list_index) {
        // Ref Picture Loop
        {
            ...
            if (picture_control_set_ptr->temporal_layer_index > 0 || list_index == 0) {
                ...
                if (picture_control_set_ptr->enable_hme_flag && /*B*/ sb_height ==
                        MAX_SB_SIZE) { //(searchCenterSad > sequence_control_set_ptr->static_config.skipTier0HmeTh)) {
                    ...
                    if (enable_hme_level_0_flag) {
                        if (context_ptr->single_hme_quadrant && !enable_hme_level_1_flag && !enable_hme_level_2_flag) {
                            ...
                            single_hme_quadrant_level0(
                                ...
                                &(x_hme_level0_search_center[search_region_number_in_width]
                                                            [search_region_number_in_height]),
                                &(y_hme_level0_search_center[search_region_number_in_width]
                                                            [search_region_number_in_height]),
                                hme_level_0_search_area_multiplier_x[picture_control_set_ptr->hierarchical_levels]
                                                                    [picture_control_set_ptr->temporal_layer_index],
                                hme_level_0_search_area_multiplier_y[picture_control_set_ptr->hierarchical_levels]
                                                                    [picture_control_set_ptr->temporal_layer_index]);
                    ...
                    if (enable_hme_level_1_flag) {
                        ...
                        while (search_region_number_in_height < context_ptr->number_hme_search_region_in_height) {
                            while (search_region_number_in_width < context_ptr->number_hme_search_region_in_width) {
                                ...
                                hme_level1_search_area_in_width =
                                    (int16_t)context_ptr
                                        ->hme_level1_search_area_in_width_array[search_region_number_in_width];
                                hme_level1_search_area_in_height =
                                    (int16_t)context_ptr
                                        ->hme_level1_search_area_in_height_array[search_region_number_in_height];
                                hme_level1(
                                    context_ptr,
                                    origin_x >> 1,
                                    origin_y >> 1,
                                    ...
                                    &(x_hme_level1_search_center[search_region_number_in_width]
                                                                [search_region_number_in_height]),
                                    &(y_hme_level1_search_center[search_region_number_in_width]
                                                                [search_region_number_in_height]));
                        ...
                        while (search_region_number_in_height < context_ptr->number_hme_search_region_in_height) {
                            while (search_region_number_in_width < context_ptr->number_hme_search_region_in_width) {
                                hme_level2(
                                    ...
                                    &(x_hme_level2_search_center[search_region_number_in_width]
                                                                [search_region_number_in_height]),
                                    &(y_hme_level2_search_center[search_region_number_in_width]
                                                                [search_region_number_in_height]));
                    ...
                    if (enable_hme_level_2_flag) {
                        ...
                        if ((ref0_poc == ref1_poc) && (list_index == 1) && (total_me_quad > 1)) {
                            for (quad_index = 0; quad_index < total_me_quad - 1; ++quad_index) {
                                for (next_quad_index = quad_index + 1; next_quad_index < total_me_quad;
                                     ++next_quad_index) {
                                    if (hme_level2_sad[quad_index / num_quad_in_width][quad_index % num_quad_in_width] >
                                        hme_level2_sad[next_quad_index / num_quad_in_width]
                                                      [next_quad_index % num_quad_in_width]) {
                                        temp_x_hme_search_center =
                                            x_hme_level2_search_center[quad_index / num_quad_in_width]
                                                                      [quad_index % num_quad_in_width];
                                        temp_y_hme_search_center =
                                            y_hme_level2_search_center[quad_index / num_quad_in_width]
                                                                      [quad_index % num_quad_in_width];
                                        temp_x_hme_distortion = hme_level2_sad[quad_index / num_quad_in_width]
                                                                              [quad_index % num_quad_in_width];

                                        x_hme_level2_search_center[quad_index / num_quad_in_width][quad_index %
                                                                                                   num_quad_in_width] =
                                            x_hme_level2_search_center[next_quad_index / num_quad_in_width]
                                                                      [next_quad_index % num_quad_in_width];
                                        y_hme_level2_search_center[quad_index / num_quad_in_width][quad_index %
                                                                                                   num_quad_in_width] =
                                            y_hme_level2_search_center[next_quad_index / num_quad_in_width]
                                                                      [next_quad_index % num_quad_in_width];
                                        hme_level2_sad[quad_index / num_quad_in_width][quad_index % num_quad_in_width] =
                                            hme_level2_sad[next_quad_index / num_quad_in_width]
                                                          [next_quad_index % num_quad_in_width];
                                        ...
                                        y_hme_level2_search_center[next_quad_index / num_quad_in_width]
                                                                  [next_quad_index % num_quad_in_width] =
                                                                      temp_y_hme_search_center;
                                        hme_level2_sad[next_quad_index / num_quad_in_width]
                                                      [next_quad_index % num_quad_in_width] = temp_x_hme_distortion;
                                    }
                                }
                            }
            ...
            if (x_search_center != 0 || y_search_center != 0) {
                check_zero_zero_center(ref_pic_ptr,
                                       context_ptr,
                                       sb_origin_x,
                                       sb_origin_y,
                                       sb_width,
            ...
            x_top_left_search_region = (int16_t)(ref_pic_ptr->origin_x + sb_origin_x) - (ME_FILTER_TAP >> 1) +
                x_search_area_origin;
            y_top_left_search_region = (int16_t)(ref_pic_ptr->origin_y + sb_origin_y) - (ME_FILTER_TAP >> 1) +
                y_search_area_origin;
            search_region_index = (x_top_left_search_region) + (y_top_left_search_region)*ref_pic_ptr->stride_y;
            ...
            {
                {
                    eb_vp9_initialize_buffer_32bits_func_ptr_array[(eb_vp9_ASM_TYPES & PREAVX2_MASK) && 1](
                        context_ptr->p_sb_best_sad[list_index][0], 21, 1, MAX_SAD_VALUE);
                    context_ptr->p_best_sad64x64 = &(context_ptr->p_sb_best_sad[list_index][0][ME_TIER_ZERO_PU_64x64]);
                    context_ptr->p_best_sad32x32 = &(
                        context_ptr->p_sb_best_sad[list_index][0][ME_TIER_ZERO_PU_32x32_0]);
                    context_ptr->p_best_sad16x16 = &(
                        context_ptr->p_sb_best_sad[list_index][0][ME_TIER_ZERO_PU_16x16_0]);
                    ...
                    context_ptr->p_best_ssd32x32 = &(
                        context_ptr->p_sb_best_ssd[list_index][0][ME_TIER_ZERO_PU_32x32_0]);
                    context_ptr->p_best_ssd16x16 = &(
                        context_ptr->p_sb_best_ssd[list_index][0][ME_TIER_ZERO_PU_16x16_0]);
                ...
                if (enable_half_pel32x32 || enable_half_pel16x16 || enable_half_pel8x8 || enable_quarter_pel) {
                    ...
                    interpolate_search_region_avc(
                        context_ptr,
                        list_index,
                        context_ptr->integer_buffer_ptr[list_index][0] + (ME_FILTER_TAP >> 1) +
                            ((ME_FILTER_TAP >> 1) * context_ptr->interpolated_full_stride[list_index][0]),
                    ...
                    half_pel_search_sb(
                        ...
                        context_ptr->interpolated_full_stride[list_index][0],
                        &(context_ptr
                              ->posb_buffer[list_index][0][(ME_FILTER_TAP >> 1) * context_ptr->interpolated_stride]),
                    ...
                    quarter_pel_search_sb(
                        ...
                        context_ptr->integer_buffer_ptr[list_index][0] + (ME_FILTER_TAP >> 1) +
                            ((ME_FILTER_TAP >> 1) * context_ptr->interpolated_full_stride[list_index][0]),
                        context_ptr->interpolated_full_stride[list_index][0],
                        &(context_ptr->posb_buffer
                              [list_index][0]
                              [(ME_FILTER_TAP >> 1) *
                               context_ptr->interpolated_stride]), //points to b position of the figure above
    ...
    for (pu_index = 0; pu_index < max_number_of_pus_per_sb; ++pu_index) {
        ...
        if (num_of_list_to_search) {
            ...
            if (condition) {
                bi_prediction_search(
                    context_ptr, pu_index, candidate_index, 1, 1, &total_me_candidate_index, picture_control_set_ptr);
            }
        }
        ...
        if (total_me_candidate_index == 3) {
            ...
            default: SVT_LOG("Err in sorting"); break;
            }
    ...
    if (sequence_control_set_ptr->static_config.rate_control_mode) {
        ...
        for (i = 0; i < 16; i++) {
            picture_control_set_ptr->rcme_distortion[lblock_index] +=
                picture_control_set_ptr->me_results[lblock_index][5 + i].distortion_direction[0].distortion;
        }
    }
