...
static EB_BOOL check_mv_for_pan_high_amp(uint32_t hierarchical_levels, uint32_t temporal_layer_index,
...
static EB_BOOL check_mv_for_tilt_high_amp(uint32_t hierarchical_levels, uint32_t temporal_layer_index,
...
static void eb_vp9_DetectGlobalMotion(SequenceControlSet      *sequence_control_set_ptr,
                                      PictureParentControlSet *picture_control_set_ptr) {
    uint32_t sb_index;
    uint32_t picture_width_in_sb = (picture_control_set_ptr->enhanced_picture_ptr->width + MAX_SB_SIZE_MINUS_1) /
        MAX_SB_SIZE;
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
        ...
        if (sb_params->is_complete_sb) {
            ...
            if ((EB_BOOL)(check_mv_for_pan_high_amp(picture_control_set_ptr->hierarchical_levels,
                                                    picture_control_set_ptr->temporal_layer_index,
                                                    &x_current_mv,
                                                    &x_left_mv) ||
                          check_mv_for_pan_high_amp(picture_control_set_ptr->hierarchical_levels,
                                                    picture_control_set_ptr->temporal_layer_index,
                                                    &x_current_mv,
                                                    &x_top_mv) ||
                          check_mv_for_pan_high_amp(picture_control_set_ptr->hierarchical_levels,
            ...
            if ((EB_BOOL)(check_mv_for_tilt_high_amp(picture_control_set_ptr->hierarchical_levels,
                                                     picture_control_set_ptr->temporal_layer_index,
                                                     &y_current_mv,
                                                     &y_left_mv) ||
                          check_mv_for_tilt_high_amp(picture_control_set_ptr->hierarchical_levels,
                                                     picture_control_set_ptr->temporal_layer_index,
                                                     &y_current_mv,
                                                     &y_top_mv) ||
                          check_mv_for_tilt_high_amp(picture_control_set_ptr->hierarchical_levels,
...
************************************************/
EbErrorType eb_vp9_initial_eb_vp9_rate_control_context_ctor(InitialRateControlContext **context_dbl_ptr,
...
************************************************/
static void eb_vp9_ReleasePaReferenceObjects(PictureParentControlSet *picture_control_set_ptr) {
...
static void stationary_edge_count_sb(SequenceControlSet      *sequence_control_set_ptr,
                                     PictureParentControlSet *picture_control_set_ptr,
                                     PictureParentControlSet *temporalPictureControlSetPtr, uint32_t total_sb_count) {
    uint32_t sb_index;
        ...
        if (sb_params->potential_logo_sb && sb_params->is_complete_sb &&
            ...
            if (tempLcuStatPtr->check1_for_logo_stationary_edge_over_time_flag) {
                for (raster_scan_block_index = PA_RASTER_SCAN_CU_INDEX_16x16_0;
                    ...
                    sb_stat_ptr->cu_stat_array[raster_scan_block_index].similar_edge_count +=
                        tempLcuStatPtr->cu_stat_array[raster_scan_block_index].edge_cu;
                }
            }
        }
        ...
        if (sb_params->potential_logo_sb && sb_params->is_complete_sb &&
            ...
            if (tempLcuStatPtr->pm_check1_for_logo_stationary_edge_over_time_flag) {
                for (raster_scan_block_index = PA_RASTER_SCAN_CU_INDEX_16x16_0;
                     raster_scan_block_index <= PA_RASTER_SCAN_CU_INDEX_16x16_15;
                     raster_scan_block_index++) {
                    sb_stat_ptr->cu_stat_array[raster_scan_block_index].pm_similar_edge_count +=
                        tempLcuStatPtr->cu_stat_array[raster_scan_block_index].edge_cu;
                }
            }
        }
...
************************************************/
static void update_global_motion_detection_over_time(EncodeContext           *encode_context_ptr,
...
static void eb_vp9_UpdateBeaInfoOverTime(EncodeContext           *encode_context_ptr,
                                         PictureParentControlSet *picture_control_set_ptr) {
    InitialRateControlReorderEntry *temporary_queue_entry_ptr;
    PictureParentControlSet        *temporary_picture_control_set_ptr;
    ...
    update_non_moving_index_array_frames_to_check = MIN(
        MIN((picture_control_set_ptr->pred_struct_ptr->pred_struct_period << 1) + 1,
            picture_control_set_ptr->frames_in_sw),
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
        ...
        for (frames_to_check_index = 0; frames_to_check_index < update_non_moving_index_array_frames_to_check - 1;
            ...
            if (temporary_picture_control_set_ptr->slice_type == I_SLICE ||
                temporary_picture_control_set_ptr->end_of_sequence_flag) {
                break;
            }
        ...
        picture_control_set_ptr->non_moving_index_array[sb_index] = (uint8_t)(nonMovingIndexOverSlidingWindow /
...
****************************************/
static void init_zz_cost_info(PictureParentControlSet *picture_control_set_ptr) {
    uint16_t sb_index;
...
** LAD Window: min (2xmgpos+1 or sliding window size)
************************************************/
static void update_motion_field_uniformity_over_time(EncodeContext           *encode_context_ptr,
                                                     SequenceControlSet      *sequence_control_set_ptr,
                                                     PictureParentControlSet *picture_control_set_ptr) {
    InitialRateControlReorderEntry *temporary_queue_entry_ptr;
    PictureParentControlSet        *temporary_picture_control_set_ptr;
    ...
    // Determine number of frames to check N
    no_frames_to_check = MIN(MIN((picture_control_set_ptr->pred_struct_ptr->pred_struct_period << 1) + 1,
                                 picture_control_set_ptr->frames_in_sw),
                             sequence_control_set_ptr->look_ahead_distance);
    ...
    for (int32_t frames_to_check_index = 0; frames_to_check_index < no_frames_to_check - 1; frames_to_check_index++) {
        ...
        if ((temporary_picture_control_set_ptr->picture_number & 3) == 0) {
            stationary_edge_count_sb(sequence_control_set_ptr,
...
** LAD Window: min (2xmgpos+1 or sliding window size)
************************************************/
static void update_homogeneity_over_time(EncodeContext           *encode_context_ptr,
                                         PictureParentControlSet *picture_control_set_ptr) {
    InitialRateControlReorderEntry *temporary_queue_entry_ptr;
    PictureParentControlSet        *temporary_picture_control_set_ptr;
    int32_t                         no_frames_to_check;
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
        ...
        input_queue_index = (encode_context_ptr->initial_rate_control_reorder_queue_head_index ==
            ...
            ? 0
            : encode_context_ptr->initial_rate_control_reorder_queue_head_index;
        for (int32_t frames_to_check_index = 0; frames_to_check_index < no_frames_to_check - 1;
             frames_to_check_index++) {
            temporary_queue_entry_ptr = encode_context_ptr->initial_rate_control_reorder_queue[input_queue_index];
            temporary_picture_control_set_ptr =
                ((PictureParentControlSet *)(temporary_queue_entry_ptr->parent_pcs_wrapper_ptr)->object_ptr);
            if (temporary_picture_control_set_ptr->scene_change_flag ||
                temporary_picture_control_set_ptr->end_of_sequence_flag) {
                break;
            }
            ...
            mean_sqr_variance64x64_based += (variance_ptr[ME_TIER_ZERO_PU_64x64]) *
                (variance_ptr[ME_TIER_ZERO_PU_64x64]);
        ...
        // Compute variance
        picture_control_set_ptr->sb_variance_of_variance_over_time[sb_index] = mean_sqr_variance64x64_based -
    ...
    picture_control_set_ptr->pic_homogenous_over_time_sb_percentage =
        (uint8_t)(count_of_homogeneous_over_time * 100 / picture_control_set_ptr->sb_total_count);

    return;
}

static void reset_homogeneity_structures(PictureParentControlSet *picture_control_set_ptr) {
    uint32_t sb_index;
...
static InitialRateControlReorderEntry *determine_picture_offset_in_queue(
    EncodeContext *encode_context_ptr, PictureParentControlSet *picture_control_set_ptr,
    ...
    queue_entry_index =
        (int32_t)(picture_control_set_ptr->picture_number -
                  encode_context_ptr
                      ->initial_rate_control_reorder_queue[encode_context_ptr
...
static void get_histogram_queue_data(SequenceControlSet *sequence_control_set_ptr, EncodeContext *encode_context_ptr,
                                     PictureParentControlSet *picture_control_set_ptr) {
...
static void update_histogram_queue_entry(SequenceControlSet      *sequence_control_set_ptr,
                                         EncodeContext           *encode_context_ptr,
                                         PictureParentControlSet *picture_control_set_ptr, uint32_t frames_in_sw) {
    HlRateControlHistogramEntry *histogram_queue_entry_ptr;
    int32_t                      histogram_queue_entry_index;
...
void *eb_vp9_initial_eb_vp9_rate_control_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->motion_estimation_results_input_fifo_ptr, &input_results_wrapper_ptr);
        ...
        if (SEGMENT_COMPLETION_MASK_TEST(picture_control_set_ptr->me_segments_completion_mask,
            ...
            me_based_global_motion_detection(sequence_control_set_ptr, picture_control_set_ptr);

            // Release Pa Ref pictures when not needed
            eb_vp9_ReleasePaReferenceObjects(picture_control_set_ptr);
            ...
            queue_entry_ptr = determine_picture_offset_in_queue(
                encode_context_ptr, picture_control_set_ptr, input_results_ptr);

            if (sequence_control_set_ptr->static_config.rate_control_mode) {
                if (sequence_control_set_ptr->look_ahead_distance != 0) {
                    // Getting the Histogram Queue Data
                    get_histogram_queue_data(sequence_control_set_ptr, encode_context_ptr, picture_control_set_ptr);
                }
            }
            ...
            while (move_slide_wondow_flag) {
                ...
                if (move_slide_wondow_flag) {
                    ...
                    queue_entry_ptr = encode_context_ptr->initial_rate_control_reorder_queue
                                          [encode_context_ptr->initial_rate_control_reorder_queue_head_index];
                    picture_control_set_ptr =
                    ...
                    if (sequence_control_set_ptr->static_config.rate_control_mode) {
                        ...
                        if (sequence_control_set_ptr->look_ahead_distance != 0) {
                            ...
                            update_histogram_queue_entry(
                                sequence_control_set_ptr, encode_context_ptr, picture_control_set_ptr, frames_in_sw);
                        }
                    }
                    ...
                    if (!picture_control_set_ptr->end_of_sequence_flag &&
                        ...
                        update_global_motion_detection_over_time(
                            encode_context_ptr, sequence_control_set_ptr, picture_control_set_ptr);
                    } else {
                        if (picture_control_set_ptr->slice_type != I_SLICE) {
                            eb_vp9_DetectGlobalMotion(sequence_control_set_ptr, picture_control_set_ptr);
                        }
                    ...
                    if (!picture_control_set_ptr->end_of_sequence_flag &&
                        ...
                        eb_vp9_UpdateBeaInfoOverTime(encode_context_ptr, picture_control_set_ptr);

                    } else {
                        // Reset zzCost information to default When there's no lookahead available
                        init_zz_cost_info(picture_control_set_ptr);
                    }
                    ...
                    if (!picture_control_set_ptr->end_of_sequence_flag &&
                        ...
                        update_homogeneity_over_time(encode_context_ptr, picture_control_set_ptr);
                    } else {
                        // Reset Homogeneity Structures to default if no lookahead is detected
                        reset_homogeneity_structures(picture_control_set_ptr);
                    }
                    ...
                    eb_vp9_get_empty_object(sequence_control_set_ptr->encode_context_ptr->stream_output_fifo_ptr,
                                            &output_stream_wrapper_ptr);
                    picture_control_set_ptr->output_stream_wrapper_ptr = output_stream_wrapper_ptr;

                    // Get Empty Results Object
                    eb_vp9_get_empty_object(context_ptr->initialrate_control_results_output_fifo_ptr,
