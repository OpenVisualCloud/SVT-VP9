 ...
 * Macros
 **********************************************************/
#define PRED_STRUCT_INDEX(hierarchical_level_count, pred_type, ref_count) \
    (((hierarchical_level_count) * EB_PRED_TOTAL_COUNT + (pred_type)) + (ref_count))
 ...
 ************************************************/
PredictionStructure *eb_vp9_get_prediction_structure(PredictionStructureGroup *prediction_structure_group_ptr,
 ...
 ******************************************************************************************/
static EbErrorType prediction_structure_ctor(PredictionStructure            **prediction_structure_dbl_ptr,
                                             const PredictionStructureConfig *prediction_structure_config_ptr,
                                             EB_PRED pred_type, uint32_t number_of_references) {
    ...
    *prediction_structure_dbl_ptr = prediction_structure_ptr;
    ...
    //----------------------------------------
    {
        ...
        for (config_entry_index = 0, entry_index = prediction_structure_config_ptr->entry_count - 1;
             config_entry_index < prediction_structure_config_ptr->entry_count;
             ++config_entry_index) {
            ...
            while (ref_index < number_of_references &&
                ...
                max_ref = MAX((int32_t)(prediction_structure_config_ptr->entry_count - entry_index - 1) +
                                  prediction_structure_config_ptr->entry_array[config_entry_index].ref_list0,
                              max_ref);
                ++ref_index;
            }
            ...
            if (pred_type == EB_PRED_RANDOM_ACCESS) {
                ...
                while (ref_index < number_of_references &&
                    ...
                    max_ref = MAX((int32_t)(prediction_structure_config_ptr->entry_count - entry_index - 1) +
                                      prediction_structure_config_ptr->entry_array[config_entry_index].ref_list1,
                                  max_ref);
                    ++ref_index;
                }
            }
        ...
        prediction_structure_ptr->pred_struct_entry_count = leading_pic_count + init_pic_count + steady_state_pic_count;
    ...
    EB_MALLOC(PredictionStructureEntry **,
              prediction_structure_ptr->pred_struct_entry_ptr_array,
              sizeof(PredictionStructureEntry *) * prediction_structure_ptr->pred_struct_entry_count,
              EB_N_PTR);
    ...
    for (config_entry_index = 0; config_entry_index < prediction_structure_config_ptr->entry_count;
         ++config_entry_index) {
        prediction_structure_ptr->temporal_layer_count = MAX(
            prediction_structure_config_ptr->entry_array[config_entry_index].temporal_layer_index,
    ...
    //----------------------------------------
    {
        for (entry_index = 0, config_entry_index = 1; entry_index < leading_pic_count;
            ...
            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->ref_list0.reference_list_count =
                ref_index;
            ...
            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->temporal_layer_index =
                prediction_structure_config_ptr->entry_array[config_entry_index].temporal_layer_index;

            // Set the Decode Order
            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->decode_order = (pred_type ==
    ...
    //----------------------------------------
    {
        ...
        for (config_entry_index = 0, poc_value = 0; entry_index < terminating_entry_index; ++entry_index, ++poc_value) {
            ...
            while (ref_index < number_of_references &&
                   prediction_structure_config_ptr->entry_array[config_entry_index].ref_list0 != 0 &&
                   poc_value - prediction_structure_config_ptr->entry_array[config_entry_index].ref_list0 >=
            ...
            case EB_PRED_LOW_DELAY_B:
                ...
                prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->ref_list1.reference_list_count =
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->ref_list0.reference_list_count;
            ...
            case EB_PRED_RANDOM_ACCESS:
                ...
                while (ref_index < number_of_references &&
                       prediction_structure_config_ptr->entry_array[config_entry_index].ref_list1 != 0 &&
                       poc_value - prediction_structure_config_ptr->entry_array[config_entry_index].ref_list1 >=
                           0) // Stop when we violate the CRA (i.e. reference past it)
                {
                    ++ref_index;
                }
...

    {
        ...
        // First, determine the Dependent List Size for each Entry by incrementing the dependent list length
        {
            ...
            for (picture_number = 0, entry_index = 0; picture_number < prediction_structure_ptr->steady_state_index;
                 ++picture_number) {
                ...
                for (ref_index = 0; ref_index <
                    ...
                    if (dep_index >= 0 &&
                        dep_index < (int32_t)(prediction_structure_ptr->steady_state_index +
                                              prediction_structure_ptr->pred_struct_period)) {
            ...
            for (entry_index = prediction_structure_ptr->steady_state_index; picture_number <=
                 prediction_structure_ptr->steady_state_index + 2 * prediction_structure_ptr->maximum_extent;
                 ++picture_number) {
                // Go through each Reference picture and accumulate counts
                for (ref_index = 0; ref_index <
                     prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->ref_list0.reference_list_count;
        ...
        for (entry_index = 0; entry_index < prediction_structure_ptr->pred_struct_entry_count; ++entry_index) {
            ...
            if (prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list0.list_count > 0) {
                EB_MALLOC(int32_t *,
                          prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list0.list,
                          sizeof(int32_t) *
            ...
            for (picture_number = 0, entry_index = 0; picture_number < prediction_structure_ptr->steady_state_index;
                ...
                for (ref_index = 0; ref_index <
                    ...
                    if (dep_index >= 0 &&
                        dep_index < (int32_t)(prediction_structure_ptr->steady_state_index +
                                              prediction_structure_ptr->pred_struct_period)) {
                        prediction_structure_ptr->pred_struct_entry_ptr_array[dep_index]->dep_list0.list
                            [prediction_structure_ptr->pred_struct_entry_ptr_array[dep_index]->dep_list0.list_count++] =
                            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
...

    {
        ...
        for (entry_index = 0; entry_index < prediction_structure_ptr->pred_struct_entry_count; ++entry_index) {
            ...
            if (prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list1.list_count > 0) {
                EB_MALLOC(int32_t *,
                          prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list1.list,
                          sizeof(int32_t) *
            ...
            for (picture_number = 0, entry_index = 0; picture_number < prediction_structure_ptr->steady_state_index;
                ...
                for (ref_index = 0; ref_index <
                    ...
                    if (dep_index >= 0 &&
                        dep_index < (int32_t)(prediction_structure_ptr->steady_state_index +
                                              prediction_structure_ptr->pred_struct_period)) {
                        prediction_structure_ptr->pred_struct_entry_ptr_array[dep_index]->dep_list1.list
                            [prediction_structure_ptr->pred_struct_entry_ptr_array[dep_index]->dep_list1.list_count++] =
                            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
    ...
    //----------------------------------------
    {
        ...
        for (current_poc_index = 0, entry_index = 0; current_poc_index < decode_order_table_size; ++current_poc_index) {
            ...
            if (pred_type == EB_PRED_RANDOM_ACCESS) {
                decode_order_table[current_poc_index] = base_number +
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->decode_order;
            } else {
        ...
        for (ref_poc_index = 0, entry_index = 0; ref_poc_index < timeline_size; ++ref_poc_index) {
            ...
            for (dep_index = 0;
                 dep_index < prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list0.list_count;
                 ++dep_index) {
                adjusted_dep_index =
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list0.list[dep_index] +
            ...
            for (dep_index = 0;
                 dep_index < prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list1.list_count;
                 ++dep_index) {
                adjusted_dep_index =
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->dep_list1.list[dep_index] +
                    (int32_t)ref_poc_index;
        ...
        for (entry_index = 0; entry_index < prediction_structure_ptr->pred_struct_entry_count; ++entry_index) {
                ...
                if (poc_in_timeline == EB_TRUE) {
                    ...
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
                        ->delta_negative_gop_pos_minus1[prediction_structure_ptr
                                                            ->pred_struct_entry_ptr_array[entry_index]
                                                            ->negative_ref_pics_total_count++] = delta_poc - 1 -
                        prev_delta_poc;
                ...
                if (poc_in_timeline == EB_TRUE) {
                    ...
                    prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
                        ->delta_positive_gop_pos_minus1[prediction_structure_ptr
                                                            ->pred_struct_entry_ptr_array[entry_index]
                                                            ->positive_ref_pics_total_count++] = -delta_poc - 1 -
                        prev_delta_poc;
                    prev_delta_poc = -delta_poc;
                }
            }
            ...
            prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]->ref_pics_override_total_count_flag =
                ((prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
                      ->ref_pics_list0_total_count_minus1 !=
                  (int32_t)prediction_structure_ptr->default_ref_pics_list0_total_count_minus1) ||
                 (prediction_structure_ptr->pred_struct_entry_ptr_array[entry_index]
                      ->ref_pics_list1_total_count_minus1 !=
 ...
 *
 *************************************************/
EbErrorType eb_vp9_prediction_structure_group_ctor(PredictionStructureGroup **prediction_structure_group_dbl_ptr,
    ...
    *prediction_structure_group_dbl_ptr = prediction_structure_group_ptr;
    ...
    while ((prediction_structure_config_array[pred_struct_index].entry_array != 0) &&
        ...
        if (prediction_structure_config_array[pred_struct_index].entry_array->temporal_layer_index == 0 &&
    ...
    EB_MALLOC(PredictionStructure **,
              prediction_structure_group_ptr->prediction_structure_ptr_array,
              sizeof(PredictionStructure *) * prediction_structure_group_ptr->prediction_structure_count,
              EB_N_PTR);
