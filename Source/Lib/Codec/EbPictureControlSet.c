...
EbErrorType eb_vp9_picture_control_set_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    ...
    *object_dbl_ptr = (EbPtr)object_ptr;
    ...
    if (init_data_ptr->is16bit == EB_TRUE) {
        ...
        return_error = eb_vp9_recon_picture_buffer_desc_ctor((EbPtr *)&(object_ptr->recon_picture_ptr),
    ...
    // Entropy Coder
    return_error = eb_vp9_entropy_coder_ctor(&object_ptr->entropy_coder_ptr, SEGMENT_ENTROPY_BUFFER_SIZE);
    ...
    return_error = eb_vp9_bitstream_ctor(&object_ptr->bitstream_ptr, PACKETIZATION_PROCESS_BUFFER_SIZE);
    ...
    for (sb_index = 0; sb_index < object_ptr->sb_total_count; ++sb_index) {
        return_error = sb_unit_ctor(&(object_ptr->sb_ptr_array[sb_index]),
    ...
    for (depth = 0; depth < NEIGHBOR_ARRAY_TOTAL_COUNT; depth++) {
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->md_luma_recon_neighbor_array[depth],
        ...
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->md_cb_recon_neighbor_array[depth],
        ...
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->md_cr_recon_neighbor_array[depth],
                                                       MAX_PICTURE_WIDTH_SIZE >> 1,
                                                       MAX_PICTURE_HEIGHT_SIZE >> 1,
                                                       sizeof(uint8_t),
                                                       SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
    ...
    return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_luma_recon_neighbor_array,
                                                   MAX_PICTURE_WIDTH_SIZE,
                                                   MAX_PICTURE_HEIGHT_SIZE,
                                                   sizeof(uint8_t),
                                                   SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
    ...
    return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_cb_recon_neighbor_array,
                                                   MAX_PICTURE_WIDTH_SIZE >> 1,
                                                   MAX_PICTURE_HEIGHT_SIZE >> 1,
                                                   sizeof(uint8_t),
                                                   SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
    ...
    return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_cr_recon_neighbor_array,
                                                   MAX_PICTURE_WIDTH_SIZE >> 1,
                                                   MAX_PICTURE_HEIGHT_SIZE >> 1,
                                                   sizeof(uint8_t),
                                                   SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
    ...
    if (is16bit) {
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_luma_recon_neighbor_array_16bit,
                                                       MAX_PICTURE_WIDTH_SIZE,
                                                       MAX_PICTURE_HEIGHT_SIZE,
                                                       sizeof(uint16_t),
                                                       SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
        ...
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_cb_recon_neighbor_array_16bit,
                                                       MAX_PICTURE_WIDTH_SIZE >> 1,
                                                       MAX_PICTURE_HEIGHT_SIZE >> 1,
                                                       sizeof(uint16_t),
                                                       SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
        ...
        return_error = eb_vp9_neighbor_array_unit_ctor(&object_ptr->ep_cr_recon_neighbor_array_16bit,
                                                       MAX_PICTURE_WIDTH_SIZE >> 1,
                                                       MAX_PICTURE_HEIGHT_SIZE >> 1,
                                                       sizeof(uint16_t),
                                                       SAMPLE_NEIGHBOR_ARRAY_GRANULARITY,
    ...
    for (uint8_t depth = 0; depth < NEIGHBOR_ARRAY_TOTAL_COUNT; depth++) {
        EB_MALLOC(PARTITION_CONTEXT *,
                  ...
                  sizeof(PARTITION_CONTEXT) * mi_cols_aligned_to_sb(mi_cols),
                  EB_N_PTR);
        EB_MALLOC(PARTITION_CONTEXT *,
                  object_ptr->md_left_seg_context[depth],
                  sizeof(PARTITION_CONTEXT) * mi_cols_aligned_to_sb(mi_rows),
                  EB_N_PTR);
        EB_MALLOC(ENTROPY_CONTEXT *,
                  object_ptr->md_above_context[depth],
                  sizeof(ENTROPY_CONTEXT) * MAX_MB_PLANE * 2 * mi_cols_aligned_to_sb(mi_cols),
                  EB_N_PTR);
        EB_MALLOC(ENTROPY_CONTEXT *,
                  object_ptr->md_left_context[depth],
                  sizeof(ENTROPY_CONTEXT) * MAX_MB_PLANE * 2 * mi_cols_aligned_to_sb(mi_rows),
                  EB_N_PTR);
    }

    EB_MALLOC(ENTROPY_CONTEXT *,
              object_ptr->ep_above_context,
              sizeof(ENTROPY_CONTEXT) * MAX_MB_PLANE * 2 * mi_cols_aligned_to_sb(mi_cols),
              EB_N_PTR);
    EB_MALLOC(ENTROPY_CONTEXT *,
    ...
    return_error = eb_vp9_enc_dec_segments_ctor(
        &object_ptr->enc_dec_segment_ctrl, init_data_ptr->enc_dec_segment_col, init_data_ptr->enc_dec_segment_row);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    // Entropy Rows
...
EbErrorType eb_vp9_picture_parent_control_set_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    ...
    EB_MALLOC(uint16_t *,
              object_ptr->cpi->td.mb.plane[0].eobs,
              sizeof(uint16_t) * (MAX_CU_SIZE << 2) * (MAX_CU_SIZE << 2),
              EB_N_PTR);
    EB_MALLOC(
        tran_low_t *, object_ptr->cpi->td.mb.plane[0].qcoeff, sizeof(tran_low_t) * MAX_CU_SIZE * MAX_CU_SIZE, EB_N_PTR);

    EB_MALLOC(uint16_t *, object_ptr->cpi->td.mb.plane[1].eobs, sizeof(uint16_t), EB_N_PTR);
    EB_MALLOC(
        tran_low_t *, object_ptr->cpi->td.mb.plane[1].qcoeff, sizeof(tran_low_t) * MAX_CU_SIZE * MAX_CU_SIZE, EB_N_PTR);
    ...
    EB_MALLOC(ENTROPY_CONTEXT *,
              object_ptr->cpi->common.above_context,
              sizeof(ENTROPY_CONTEXT) * MAX_MB_PLANE * 2 * mi_cols_aligned_to_sb(object_ptr->cpi->common.mi_cols),
              EB_N_PTR);

    object_ptr->cpi->common.lf.lfm_stride = (object_ptr->cpi->common.mi_cols + (MI_BLOCK_SIZE - 1)) >> 3;

    EB_MALLOC(LOOP_FILTER_MASK *,
              object_ptr->cpi->common.lf.lfm,
              sizeof(LOOP_FILTER_MASK) * ((object_ptr->cpi->common.mi_rows + (MI_BLOCK_SIZE - 1)) >> 3) *
                  object_ptr->cpi->common.lf.lfm_stride,
              EB_N_PTR);
    ...
    *object_dbl_ptr = (EbPtr)object_ptr;
    ...
    EB_MALLOC(
        uint32_t ****, object_ptr->picture_histogram, sizeof(uint32_t ***) * MAX_NUMBER_OF_REGIONS_IN_WIDTH, EB_N_PTR);

    for (region_in_picture_width_index = 0; region_in_picture_width_index < MAX_NUMBER_OF_REGIONS_IN_WIDTH;
         region_in_picture_width_index++) { // loop over horizontal regions
        EB_MALLOC(uint32_t ***,
                  object_ptr->picture_histogram[region_in_picture_width_index],
    ...
    for (region_in_picture_width_index = 0; region_in_picture_width_index < MAX_NUMBER_OF_REGIONS_IN_WIDTH;
         region_in_picture_width_index++) { // loop over horizontal regions
        for (region_in_picture_height_index = 0; region_in_picture_height_index < MAX_NUMBER_OF_REGIONS_IN_HEIGHT;
        ...
        for (region_in_picture_height_index = 0; region_in_picture_height_index < MAX_NUMBER_OF_REGIONS_IN_HEIGHT;
            ...
            for (video_component = 0; video_component < 3; ++video_component) {
                EB_MALLOC(uint32_t *,
                          object_ptr->picture_histogram[region_in_picture_width_index][region_in_picture_height_index]
                                                       [video_component],
                          sizeof(uint32_t) * HISTOGRAM_NUMBER_OF_BINS,
                          EB_N_PTR);
            }
        }
    }
    ...
    EB_MALLOC(
        uint16_t *, object_ptr->ois_distortion_histogram, sizeof(uint16_t) * NUMBER_OF_INTRA_SAD_INTERVALS, EB_N_PTR);

    EB_MALLOC(
        uint32_t *, object_ptr->intra_sad_interval_index, sizeof(uint32_t) * object_ptr->sb_total_count, EB_N_PTR);
    EB_MALLOC(
        uint32_t *, object_ptr->inter_sad_interval_index, sizeof(uint32_t) * object_ptr->sb_total_count, EB_N_PTR);
    ...
    EB_MALLOC(EB_SB_COMPLEXITY_STATUS *,
              object_ptr->complex_sb_array,
              sizeof(EB_SB_COMPLEXITY_STATUS) * object_ptr->sb_total_count,
              EB_N_PTR);

    EB_CREATEMUTEX(EbHandle, object_ptr->rc_distortion_histogram_mutex, sizeof(EbHandle), EB_MUTEX);

    EB_MALLOC(EB_SB_DEPTH_MODE *,
              object_ptr->sb_depth_mode_array,
              sizeof(EB_SB_DEPTH_MODE) * object_ptr->sb_total_count,
              EB_N_PTR);
