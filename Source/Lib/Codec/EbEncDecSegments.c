...
EbErrorType eb_vp9_enc_dec_segments_ctor(EncDecSegments **segments_dbl_ptr, uint32_t segment_col_count,
    ...
    *segments_dbl_ptr = segments_ptr;
    ...
    // Start Arrays
    EB_MALLOC(
        uint16_t *, segments_ptr->x_start_array, sizeof(uint16_t) * segments_ptr->segment_max_total_count, EB_N_PTR);

    EB_MALLOC(
        uint16_t *, segments_ptr->y_start_array, sizeof(uint16_t) * segments_ptr->segment_max_total_count, EB_N_PTR);

    EB_MALLOC(uint16_t *,
              segments_ptr->valid_sb_count_array,
              sizeof(uint16_t) * segments_ptr->segment_max_total_count,
              EB_N_PTR);

    // Dependency map
    EB_MALLOC(uint8_t *,
              segments_ptr->dep_map.dependency_map,
              sizeof(uint8_t) * segments_ptr->segment_max_total_count,
              EB_N_PTR);

    EB_CREATEMUTEX(EbHandle, segments_ptr->dep_map.update_mutex, sizeof(EbHandle), EB_MUTEX);

    // Segment rows
    EB_MALLOC(EncDecSegSegmentRow *,
              segments_ptr->row_array,
              sizeof(EncDecSegSegmentRow) * segments_ptr->segment_max_row_count,
              EB_N_PTR)
...
void eb_vp9_enc_dec_segments_init(EncDecSegments *segments_ptr, uint32_t seg_col_count, uint32_t seg_row_count,
    ...
    segments_ptr->segment_row_count   = seg_row_count;
    ...
    for (y = 0; y < pic_height_sb; ++y) {
        for (x = 0; x < pic_width_sb; ++x) {
            band_index    = BAND_INDEX(x, y, segments_ptr->segment_band_count, segments_ptr->sb_band_count);
    ...
    for (row_index = 0; row_index < segments_ptr->segment_row_count; ++row_index) {
        ...
        yLast = ((((row_index + 1) * segments_ptr->sb_row_count) + (segments_ptr->segment_row_count - 1)) /
                 segments_ptr->segment_row_count) -
            1;
        band_index = BAND_INDEX(0, y, segments_ptr->segment_band_count, segments_ptr->sb_band_count);
        ...
        band_index = BAND_INDEX(pic_width_sb - 1, yLast, segments_ptr->segment_band_count, segments_ptr->sb_band_count);
