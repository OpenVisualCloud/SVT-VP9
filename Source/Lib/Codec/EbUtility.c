...
PaBlockStats *pa_get_block_stats(int block_index) { return &pa_get_block_stats_array[block_index]; }
...
static BLOCK_SIZE get_plane_block_size(BLOCK_SIZE bsize, int subsampling_x, int subsampling_y) {
    ...
    for (part_it = 0; part_it < max_part_updated; part_it++) {
        ...
        for (nsq_it = 0; nsq_it < tot_num_ns_per_part; nsq_it++) {
            ...
            if (ep_block_stats_ptr_mds[*idx_mds].bwidth == 4 && ep_block_stats_ptr_mds[*idx_mds].bheight == 4)
                ...
                if ((ep_block_stats_ptr_mds[*idx_mds].bwidth >> 1) < ep_block_stats_ptr_mds[*idx_mds].bwidth_uv ||
                    (ep_block_stats_ptr_mds[*idx_mds].bheight >> 1) < ep_block_stats_ptr_mds[*idx_mds].bheight_uv) {
                int num_blk_same_uv = 1;
                if (ep_block_stats_ptr_mds[*idx_mds].bwidth >> 1 < 4)
                    num_blk_same_uv *= 2;
                if (ep_block_stats_ptr_mds[*idx_mds].bheight >> 1 < 4)
                    num_blk_same_uv *= 2;
                if (ep_block_stats_ptr_mds[*idx_mds].nsi != (num_blk_same_uv - 1) &&
                    ep_block_stats_ptr_mds[*idx_mds].nsi != (2 * num_blk_same_uv - 1))
                    ep_block_stats_ptr_mds[*idx_mds].has_uv = 0;
            }

            ep_block_stats_ptr_mds[*idx_mds].bsize_uv = get_plane_block_size(
                ep_block_stats_ptr_mds[*idx_mds].bsize, 1, 1);
            ep_block_stats_ptr_mds[*idx_mds].tx_size = blocksize_to_txsize[ep_block_stats_ptr_mds[*idx_mds].bsize];
            ep_block_stats_ptr_mds[*idx_mds].tx_size_uv =
                blocksize_to_txsize[ep_block_stats_ptr_mds[*idx_mds].bsize_uv];
...
const EpBlockStats *ep_get_block_stats(uint32_t bidx_mds) { return &ep_block_stats_ptr_mds[bidx_mds]; }
