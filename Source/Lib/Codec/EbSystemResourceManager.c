 ...
 * eb_fifo_ctor
 **************************************/
static EbErrorType eb_fifo_ctor(EbFifo *fifo_ptr, uint32_t initial_count, uint32_t max_count,
                                EbObjectWrapper *first_wrapper_ptr, EbObjectWrapper *last_wrapper_ptr,
                                EbMuxingQueue *queue_ptr) {
    // Create Counting Semaphore
    EB_CREATESEMAPHORE(
        EbHandle, fifo_ptr->counting_semaphore, sizeof(EbHandle), EB_SEMAPHORE, initial_count, max_count);
 ...
 * eb_fifo_push_back
 **************************************/
static EbErrorType eb_fifo_push_back(EbFifo *fifo_ptr, EbObjectWrapper *wrapper_ptr) {
 ...
 * eb_fifo_pop_front
 **************************************/
static EbErrorType eb_fifo_pop_front(EbFifo *fifo_ptr, EbObjectWrapper **wrapper_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 **************************************/
static EbErrorType eb_circular_buffer_ctor(EbCircularBuffer **buffer_dbl_ptr, uint32_t buffer_total_count) {
 ...
 **************************************/
static EbErrorType eb_circular_buffer_pop_front(EbCircularBuffer *buffer_ptr, EbPtr *object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 **************************************/
static EbErrorType eb_circular_buffer_push_back(EbCircularBuffer *buffer_ptr, EbPtr object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    buffer_ptr->tail_index = (buffer_ptr->tail_index == buffer_ptr->buffer_total_count - 1)
        ? 0
        : buffer_ptr->tail_index + 1;
 ...
 **************************************/
static EbErrorType eb_circular_buffer_push_front(EbCircularBuffer *buffer_ptr, EbPtr object_ptr) {
    EbErrorType return_error = EB_ErrorNone;

    // Decrement the head_index
    buffer_ptr->head_index = (buffer_ptr->head_index == 0) ? buffer_ptr->buffer_total_count - 1
                                                           : buffer_ptr->head_index - 1;
 ...
 **************************************/
static EbErrorType eb_muxing_queue_ctor(EbMuxingQueue **queue_dbl_ptr, uint32_t object_total_count,
                                        uint32_t process_total_count, EbFifo ***process_fifo_ptr_array_ptr) {
    EbMuxingQueue *queue_ptr;
    ...
    *queue_dbl_ptr = queue_ptr;
    ...
    return_error = eb_circular_buffer_ctor(&queue_ptr->object_queue, object_total_count);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    // Construct Process Circular Buffer
    return_error = eb_circular_buffer_ctor(&queue_ptr->process_queue, queue_ptr->process_total_count);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    // Construct the Process Fifos
    EB_MALLOC(
        EbFifo **, queue_ptr->process_fifo_ptr_array, sizeof(EbFifo *) * queue_ptr->process_total_count, EB_N_PTR);
    ...
    for (process_index = 0; process_index < queue_ptr->process_total_count; ++process_index) {
        EB_MALLOC(EbFifo *,
                  ...
                  sizeof(EbFifo) * queue_ptr->process_total_count,
                  EB_N_PTR);
 ...
 **************************************/
static EbErrorType eb_muxing_queue_object_push_back(EbMuxingQueue *queue_ptr, EbObjectWrapper *object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
...
**************************************/
static EbErrorType eb_muxing_queue_object_push_front(EbMuxingQueue *queue_ptr, EbObjectWrapper *object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *********************************************************************/
EbErrorType eb_vp9_object_release_enable(EbObjectWrapper *wrapper_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *      pointer to the EbObjectWrapper to be modified.
 *********************************************************************/
EbErrorType eb_vp9_object_release_disable(EbObjectWrapper *wrapper_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *      pointer to the EbObjectWrapper to be modified.
 *********************************************************************/
EbErrorType eb_vp9_object_inc_live_count(EbObjectWrapper *wrapper_ptr, uint32_t increment_number) {
    EbErrorType return_error = EB_ErrorNone;
...
EbErrorType eb_vp9_system_resource_ctor(EbSystemResource **resource_dbl_ptr, uint32_t object_total_count,
    ...
    *resource_dbl_ptr = resource_ptr;
    ...
    EB_MALLOC(EbObjectWrapper **,
              ...
              sizeof(EbObjectWrapper *) * resource_ptr->object_total_count,
              EB_N_PTR);
    ...
    for (wrapperIndex = 0; wrapperIndex < resource_ptr->object_total_count; ++wrapperIndex) {
        ...
        if (object_ctor) {
            return_error = object_ctor(&resource_ptr->wrapper_ptr_pool[wrapperIndex]->object_ptr, object_init_data_ptr);
            if (return_error == EB_ErrorInsufficientResources) {
                return EB_ErrorInsufficientResources;
            }
        }
    ...
    return_error = eb_muxing_queue_ctor(&resource_ptr->empty_queue,
    ...
    for (wrapperIndex = 0; wrapperIndex < resource_ptr->object_total_count; ++wrapperIndex) {
        eb_muxing_queue_object_push_back(resource_ptr->empty_queue, resource_ptr->wrapper_ptr_pool[wrapperIndex]);
    }
 ...
 *********************************************************************/
EbErrorType eb_vp9_post_full_object(EbObjectWrapper *object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *********************************************************************/
EbErrorType eb_vp9_release_object(EbObjectWrapper *object_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *********************************************************************/
EbErrorType eb_vp9_get_empty_object(EbFifo *empty_fifo_ptr, EbObjectWrapper **wrapper_dbl_ptr) {
    EbErrorType return_error = EB_ErrorNone;
 ...
 *      EbObjectWrapper pointer.
 *********************************************************************/
EbErrorType eb_vp9_get_full_object(EbFifo *full_fifo_ptr, EbObjectWrapper **wrapper_dbl_ptr) {
    EbErrorType return_error = EB_ErrorNone;
