 ...
 **********************************/
EbErrorType output_bitstream_write(OutputBitstreamUnit *bitstream_ptr, uint32_t bits, uint32_t number_of_bits) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    if ((int32_t)number_of_bits < bitstream_ptr->valid_bits_count) {
        ...
        *bitstream_ptr->buffer++ = (uint8_t)eb_vp9_endian_swap(bitstream_ptr->byte_holder);
        ...
        bitstream_ptr->byte_holder      = (0 == shift_count) ? 0 : (bits << bitstream_ptr->valid_bits_count);
    }

    return return_error;
}

EbErrorType output_bitstream_write_byte(OutputBitstreamUnit *bitstream_ptr, uint32_t bits) {
    EbErrorType return_error = EB_ErrorNone;
...
EbErrorType output_bitstream_write_align_zero(OutputBitstreamUnit *bitstream_ptr) {
    ...
    output_bitstream_write(bitstream_ptr, 0, bitstream_ptr->valid_bits_count & 0x7);
 ...
 *   Intended to be used in CABAC
 **********************************/
EbErrorType eb_vp9_output_bitstream_rbsp_to_payload(OutputBitstreamUnit *bitstream_ptr, EbByte output_buffer,
