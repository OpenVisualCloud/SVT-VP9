...
static const uint8_t adp_luminosity_change_th_array[MAX_HIERARCHICAL_LEVEL][MAX_TEMPORAL_LAYERS] = {
    // [Highest Temporal Layer] [Temporal Layer Index]
    {2},
    {2, 2},
    {3, 2, 2},
    {3, 3, 2, 2},
 ...
 ******************************************************/
EbErrorType eb_vp9_mode_decision_configuration_context_ctor(ModeDecisionConfigurationContext **context_dbl_ptr,
...
EB_AURA_STATUS aura_detection64x64(PictureControlSet *picture_control_set_ptr, uint8_t picture_qp, uint32_t sb_index) {
    ...
    dist_thresh0 = picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag ||
    ...
    if (!sb_params->is_edge_sb) {
        ...
        if ((curr_dist > 64 * 64) &&
            ...
            (abs(x_mv0) > global_motion_threshold[picture_control_set_ptr->parent_pcs_ptr->hierarchical_levels]
                                                 [picture_control_set_ptr->parent_pcs_ptr->temporal_layer_index] ||
             abs(y_mv0) > global_motion_threshold[picture_control_set_ptr->parent_pcs_ptr->hierarchical_levels]
                                                 [picture_control_set_ptr->parent_pcs_ptr->temporal_layer_index] ||
             abs(x_mv1) > global_motion_threshold[picture_control_set_ptr->parent_pcs_ptr->hierarchical_levels]
                                                 [picture_control_set_ptr->parent_pcs_ptr->temporal_layer_index] ||
             abs(y_mv1) > global_motion_threshold[picture_control_set_ptr->parent_pcs_ptr->hierarchical_levels]
                                                 [picture_control_set_ptr->parent_pcs_ptr->temporal_layer_index])) {
            //Top Distortion
            sb_offset = -picture_width_in_sb;
            top_dist  = picture_control_set_ptr->parent_pcs_ptr->me_results[sb_index + sb_offset]
                           ->distortion_direction[0]
                           .distortion;

            //TopLeft Distortion
            sb_offset  = -picture_width_in_sb - 1;
            top_l_dist = picture_control_set_ptr->parent_pcs_ptr->me_results[sb_index + sb_offset]
                             ->distortion_direction[0]
                             .distortion;

            //TopRightDistortion
            sb_offset  = -picture_width_in_sb + 1;
            top_r_dist = picture_control_set_ptr->parent_pcs_ptr->me_results[sb_index + sb_offset]
                             ->distortion_direction[0]
                             .distortion;
            ...
            //left Distortion
            sb_offset = -1;
            left_dist = picture_control_set_ptr->parent_pcs_ptr->me_results[sb_index + sb_offset]
                            ->distortion_direction[0]
                            .distortion;
...
EbErrorType eb_vp9_derive_default_segments(PictureControlSet                *picture_control_set_ptr,
    ...
    if (picture_control_set_ptr->temporal_layer_index == 0) {
        if (context_ptr->adp_depth_sensitive_picture_class &&
            context_ptr->budget >=
                (uint32_t)(picture_control_set_ptr->parent_pcs_ptr->sb_total_count * LIGHT_BDP_COST)) {
            ...
            else if (context_ptr->budget >
                     (uint32_t)(picture_control_set_ptr->parent_pcs_ptr->sb_total_count * OPEN_LOOP_COST)) {
...
void eb_vp9_configure_adp(PictureControlSet *picture_control_set_ptr, ModeDecisionConfigurationContext *context_ptr) {
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->non_moving_average_score != INVALID_NON_MOVING_SCORE &&
        picture_control_set_ptr->parent_pcs_ptr->non_moving_average_score <
            ADP_CONFIG_NON_MOVING_INDEX_TH_1) { // could not seen by the eye if very active
        if (picture_control_set_ptr->parent_pcs_ptr->pic_noise_class > PIC_NOISE_CLASS_3 ||
            picture_control_set_ptr->parent_pcs_ptr->high_dark_low_light_area_density_flag ||
            luminosity_change) { // potential complex picture: luminosity Change (e.g. fade, light..)
        ...
        else if (picture_control_set_ptr->parent_pcs_ptr->non_moving_average_score >=
                     ADP_CONFIG_NON_MOVING_INDEX_TH_0 &&
                 picture_control_set_ptr->parent_pcs_ptr->pic_noise_class == PIC_NOISE_CLASS_3) {
            context_ptr->adp_depth_sensitive_picture_class = DEPTH_SENSITIVE_PIC_CLASS_1;
        }
    }
}
...
void eb_vp9_set_sb_budget(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    if (context_ptr->adp_refinement_mode == 2 && is_avc_partitioning_mode_flag) {
        ...
        context_ptr->sb_score_array[sb_index] = CLIP3(
            context_ptr->sb_min_score, context_ptr->sb_max_score, context_ptr->sb_score_array[sb_index]);
        ...
        if ((score_to_min <= (max_to_min_score * context_ptr->score_th[0]) / 100 && context_ptr->score_th[0] != 0) ||
        ...
        if (is_avc_partitioning_mode_flag) {
            if (context_ptr->sb_cost_array[sb_index] > context_ptr->cost_depth_mode[SB_AVC_DEPTH_MODE - 1]) {
                context_ptr->predicted_cost -= (context_ptr->sb_cost_array[sb_index] -
                                                context_ptr->cost_depth_mode[SB_AVC_DEPTH_MODE - 1]);
                context_ptr->sb_cost_array[sb_index] = context_ptr->cost_depth_mode[SB_AVC_DEPTH_MODE - 1];
            } else if (context_ptr->sb_cost_array[sb_index] >
                           context_ptr->cost_depth_mode[SB_LIGHT_AVC_DEPTH_MODE - 1] &&
...
void eb_vp9_derive_optimal_budget_per_sb(SequenceControlSet               *sequence_control_set_ptr,
    ...
    while (deviation_to_target != 0 && (initial_shooting == final_shooting) &&
        ...
        deviation_to_target = (ABS((int32_t)(context_ptr->predicted_cost - context_ptr->budget)) * 1000) /
            context_ptr->budget;
...
void compute_refinement_cost(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    if (avc_refinement_cost <= context_ptr->budget &&
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; sb_index++) {
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE) {
            ...
            if (sb_params->pa_raster_scan_block_validity[PA_RASTER_SCAN_CU_INDEX_64x64] == EB_FALSE) {
                ...
                distortion              = 0;
                for (cu8x8Index = PA_RASTER_SCAN_CU_INDEX_8x8_0; cu8x8Index <= PA_RASTER_SCAN_CU_INDEX_8x8_63;
                     cu8x8Index++) {
                    if (sb_params->pa_raster_scan_block_validity[cu8x8Index]) {
                        distortion += picture_control_set_ptr->parent_pcs_ptr->me_results[sb_index][cu8x8Index]
                                          .distortion_direction[0]
                                          .distortion;
                        validCu8x8Count++;
                    }
                ...
                if (validCu8x8Count > 0)
                    distortion = CLIP3(picture_control_set_ptr->parent_pcs_ptr->min_me_distortion,
                                       picture_control_set_ptr->parent_pcs_ptr->max_me_distortion,
                                       (distortion / validCu8x8Count) * 64);

                // Do not perform LCU score manipulation for incomplete LCUs as not valid signals
                sb_score = distortion;

            } else {
                distortion = picture_control_set_ptr->parent_pcs_ptr
                                 ->me_results[sb_index][PA_RASTER_SCAN_CU_INDEX_64x64]
                                 .distortion_direction[0]
                                 .distortion;
                ...
                if (sequence_control_set_ptr->static_config.tune == TUNE_SQ) {
                    if (
                        ...
                        && sequence_control_set_ptr->input_resolution == INPUT_SIZE_4K_RANGE) {
                        sb_score += (((picture_control_set_ptr->parent_pcs_ptr->max_me_distortion - sb_score) *
                                      ADP_CLASS_SHIFT_DIST_1) /
                                     100);

                    } else {
                        // Use LCU variance & activity
                        if (picture_control_set_ptr->parent_pcs_ptr->non_moving_index_array[sb_index] ==
                                ADP_CLASS_NON_MOVING_INDEX_TH_2 &&
                            picture_control_set_ptr->parent_pcs_ptr->variance[sb_index][PA_RASTER_SCAN_CU_INDEX_64x64] >
                                IS_COMPLEX_SB_VARIANCE_TH &&
                            (sequence_control_set_ptr->static_config.frame_rate >> 16) > 30)

                            sb_score -= (((sb_score - picture_control_set_ptr->parent_pcs_ptr->min_me_distortion) *
                                          ADP_CLASS_SHIFT_DIST_0) /
                                         100);
                        // Use LCU luminosity
                        if (sequence_control_set_ptr->input_resolution == INPUT_SIZE_4K_RANGE) {
                            // Shift to the left dark LCUs & shift to the right otherwise ONLY if a high dark area is present
                            if (picture_control_set_ptr->parent_pcs_ptr->black_area_percentage >
                                ADP_BLACK_AREA_PERCENTAGE) {
                                if (picture_control_set_ptr->parent_pcs_ptr
                                        ->y_mean[sb_index][PA_RASTER_SCAN_CU_INDEX_64x64] < ADP_DARK_SB_TH)
                                    sb_score -= (((sb_score -
                                                   picture_control_set_ptr->parent_pcs_ptr->min_me_distortion) *
                                                  ADP_CLASS_SHIFT_DIST_0) /
                                                 100);
                                else
                                    sb_score += (((picture_control_set_ptr->parent_pcs_ptr->max_me_distortion -
                                                   sb_score) *
                                                  ADP_CLASS_SHIFT_DIST_0) /
                                                 100);
                            }
...
void eb_vp9_derive_sb_md_mode(SequenceControlSet *sequence_control_set_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    // Set the target budget
    if (sequence_control_set_ptr->static_config.tune == TUNE_SQ) {
        SetTargetBudgetSq(sequence_control_set_ptr, picture_control_set_ptr, context_ptr);
...
void eb_vp9_MdcInterDepthDecision(ModeDecisionConfigurationContext *context_ptr, uint32_t origin_x, uint32_t origin_y,
    ...
    if (GROUP_OF_4_16x16_BLOCKS(pa_get_block_stats(depth_two_candidate_block_index)->origin_x,
                                pa_get_block_stats(depth_two_candidate_block_index)->origin_y) &&
    ...
    if (GROUP_OF_4_32x32_BLOCKS(pa_get_block_stats(depth_one_candidate_block_index)->origin_x,
                                pa_get_block_stats(depth_one_candidate_block_index)->origin_y) &&
...
static void prediction_partition_loop(SequenceControlSet               *sequence_control_set_ptr,
                                      PictureControlSet                *picture_control_set_ptr,
                                      ModeDecisionConfigurationContext *context_ptr, SbUnit *sb_ptr,
                                      uint32_t start_depth, uint32_t end_depth) {
    ...
    for (block_index = 0; block_index < PA_BLOCK_MAX_COUNT; ++block_index) {
        ...
        if (sb_params->pa_raster_scan_block_validity[block_index_in_rater_scan]) {
            ...
            if (depth >= start_depth && depth <= end_depth) {
                if (picture_control_set_ptr->slice_type != I_SLICE) {
                    MeCuResults *me_pu_result = &picture_control_set_ptr->parent_pcs_ptr
                                                     ->me_results[sb_ptr->sb_index][block_index_in_rater_scan];
                    ...
                    block_ptr->early_cost = inter_fast_cost(picture_control_set_ptr,
                                                            0,
                ...
                if (end_depth == 2) {
                    context_ptr->group_of8x8_blocks_count = depth == 2 ? incremental_count[block_index_in_rater_scan]
                                                                       : 0;
                } else if (end_depth == 1) {
                    context_ptr->group_of16x16_blocks_count = depth == 1 ? incremental_count[block_index_in_rater_scan]
                                                                         : 0;
                }
...
static const uint8_t parentblock_index[PA_BLOCK_MAX_COUNT] = {
    0, 0,  0, 0, 1,  2, 3,  5, 0, 1,  2, 3, 10, 0, 1,  2, 3, 15, 0, 1, 2, 3, 21, 0, 0, 1,  2, 3, 5,
    0, 1,  2, 3, 10, 0, 1,  2, 3, 15, 0, 1, 2,  3, 42, 0, 0, 1,  2, 3, 5, 0, 1,  2, 3, 10, 0, 1, 2,
    3, 15, 0, 1, 2,  3, 36, 0, 0, 1,  2, 3, 5,  0, 1,  2, 3, 10, 0, 1, 2, 3, 15, 0, 1, 2,  3,
};

static EbErrorType mdc_refinement(MdcpLocalCodingUnit *local_cu_array, uint32_t block_index, uint32_t depth,
                                  uint8_t refinement_level, uint8_t lowest_level) {
    EbErrorType return_error = EB_ErrorNone;
...
static void refinement_prediction_loop(SequenceControlSet *sequence_control_set_ptr,
                                       PictureControlSet *picture_control_set_ptr, SbUnit *sb_ptr, uint32_t sb_index,
                                       ModeDecisionConfigurationContext *context_ptr) {
    MdcpLocalCodingUnit *local_cu_array       = context_ptr->local_cu_array;
    SbParams            *sb_params            = &sequence_control_set_ptr->sb_params_array[sb_index];
            ...
            if (sb_ptr->picture_control_set_ptr->slice_type == I_SLICE) {
                {
                    ...
                    lowest_level = (refinement_level & REFINEMENT_Pp3) ? REFINEMENT_Pp3
                        : (refinement_level & REFINEMENT_Pp2)          ? REFINEMENT_Pp2
                        : (refinement_level & REFINEMENT_Pp1)          ? REFINEMENT_Pp1
                        : (refinement_level & REFINEMENT_P)            ? REFINEMENT_P
                        : (refinement_level & REFINEMENT_Pm1)          ? REFINEMENT_Pm1
                        : (refinement_level & REFINEMENT_Pm2)          ? REFINEMENT_Pm2
                        : (refinement_level & REFINEMENT_Pm3)          ? REFINEMENT_Pm3
                                                                       : 0x00;

                    mdc_refinement(&(*context_ptr->local_cu_array), block_index, depth, refinement_level, lowest_level);
                }
            } else {
                ...
                if (picture_control_set_ptr->parent_pcs_ptr->cu8x8_mode == CU_8x8_MODE_1) {
                    refinement_level = ((refinement_level & REFINEMENT_Pp1) && depth == 2)
                        ? refinement_level - REFINEMENT_Pp1
                        : ((refinement_level & REFINEMENT_Pp2) && depth == 1) ? refinement_level - REFINEMENT_Pp2
                        : ((refinement_level & REFINEMENT_Pp3) && depth == 0) ? refinement_level - REFINEMENT_Pp3
                                                                              : refinement_level;
                }
    ...
    while (block_index < PA_BLOCK_MAX_COUNT) {
        ...
        if (sb_params->pa_raster_scan_block_validity[MD_SCAN_TO_RASTER_SCAN[block_index]]) {
            ...
            case 2:
                ...
                cu_class = (block_stats_ptr->size > MAX_CU_SIZE ||
                            (slice_type == I_SLICE && block_stats_ptr->size > MAX_INTRA_SIZE))
                    ? DO_NOT_ADD_CU_CONTINUE_SPLIT
                    : cu_class;

                // Take into account MIN_CU_SIZE
                cu_class = (block_stats_ptr->size == MIN_CU_SIZE ||
                            (slice_type == I_SLICE && block_stats_ptr->size == MIN_INTRA_SIZE))
                    ? ADD_CU_STOP_SPLIT
                    : cu_class;
...
static EbErrorType early_mode_decision_sb(SequenceControlSet               *sequence_control_set_ptr,
                                          PictureControlSet                *picture_control_set_ptr,
                                          ModeDecisionConfigurationContext *context_ptr, SbUnit *sb_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    prediction_partition_loop(
        sequence_control_set_ptr, picture_control_set_ptr, context_ptr, sb_ptr, start_depth, end_depth);

    refinement_prediction_loop(
        sequence_control_set_ptr, picture_control_set_ptr, sb_ptr, sb_ptr->sb_index, context_ptr);

    forward_cu_to_mode_decision(sequence_control_set_ptr, picture_control_set_ptr, sb_ptr->sb_index, context_ptr);
...
void *eb_vp9_mode_decision_configuration_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->rate_control_input_fifo_ptr, &rate_control_results_wrapper_ptr);
        ...
        picture_control_set_ptr  = (PictureControlSet *)
                                      rate_control_results_ptr->picture_control_set_wrapper_ptr->object_ptr;
        ...
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost[0] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvcosts[0][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost[1] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvcosts[1][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost[0] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvsadcosts[0][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost[1] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvsadcosts[1][MV_MAX];
        cal_nmvsadcosts(picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost);

        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost_hp[0] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvcosts_hp[0][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost_hp[1] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvcosts_hp[1][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost_hp[0] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvsadcosts_hp[0][MV_MAX];
        picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost_hp[1] =
            &picture_control_set_ptr->parent_pcs_ptr->cpi->nmvsadcosts_hp[1][MV_MAX];
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->cpi->common.allow_high_precision_mv) {
            picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.mvcost =
                picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost_hp;
            picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.mvsadcost =
                picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvsadcost_hp;
        } else {
            picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.mvcost =
                picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.nmvcost;
            picture_control_set_ptr->parent_pcs_ptr->cpi->td.mb.mvsadcost =
        ...
        EB_PICNOISE_CLASS pic_noise_classTH = (picture_control_set_ptr->parent_pcs_ptr->noise_detection_th == 0)
            ? PIC_NOISE_CLASS_1
            : PIC_NOISE_CLASS_3;
        ...
        picture_control_set_ptr->scene_characteristic_id =
            ((!picture_control_set_ptr->parent_pcs_ptr->is_pan) &&
             ...
             (picture_control_set_ptr->parent_pcs_ptr->pic_homogenous_over_time_sb_percentage < 50))
            ? EB_FRAME_CARAC_1
            : picture_control_set_ptr->scene_characteristic_id;

        picture_control_set_ptr->scene_characteristic_id =
            ((picture_control_set_ptr->parent_pcs_ptr->is_pan) && (!picture_control_set_ptr->parent_pcs_ptr->is_tilt) &&
             (picture_control_set_ptr->parent_pcs_ptr->grass_percentage_in_picture > 35) &&
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_SB_SWITCH_DEPTH_MODE) {
            ...
            for (int sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
                if (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] == SB_FULL85_DEPTH_MODE) {
                    sb_depth_85_block(sequence_control_set_ptr, picture_control_set_ptr, sb_index);
                } else if (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                           SB_FULL84_DEPTH_MODE) {
                    sb_depth_84_block(sequence_control_set_ptr, picture_control_set_ptr, sb_index);
                } else if (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                           SB_AVC_DEPTH_MODE) {
                    sb_depth_8x8_16x16_block(sequence_control_set_ptr, picture_control_set_ptr, sb_index);
                } else if (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                           SB_LIGHT_AVC_DEPTH_MODE) {
                    sb_depth_16x16_block(sequence_control_set_ptr, picture_control_set_ptr, sb_index);
                } else if (picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                               SB_OPEN_LOOP_DEPTH_MODE ||
                           picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                               SB_LIGHT_OPEN_LOOP_DEPTH_MODE ||
                           ...
                           picture_control_set_ptr->parent_pcs_ptr->sb_depth_mode_array[sb_index] ==
                               SB_PRED_OPEN_LOOP_1_NFL_DEPTH_MODE) {
                    sb_depth_open_loop(context_ptr, sequence_control_set_ptr, picture_control_set_ptr, sb_index);
                }
            }
        } else if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_FULL85_DEPTH_MODE) {
            picture_depth_85_block(sequence_control_set_ptr, picture_control_set_ptr);
        } else if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_FULL84_DEPTH_MODE) {
            picture_depth_84_block(sequence_control_set_ptr, picture_control_set_ptr);
        } else if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode == PIC_OPEN_LOOP_DEPTH_MODE) {
            picture_depth_open_loop(context_ptr, sequence_control_set_ptr, picture_control_set_ptr);
        }

        // Post the results to the MD processes
        eb_vp9_get_empty_object(context_ptr->mode_decision_configuration_output_fifo_ptr, &enc_dec_tasks_wrapper_ptr);
