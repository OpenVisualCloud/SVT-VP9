...
static inline void svt_vp9_sleep(const int milliseconds) {
    if (milliseconds) {
#ifdef _WIN32
        Sleep(milliseconds);
#else
...
static inline double svt_vp9_compute_overall_elapsed_time_ms(
...
static inline double svt_vp9_compute_overall_elapsed_time(
    const uint64_t start_seconds, const uint64_t start_useconds,
    const uint64_t finish_seconds, const uint64_t finish_useconds) {
...
static inline void svt_vp9_get_time(uint64_t *const seconds,
                                    uint64_t *const useconds) {
#ifdef _WIN32
