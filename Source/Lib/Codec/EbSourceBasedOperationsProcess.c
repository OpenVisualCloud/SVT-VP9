...
EbErrorType eb_vp9_source_based_operations_context_ctor(SourceBasedOperationsContext **context_dbl_ptr,
...
******************************************************/
static void determine_isolated_non_homogeneous_region_in_picture(SequenceControlSet      *sequence_control_set_ptr,
                                                                 PictureParentControlSet *picture_control_set_ptr) {
    uint32_t sb_index;
    uint32_t cuu_index;
    ...
    for (sb_index = 0; sb_index < sb_total_count; ++sb_index) {
        ...
        if ((sb_params->horizontal_index > 0) && (sb_params->horizontal_index < picture_width_in_sb - 1) &&
            ...
            // top neighbors
            count_of_med_variance_sb +=
                ((picture_control_set_ptr->variance[sb_index - picture_width_in_sb - 1][ME_TIER_ZERO_PU_64x64]) <=
                 MEDIUM_SB_VARIANCE)
                ? 1
                : 0;
            count_of_med_variance_sb +=
                (picture_control_set_ptr->variance[sb_index - picture_width_in_sb][ME_TIER_ZERO_PU_64x64] <=
                 MEDIUM_SB_VARIANCE)
                ? 1
                : 0;
            count_of_med_variance_sb +=
                (picture_control_set_ptr->variance[sb_index - picture_width_in_sb + 1][ME_TIER_ZERO_PU_64x64] <=
                     MEDIUM_SB_VARIANCE &&
                 sequence_control_set_ptr->sb_params_array[sb_index - picture_width_in_sb + 1].is_complete_sb)
                ? 1
                : 0;
            // bottom
            count_of_med_variance_sb +=
            ...
            count_of_med_variance_sb +=
                (picture_control_set_ptr->variance[sb_index + picture_width_in_sb][ME_TIER_ZERO_PU_64x64] <=
                     MEDIUM_SB_VARIANCE &&
                 sequence_control_set_ptr->sb_params_array[sb_index + picture_width_in_sb].is_complete_sb)
                ? 1
                : 0;
            count_of_med_variance_sb +=
                (picture_control_set_ptr->variance[sb_index + picture_width_in_sb + 1][ME_TIER_ZERO_PU_64x64] <=
                ...
                : 0;
            // left right
            count_of_med_variance_sb += (picture_control_set_ptr->variance[sb_index + 1][ME_TIER_ZERO_PU_64x64] <=
                                             MEDIUM_SB_VARIANCE &&
                                         sequence_control_set_ptr->sb_params_array[sb_index + 1].is_complete_sb)
                ? 1
                : 0;
            count_of_med_variance_sb += (picture_control_set_ptr->variance[sb_index - 1][ME_TIER_ZERO_PU_64x64] <=
                                         MEDIUM_SB_VARIANCE)
...
static void determine_more_potential_aura_areas(SequenceControlSet      *sequence_control_set_ptr,
                                                PictureParentControlSet *picture_control_set_ptr) {
    uint16_t sb_index;
    int32_t  sb_hor, sb_ver, sb_ver_offset;
        ...
        if (!sb_params->is_edge_sb) {
            ...
            if (picture_control_set_ptr->edge_results_ptr[sb_index].edge_block_num &&
                ...
                for (sb_ver = -1; sb_ver <= 1; sb_ver++) {
                    ...
                    for (sb_hor = -1; sb_hor <= 1; sb_hor++) {
                        count_of_non_edge_blocks += (!picture_control_set_ptr
                                                          ->edge_results_ptr[sb_index + sb_ver_offset + sb_hor]
                                                          .edge_block_num) &&
    ...
    picture_control_set_ptr->percentage_of_edge_in_light_background = (uint8_t)(count_of_edge_blocks * 100 /
                                                                                sb_total_count);
}
...
void eb_vp9_derive_high_dark_area_density_flag(SequenceControlSet      *sequence_control_set_ptr,
    ...
    for (region_in_picture_width_index = 0;
        ...
        for (region_in_picture_height_index = 0;
            ...
            for (luma_histogram_bin = 0; luma_histogram_bin < LOW_MEAN_TH_0;
                 luma_histogram_bin++) { // loop over the 1st LOW_MEAN_THLD bins
                black_samples_count +=
                    picture_control_set_ptr->picture_histogram[region_in_picture_width_index]
                                                              [region_in_picture_height_index][0][luma_histogram_bin];
            }
        }
    }
    ...
    for (region_in_picture_width_index = 0;
        ...
        for (region_in_picture_height_index = 0;
             region_in_picture_height_index < sequence_control_set_ptr->picture_analysis_number_of_regions_per_height;
             region_in_picture_height_index++) { // loop over vertical regions
            for (luma_histogram_bin = 0; luma_histogram_bin < LOW_MEAN_TH_1;
                 luma_histogram_bin++) { // loop over the 1st LOW_MEAN_THLD bins
                black_samples_count +=
                    picture_control_set_ptr->picture_histogram[region_in_picture_width_index]
                                                              [region_in_picture_height_index][0][luma_histogram_bin];
            }
            for (luma_histogram_bin = HIGH_MEAN_TH; luma_histogram_bin < HISTOGRAM_NUMBER_OF_BINS;
                 luma_histogram_bin++) {
                white_samples_count +=
                    picture_control_set_ptr->picture_histogram[region_in_picture_width_index]
                                                              [region_in_picture_height_index][0][luma_histogram_bin];
    ...
    black_area_percentage = (black_samples_count * 100) /
        (sequence_control_set_ptr->luma_width * sequence_control_set_ptr->luma_height);
    white_area_percentage = (white_samples_count * 100) /
        (sequence_control_set_ptr->luma_width * sequence_control_set_ptr->luma_height);
...
void temporal_high_contrast_classifier(SourceBasedOperationsContext *context_ptr,
    ...
    if (picture_control_set_ptr->slice_type == B_SLICE) {
        for (blk_it = 0; blk_it < 4; blk_it++) {
            nsad = ((uint32_t)picture_control_set_ptr->me_results[sb_index][1 + blk_it]
...
void populate_from_current_sb_to_neighbor_sbs(PictureParentControlSet *picture_control_set_ptr,
    ...
    if ((sb_origin_x < picture_control_set_ptr->enhanced_picture_ptr->width - MAX_SB_SIZE) &&
        (sb_origin_y >= MAX_SB_SIZE)) {
        output_buffer[sb_adrr - picture_width_in_sbs + 1] = input_to_populate;
    }

    // Copy to bottom-left LCU if present
    if ((sb_origin_x >= MAX_SB_SIZE) &&
        (sb_origin_y < picture_control_set_ptr->enhanced_picture_ptr->height - MAX_SB_SIZE)) {
        output_buffer[sb_adrr + picture_width_in_sbs - 1] = input_to_populate;
    }
...
EB_BOOL is_spatially_complex_area(
    ...
    if (sb_origin_y != 0) {
        available_sbs_count++;
        if ((picture_control_set_ptr->variance[sb_adrr - picture_width_in_sbs][ME_TIER_ZERO_PU_64x64]) >
    ...
    if ((sb_origin_x >= MAX_SB_SIZE) && (sb_origin_y >= MAX_SB_SIZE)) {
        available_sbs_count++;
        if ((picture_control_set_ptr->variance[sb_adrr - picture_width_in_sbs - 1][ME_TIER_ZERO_PU_64x64]) >
            IS_COMPLEX_SB_VARIANCE_TH) {
            high_variance_sbs_count++;
        }
    }
...
void eb_vp9_derive_blockiness_present_flag(SequenceControlSet      *sequence_control_set_ptr,
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
        ...
        if (is_spatially_complex_area(
                picture_control_set_ptr, sb_index, sb_params_ptr->origin_x, sb_params_ptr->origin_y) &&
            ...
            if (picture_control_set_ptr->non_moving_index_array[sb_index] == SB_COMPLEXITY_NON_MOVING_INDEX_TH_0 &&
                picture_control_set_ptr->non_moving_average_score >= SB_COMPLEXITY_NON_MOVING_INDEX_TH_1 &&
            ...
            else if (picture_control_set_ptr->non_moving_index_array[sb_index] == SB_COMPLEXITY_NON_MOVING_INDEX_TH_0 &&
                     picture_control_set_ptr->non_moving_average_score >= SB_COMPLEXITY_NON_MOVING_INDEX_TH_2 &&
                     picture_control_set_ptr->non_moving_average_score < SB_COMPLEXITY_NON_MOVING_INDEX_TH_1) {
...
void eb_vp9_derive_min_max_me_distortion(SequenceControlSet      *sequence_control_set_ptr,
    ...
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; sb_index++) {
        ...
        if (sb_params->pa_raster_scan_block_validity[PA_RASTER_SCAN_CU_INDEX_64x64]) {
            me_distortion = picture_control_set_ptr->me_results[sb_index][PA_RASTER_SCAN_CU_INDEX_64x64]
                                .distortion_direction[0]
                                .distortion;
            picture_control_set_ptr->min_me_distortion = me_distortion < picture_control_set_ptr->min_me_distortion
                ? me_distortion
                : picture_control_set_ptr->min_me_distortion;
            picture_control_set_ptr->max_me_distortion = me_distortion > picture_control_set_ptr->max_me_distortion
                ? me_distortion
                : picture_control_set_ptr->max_me_distortion;
        }
    }
}

static void stationary_edge_over_update_over_time_sb(SequenceControlSet      *sequence_control_set_ptr,
    ...
    for (sb_index = 0; sb_index < total_sb_count; sb_index++) {
        ...
        if (sb_params->potential_logo_sb && sb_params->is_complete_sb &&
            ...
            for (raster_scan_block_index = PA_RASTER_SCAN_CU_INDEX_16x16_0;
                 raster_scan_block_index <= PA_RASTER_SCAN_CU_INDEX_16x16_15;
                 raster_scan_block_index++) {
                similar_edge_count_sb += (sb_stat_ptr->cu_stat_array[raster_scan_block_index].similar_edge_count >
                                          slide_window_th)
                    ? 1
                    : 0;
            }
        ...
        if (sb_params->potential_logo_sb && sb_params->is_complete_sb &&
            ...
            for (raster_scan_block_index = PA_RASTER_SCAN_CU_INDEX_16x16_0;
                 raster_scan_block_index <= PA_RASTER_SCAN_CU_INDEX_16x16_15;
                 raster_scan_block_index++) {
                similar_edge_count_sb += (sb_stat_ptr->cu_stat_array[raster_scan_block_index].pm_similar_edge_count >
                                          slide_window_th)
                    ? 1
                    : 0;
            }
                ...
                sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag) {
                {
                    ...
                    for (sb_ver = sb_ver_s; sb_ver <= sb_ver_e; sb_ver++) {
                        ...
                        for (sb_hor = sb_hor_s; sb_hor <= sb_hor_e; sb_hor++) {
                            count_of_neighbors += (picture_control_set_ptr
                                                       ->sb_stat_array[sb_index + sb_ver_offset + sb_hor]
            ...
            if (sb_params->potential_logo_sb && sb_params->is_complete_sb &&
                ...
                sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag) {
                {
                    ...
                    for (sb_ver = sb_ver_s; sb_ver <= sb_ver_e; sb_ver++) {
                        sb_ver_offset = sb_ver * (int32_t)picture_width_in_sb;
                        for (sb_hor = sb_hor_s; sb_hor <= sb_hor_e; sb_hor++) {
                            count_of_neighbors_pm += (picture_control_set_ptr
                                                          ->sb_stat_array[sb_index + sb_ver_offset + sb_hor]
                                                          .pm_stationary_edge_over_time_flag == 1);
                        }
                    }
...

    {
        ...
        for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
            ...
            {
                if (sb_stat_ptr->stationary_edge_over_time_flag == 0 && sb_params->potential_logo_sb &&
                    (sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag || !sb_params->is_complete_sb)) {
                    sb_hor_s           = (sb_x > 0) ? -1 : 0;
                    sb_hor_e           = (sb_x < picture_width_in_sb - 1) ? 1 : 0;
                    sb_ver_s           = (sb_y > 0) ? -1 : 0;
                    sb_ver_e           = (sb_y < picture_height_in_sb - 1) ? 1 : 0;
...

    {
        ...
        for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count; ++sb_index) {
            ...
            {
                if (sb_stat_ptr->pm_stationary_edge_over_time_flag == 0 && sb_params->potential_logo_sb &&
                    (sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag || !sb_params->is_complete_sb)) {
                    sb_hor_s           = (sb_x > 0) ? -1 : 0;
                    sb_hor_e           = (sb_x < picture_width_in_sb - 1) ? 1 : 0;
                    sb_ver_s           = (sb_y > 0) ? -1 : 0;
                ...
                if (sb_stat_ptr->pm_stationary_edge_over_time_flag == 0 && sb_params->potential_logo_sb &&
                    ...
                    for (sb_ver = sb_ver_s; sb_ver <= sb_ver_e; sb_ver++) {
                        ...
                        for (sb_hor = sb_hor_s; sb_hor <= sb_hor_e; sb_hor++) {
                            count_of_neighbors += (picture_control_set_ptr
                                                       ->sb_stat_array[sb_index + sb_ver_offset + sb_hor]
                                                       .pm_stationary_edge_over_time_flag == 2);
                        }
                    }
                    if (count_of_neighbors > 3) {
...
void *eb_vp9_source_based_operations_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->initial_rate_control_results_input_fifo_ptr, &input_results_wrapper_ptr);
        ...
        for (sb_index = 0; sb_index < sb_total_count; ++sb_index) {
            ...
            grass_skin_sb(context_ptr, sequence_control_set_ptr, picture_control_set_ptr, sb_index);

            // Spatial high contrast classifier
            if (is_complete_sb) {
                spatial_high_contrast_classifier(context_ptr, picture_control_set_ptr, sb_index);
            }

            // Temporal high contrast classifier
            if (is_complete_sb) {
                temporal_high_contrast_classifier(context_ptr, picture_control_set_ptr, sb_index);

                if (context_ptr->high_contrast_num && context_ptr->high_dist) {
                    populate_from_current_sb_to_neighbor_sbs(picture_control_set_ptr,
        ...
        if (!picture_control_set_ptr->end_of_sequence_flag && sequence_control_set_ptr->look_ahead_distance != 0) {
            stationary_edge_over_update_over_time_sb(
                ...
                MIN(MIN((picture_control_set_ptr->pred_struct_ptr->pred_struct_period << 1) + 1,
        ...
        eb_vp9_get_empty_object(context_ptr->picture_demux_results_output_fifo_ptr, &output_results_wrapper_ptr);
