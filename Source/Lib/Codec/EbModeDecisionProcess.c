...
void eb_vp9_reset_mode_decision_neighbor_arrays(PictureControlSet *picture_control_set_ptr) {
    ...
    for (neighbor_array_count = 0; neighbor_array_count < NEIGHBOR_ARRAY_TOTAL_COUNT; neighbor_array_count++) {
        ...
        memset(picture_control_set_ptr->md_above_context[neighbor_array_count],
               0,
               sizeof(*picture_control_set_ptr->md_above_context[neighbor_array_count]) * MAX_MB_PLANE * 2 *
                   mi_cols_aligned_to_sb(cm->mi_cols));
        memset(picture_control_set_ptr->md_above_seg_context[neighbor_array_count],
               0,
               sizeof(*picture_control_set_ptr->md_above_seg_context[neighbor_array_count]) *
                   mi_cols_aligned_to_sb(cm->mi_cols));

        memset(picture_control_set_ptr->md_left_context[neighbor_array_count],
               0,
               sizeof(*picture_control_set_ptr->md_left_context[neighbor_array_count]) * MAX_MB_PLANE * 2 *
                   mi_cols_aligned_to_sb(cm->mi_rows));
        memset(picture_control_set_ptr->md_left_seg_context[neighbor_array_count],
               0,
               sizeof(*picture_control_set_ptr->md_left_seg_context[neighbor_array_count]) *
                   mi_cols_aligned_to_sb(cm->mi_rows));
    }
...
void eb_vp9_reset_mode_decision(EncDecContext *context_ptr, PictureControlSet *picture_control_set_ptr,
    ...
    // 32x32 CU
    context_ptr->buffer_depth_index_start[1] = context_ptr->buffer_depth_index_start[0] +
        context_ptr->buffer_depth_index_width[0];
    context_ptr->buffer_depth_index_width[1] = MAX_NFL + 1; // MAX_NFL NFL + 1 for temporary data

    // 16x16 CU
    context_ptr->buffer_depth_index_start[2] = context_ptr->buffer_depth_index_start[1] +
        context_ptr->buffer_depth_index_width[1];
    context_ptr->buffer_depth_index_width[2] = MAX_NFL + 1; // MAX_NFL NFL + 1 for temporary data

    // 8x8 CU
    context_ptr->buffer_depth_index_start[3] = context_ptr->buffer_depth_index_start[2] +
        context_ptr->buffer_depth_index_width[2];
    context_ptr->buffer_depth_index_width[3] = MAX_NFL + 1; // MAX_NFL NFL + 1 for temporary data

    // 4x4 CU
    context_ptr->buffer_depth_index_start[4] = context_ptr->buffer_depth_index_start[3] +
        context_ptr->buffer_depth_index_width[3];
