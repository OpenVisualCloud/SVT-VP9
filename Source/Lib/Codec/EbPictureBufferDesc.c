 ...
 *  the descriptor.
 *****************************************/
EbErrorType eb_vp9_picture_buffer_desc_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    ...
    *object_dbl_ptr = (EbPtr)picture_buffer_desc_ptr;
    ...
    if (picture_buffer_desc_init_data_ptr->buffer_enable_mask & PICTURE_BUFFER_DESC_Y_FLAG) {
        EB_ALLIGN_MALLOC(EbByte,
                         picture_buffer_desc_ptr->buffer_y,
    ...
    if (picture_buffer_desc_init_data_ptr->buffer_enable_mask & PICTURE_BUFFER_DESC_Cb_FLAG) {
        EB_ALLIGN_MALLOC(EbByte,
                         picture_buffer_desc_ptr->buffer_cb,
    ...
    if (picture_buffer_desc_init_data_ptr->buffer_enable_mask & PICTURE_BUFFER_DESC_Cr_FLAG) {
        EB_ALLIGN_MALLOC(EbByte,
                         picture_buffer_desc_ptr->buffer_cr,
                         picture_buffer_desc_ptr->chroma_size * bytesPerPixel * sizeof(uint8_t),
                         EB_A_PTR);
 ...
 *  the descriptor.
 *****************************************/
EbErrorType eb_vp9_recon_picture_buffer_desc_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    EbPictureBufferDesc         *picture_buffer_desc_ptr;
    EbPictureBufferDescInitData *picture_buffer_desc_init_data_ptr = (EbPictureBufferDescInitData *)
        object_init_data_ptr;
    ...
    *object_dbl_ptr = (EbPtr)picture_buffer_desc_ptr;
    ...
    if (picture_buffer_desc_init_data_ptr->buffer_enable_mask & PICTURE_BUFFER_DESC_Y_FLAG) {
        EB_CALLOC(EbByte,
                  picture_buffer_desc_ptr->buffer_y,
                  picture_buffer_desc_ptr->luma_size * bytesPerPixel * sizeof(uint8_t) +
                      (picture_buffer_desc_ptr->width + 1) * 2 * bytesPerPixel * sizeof(uint8_t),
                  1,
                  EB_N_PTR);
    ...
    if (picture_buffer_desc_init_data_ptr->buffer_enable_mask & PICTURE_BUFFER_DESC_Cb_FLAG) {
        EB_CALLOC(EbByte,
                  picture_buffer_desc_ptr->buffer_cb,
                  picture_buffer_desc_ptr->chroma_size * bytesPerPixel * sizeof(uint8_t) +
                      ((picture_buffer_desc_ptr->width >> 1) + 1) * 2 * bytesPerPixel * sizeof(uint8_t),
                  1,
                  EB_N_PTR);
