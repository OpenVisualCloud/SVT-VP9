...
EbErrorType eb_vp9_trans_quant_buffers_ctor(EbTransQuantBuffers *trans_quant_buffers_ptr) {
    ...
    return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&(trans_quant_buffers_ptr->tu_trans_coeff_nx_n_ptr),
                                                   (EbPtr)&trans_coeff_init_array);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }

    return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&(trans_quant_buffers_ptr->tu_trans_coeff_n2x_n2_ptr),
                                                   (EbPtr)&trans_coeff_init_array);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&(trans_quant_buffers_ptr->tu_quant_coeff_nx_n_ptr),
                                                   (EbPtr)&trans_coeff_init_array);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }

    return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&(trans_quant_buffers_ptr->tu_quant_coeff_n2x_n2_ptr),
                                                   (EbPtr)&trans_coeff_init_array);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
