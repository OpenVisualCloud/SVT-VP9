...
#define EB_INPUTVIDEOBUFFERSIZE \
    0x10000 //   832*480*3//      // Input Slice Size , must me a multiple of 2 in case of 10 bit video.
...
#define EB_OUTPUTSTREAMBUFFERSIZE_MACRO(ResolutionSize)        \
    ((ResolutionSize) < (INPUT_SIZE_1080i_TH)       ? 0x1E8480 \
         : (ResolutionSize) < (INPUT_SIZE_1080p_TH) ? 0x2DC6C0 \
         : (ResolutionSize) < (INPUT_SIZE_4K_TH)    ? 0x2DC6C0 \
                                                    : 0x2DC6C0)

static uint64_t max_luma_picture_size[TOTAL_LEVEL_COUNT] = {36864U,
                                                            122880U,
                                                            245760U,
                                                            552960U,
                                                            983040U,
                                                            ...
                                                            35651584U,
                                                            35651584U};
static uint64_t max_luma_sample_rate[TOTAL_LEVEL_COUNT]  = {552960U,
                                                            3686400U,
                                                            7372800U,
                                                            16588800U,
                                                            33177600U,
...
static uint32_t get_cpu_asm_type() {
    ...
    else if (can_use_intel_core4th_gen_features() == 1)
...
// Rate Control
static RateControlPorts rate_control_ports[] = {{RATE_CONTROL_INPUT_PORT_PICTURE_MANAGER, 0},
...
static EncDecPorts enc_dec_ports[] = {
    {ENCDEC_INPUT_PORT_MDC, 0}, {ENCDEC_INPUT_PORT_ENCDEC, 0}, {ENCDEC_INPUT_PORT_INVALID, 0}};
...
static EbErrorType init_thread_managment_params() {
    ...
    int         processor_id_len = strlen(PROCESSORID);
    int         physical_id_len  = strlen(PHYSICALID);
...
static void lib_svt_vp9_encoder_send_error_exit(EbPtr h_component, uint32_t error_code) {
    ...
    eb_vp9_get_empty_object((p_enc_comp_data->output_stream_buffer_producer_fifo_ptr_dbl_array[0])[0], &eb_wrapper_ptr);
 ...
 **********************************/
static EbErrorType eb_enc_handle_ctor(EbEncHandle **enc_handle_dbl_ptr, EbHandleType eb_handle_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    // Allocate Memory
    EbEncHandle *enc_handle_ptr = (EbEncHandle *)malloc(sizeof(EbEncHandle));
    *enc_handle_dbl_ptr         = enc_handle_ptr;
    if (enc_handle_ptr == (EbEncHandle *)NULL) {
        return EB_ErrorInsufficientResources;
    }
    ...
    if (memory_map == (EbMemoryMapEntry *)NULL) {
        return EB_ErrorInsufficientResources;
    }
    ...
    EB_MALLOC(uint32_t *,
              ...
              sizeof(uint32_t) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    ...
    enc_handle_ptr->sequence_control_set_pool_ptr                     = (EbSystemResource *)NULL;
    enc_handle_ptr->sequence_control_set_pool_producer_fifo_ptr_array = (EbFifo **)NULL;

    // Picture Buffers
    enc_handle_ptr->reference_picture_pool_ptr_array    = (EbSystemResource **)NULL;
    enc_handle_ptr->pa_reference_picture_pool_ptr_array = (EbSystemResource **)NULL;

    // Picture Buffer Producer Fifos
    enc_handle_ptr->reference_picture_pool_producer_fifo_ptr_dbl_array    = (EbFifo ***)NULL;
    enc_handle_ptr->pa_reference_picture_pool_producer_fifo_ptr_dbl_array = (EbFifo ***)NULL;

    // Threads
    enc_handle_ptr->resource_coordination_thread_handle             = (EbHandle)NULL;
    enc_handle_ptr->picture_analysis_thread_handle_array            = (EbHandle *)NULL;
    enc_handle_ptr->picture_decision_thread_handle                  = (EbHandle)NULL;
    enc_handle_ptr->motion_estimation_thread_handle_array           = (EbHandle *)NULL;
    enc_handle_ptr->initial_rate_control_thread_handle              = (EbHandle)NULL;
    enc_handle_ptr->source_based_operations_thread_handle_array     = (EbHandle *)NULL;
    enc_handle_ptr->picture_manager_thread_handle                   = (EbHandle)NULL;
    enc_handle_ptr->rate_control_thread_handle                      = (EbHandle)NULL;
    enc_handle_ptr->mode_decision_configuration_thread_handle_array = (EbHandle *)NULL;
    enc_handle_ptr->enc_dec_thread_handle_array                     = (EbHandle *)NULL;
    enc_handle_ptr->entropy_coding_thread_handle_array              = (EbHandle *)NULL;
    enc_handle_ptr->packetization_thread_handle                     = (EbHandle)NULL;

    // Contexts
    enc_handle_ptr->resource_coordination_context_ptr             = (EbPtr)NULL;
    enc_handle_ptr->picture_analysis_context_ptr_array            = (EbPtr *)NULL;
    enc_handle_ptr->picture_decision_context_ptr                  = (EbPtr)NULL;
    enc_handle_ptr->motion_estimation_context_ptr_array           = (EbPtr *)NULL;
    enc_handle_ptr->initial_rate_control_context_ptr              = (EbPtr)NULL;
    enc_handle_ptr->source_based_operations_context_ptr_array     = (EbPtr *)NULL;
    enc_handle_ptr->picture_manager_context_ptr                   = (EbPtr)NULL;
    enc_handle_ptr->rate_control_context_ptr                      = (EbPtr)NULL;
    enc_handle_ptr->mode_decision_configuration_context_ptr_array = (EbPtr *)NULL;
    enc_handle_ptr->enc_dec_context_ptr_array                     = (EbPtr *)NULL;
    enc_handle_ptr->entropy_coding_context_ptr_array              = (EbPtr *)NULL;
    enc_handle_ptr->packetization_context_ptr                     = (EbPtr)NULL;

    // System Resource Managers
    enc_handle_ptr->input_buffer_resource_ptr                  = (EbSystemResource *)NULL;
    enc_handle_ptr->output_stream_buffer_resource_ptr_array    = (EbSystemResource **)NULL;
    enc_handle_ptr->resource_coordination_results_resource_ptr = (EbSystemResource *)NULL;
    enc_handle_ptr->picture_analysis_results_resource_ptr      = (EbSystemResource *)NULL;
    enc_handle_ptr->picture_decision_results_resource_ptr      = (EbSystemResource *)NULL;
    enc_handle_ptr->motion_estimation_results_resource_ptr     = (EbSystemResource *)NULL;
    enc_handle_ptr->initial_rate_control_results_resource_ptr  = (EbSystemResource *)NULL;
    enc_handle_ptr->picture_demux_results_resource_ptr         = (EbSystemResource *)NULL;
    enc_handle_ptr->rate_control_tasks_resource_ptr            = (EbSystemResource *)NULL;
    enc_handle_ptr->rate_control_results_resource_ptr          = (EbSystemResource *)NULL;
    enc_handle_ptr->enc_dec_tasks_resource_ptr                 = (EbSystemResource *)NULL;
    enc_handle_ptr->enc_dec_results_resource_ptr               = (EbSystemResource *)NULL;
    enc_handle_ptr->entropy_coding_results_resource_ptr        = (EbSystemResource *)NULL;
    enc_handle_ptr->output_recon_buffer_resource_ptr_array     = (EbSystemResource **)NULL;

    // Inter-Process Producer Fifos
    enc_handle_ptr->input_buffer_producer_fifo_ptr_array                  = (EbFifo **)NULL;
    enc_handle_ptr->output_stream_buffer_producer_fifo_ptr_dbl_array      = (EbFifo ***)NULL;
    enc_handle_ptr->resource_coordination_results_producer_fifo_ptr_array = (EbFifo **)NULL;
    enc_handle_ptr->picture_demux_results_producer_fifo_ptr_array         = (EbFifo **)NULL;
    enc_handle_ptr->picture_manager_results_producer_fifo_ptr_array       = (EbFifo **)NULL;
    enc_handle_ptr->rate_control_tasks_producer_fifo_ptr_array            = (EbFifo **)NULL;
    enc_handle_ptr->rate_control_results_producer_fifo_ptr_array          = (EbFifo **)NULL;
    enc_handle_ptr->enc_dec_tasks_producer_fifo_ptr_array                 = (EbFifo **)NULL;
    enc_handle_ptr->enc_dec_results_producer_fifo_ptr_array               = (EbFifo **)NULL;
    enc_handle_ptr->entropy_coding_results_producer_fifo_ptr_array        = (EbFifo **)NULL;
    enc_handle_ptr->output_recon_buffer_producer_fifo_ptr_dbl_array       = (EbFifo ***)NULL;

    // Inter-Process Consumer Fifos
    enc_handle_ptr->input_buffer_consumer_fifo_ptr_array                  = (EbFifo **)NULL;
    enc_handle_ptr->output_stream_buffer_consumer_fifo_ptr_dbl_array      = (EbFifo ***)NULL;
    enc_handle_ptr->resource_coordination_results_consumer_fifo_ptr_array = (EbFifo **)NULL;
    enc_handle_ptr->picture_demux_results_consumer_fifo_ptr_array         = (EbFifo **)NULL;
    enc_handle_ptr->rate_control_tasks_consumer_fifo_ptr_array            = (EbFifo **)NULL;
    enc_handle_ptr->rate_control_results_consumer_fifo_ptr_array          = (EbFifo **)NULL;
    enc_handle_ptr->enc_dec_tasks_consumer_fifo_ptr_array                 = (EbFifo **)NULL;
    enc_handle_ptr->enc_dec_results_consumer_fifo_ptr_array               = (EbFifo **)NULL;
    enc_handle_ptr->entropy_coding_results_consumer_fifo_ptr_array        = (EbFifo **)NULL;
    enc_handle_ptr->output_recon_buffer_consumer_fifo_ptr_dbl_array       = (EbFifo ***)NULL;

    // Initialize Callbacks
    EB_MALLOC(EbCallback **,
              enc_handle_ptr->app_callback_ptr_array,
              sizeof(EbCallback *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    ...
    enc_handle_ptr->app_callback_ptr_array[0]->app_private_data = NULL;
    enc_handle_ptr->app_callback_ptr_array[0]->handle           = eb_handle_ptr;

    // Initialize Sequence Control Set Instance Array
    EB_MALLOC(EbSequenceControlSetInstance **,
              enc_handle_ptr->sequence_control_set_instance_array,
              sizeof(EbSequenceControlSetInstance *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
...
static uint64_t get_affinity_mask(uint32_t lpnum) {
    uint64_t mask = 0x1;
    for (uint32_t i = lpnum - 1; i > 0; i--) mask += (uint64_t)1 << i;
    return mask;
}
#endif
...
static EbErrorType lib_allocate_frame_buffer(SequenceControlSet *sequence_control_set_ptr,
    ...
    return_error = eb_vp9_picture_buffer_desc_ctor((EbPtr *)&(input_buffer->p_buffer),
...
static EbErrorType input_buffer_header_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    ...
    *object_dbl_ptr = (EbPtr)input_buffer;
    ...
    lib_allocate_frame_buffer(sequence_control_set_ptr, input_buffer);
...
EB_API EbErrorType eb_vp9_init_encoder(EbComponentType *svt_enc_component) {
    ...
    ************************************/
     ...
     ************************************/
    return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->sequence_control_set_pool_ptr,
     ...
     ************************************/
    EB_MALLOC(EbSystemResource **,
              enc_handle_ptr->picture_parent_control_set_pool_ptr_array,
              sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->picture_parent_control_set_pool_producer_fifo_ptr_dbl_array,
              sizeof(EbSystemResource **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
     ...
     ************************************/
    EB_MALLOC(EbSystemResource **,
              enc_handle_ptr->picture_control_set_pool_ptr_array,
              sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->picture_control_set_pool_producer_fifo_ptr_dbl_array,
              sizeof(EbSystemResource **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    ...
    // The segment Width & Height Arrays are in units of LCUs, not samples
    {
        ...
        for (i = 0; i <= scs_ptr->hierarchical_levels; ++i) {
            input_data.enc_dec_segment_col = scs_ptr->enc_dec_segment_col_count_array[i] >
                    input_data.enc_dec_segment_col
                ? (uint16_t)scs_ptr->enc_dec_segment_col_count_array[i]
                : input_data.enc_dec_segment_col;
            input_data.enc_dec_segment_row = scs_ptr->enc_dec_segment_row_count_array[i] >
    ...
    EB_MALLOC(EbSystemResource **,
              enc_handle_ptr->reference_picture_pool_ptr_array,
              sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbSystemResource **,
              enc_handle_ptr->pa_reference_picture_pool_ptr_array,
              sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);

    // Allocate Producer Fifo Arrays
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->reference_picture_pool_producer_fifo_ptr_dbl_array,
              sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->pa_reference_picture_pool_producer_fifo_ptr_dbl_array,
              sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    ...
    eb_reference_object_desc_init_data_structure.reference_picture_desc_init_data =
    ...
    eb_pa_reference_object_desc_init_data_structure.reference_picture_desc_init_data =
        reference_picture_buffer_desc_init_data;
    eb_pa_reference_object_desc_init_data_structure.quarter_picture_desc_init_data =
        quarter_decim_picture_buffer_desc_init_data;
    eb_pa_reference_object_desc_init_data_structure.sixteenth_picture_desc_init_data =
        sixteenth_decim_picture_buffer_desc_init_data;
    ...
    scs_ptr->encode_context_ptr->reference_picture_pool_fifo_ptr =
        (enc_handle_ptr->reference_picture_pool_producer_fifo_ptr_dbl_array[0])[0];
    scs_ptr->encode_context_ptr->pa_reference_picture_pool_fifo_ptr =
        (enc_handle_ptr->pa_reference_picture_pool_producer_fifo_ptr_dbl_array[0])[0];
    ...
    return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->input_buffer_resource_ptr,
    ...
    EB_MALLOC(EbSystemResource **,
              enc_handle_ptr->output_stream_buffer_resource_ptr_array,
              sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->output_stream_buffer_producer_fifo_ptr_dbl_array,
              sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);
    EB_MALLOC(EbFifo ***,
              enc_handle_ptr->output_stream_buffer_consumer_fifo_ptr_dbl_array,
              sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
              EB_N_PTR);

    return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->output_stream_buffer_resource_ptr_array[0],
    ...
    if (scs_ptr->static_config.recon_file) {
        ...
        EB_MALLOC(EbSystemResource **,
                  enc_handle_ptr->output_recon_buffer_resource_ptr_array,
                  sizeof(EbSystemResource *) * enc_handle_ptr->encode_instance_total_count,
                  EB_N_PTR);
        EB_MALLOC(EbFifo ***,
                  enc_handle_ptr->output_recon_buffer_producer_fifo_ptr_dbl_array,
                  sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
                  EB_N_PTR);
        EB_MALLOC(EbFifo ***,
                  enc_handle_ptr->output_recon_buffer_consumer_fifo_ptr_dbl_array,
                  sizeof(EbFifo **) * enc_handle_ptr->encode_instance_total_count,
                  EB_N_PTR);

        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->output_recon_buffer_resource_ptr_array[0],
    ...
    // Picture Analysis Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->picture_analysis_results_resource_ptr,
    ...
    // Picture Decision Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->picture_decision_results_resource_ptr,
    ...
    // Motion Estimation Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->motion_estimation_results_resource_ptr,
    ...
    // Rate Control Tasks
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->rate_control_tasks_resource_ptr,
    ...
    // Rate Control Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->rate_control_results_resource_ptr,
    ...
    // EncDec Tasks
    {
        ...
        for (i = 0; i <= scs_ptr->hierarchical_levels; ++i) {
            mode_decision_result_init_data.enc_dec_segment_row_count = MAX(
                mode_decision_result_init_data.enc_dec_segment_row_count, scs_ptr->enc_dec_segment_row_count_array[i]);
        }

        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->enc_dec_tasks_resource_ptr,
    ...
    // EncDec Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->enc_dec_results_resource_ptr,
    ...
    // Entropy Coding Results
    {
        ...
        return_error = eb_vp9_system_resource_ctor(&enc_handle_ptr->entropy_coding_results_resource_ptr,
    ...
    scs_ptr->encode_context_ptr->stream_output_fifo_ptr =
        (enc_handle_ptr->output_stream_buffer_producer_fifo_ptr_dbl_array[0])[0];
    if (scs_ptr->static_config.recon_file)
        scs_ptr->encode_context_ptr->recon_output_fifo_ptr =
            (enc_handle_ptr->output_recon_buffer_producer_fifo_ptr_dbl_array[0])[0];
    /************************************
     * Contexts
     ************************************/
    ...
    return_error = eb_vp9_resource_coordination_context_ctor(
        ...
        enc_handle_ptr->picture_parent_control_set_pool_producer_fifo_ptr_dbl_array
            [0], //ResourceCoordination works with ParentPCS
        enc_handle_ptr->sequence_control_set_instance_array,
    ...
    EB_MALLOC(EbPtr *,
              ...
              sizeof(EbPtr) * scs_ptr->picture_analysis_process_init_count,
              EB_N_PTR);
    ...
    EB_MALLOC(EbPtr *,
              ...
              sizeof(EbPtr) * scs_ptr->motion_estimation_process_init_count,
              EB_N_PTR);
    ...
    EB_MALLOC(EbPtr *,
              ...
              sizeof(EbPtr) * scs_ptr->source_based_operations_process_init_count,
              EB_N_PTR);
    ...
    return_error = eb_vp9_picture_manager_context_ctor(
        ...
        enc_handle_ptr->rate_control_tasks_producer_fifo_ptr_array[rate_control_port_lookup(
    ...
    return_error = eb_vp9_rate_control_context_ctor((RateControlContext **)&enc_handle_ptr->rate_control_context_ptr,
    ...
    // Mode Decision Configuration Contexts
    {
        ...
        EB_MALLOC(EbPtr *,
                  ...
                  sizeof(EbPtr) * scs_ptr->mode_decision_configuration_process_init_count,
                  EB_N_PTR);

        for (process_index = 0; process_index < scs_ptr->mode_decision_configuration_process_init_count;
             ++process_index) {
            return_error = eb_vp9_mode_decision_configuration_context_ctor(
                (ModeDecisionConfigurationContext **)&enc_handle_ptr
                    ->mode_decision_configuration_context_ptr_array[process_index],
                enc_handle_ptr->rate_control_results_consumer_fifo_ptr_array[process_index],

                enc_handle_ptr
                    ->enc_dec_tasks_producer_fifo_ptr_array[enc_dec_port_lookup(ENCDEC_INPUT_PORT_MDC, process_index)],
                ((scs_ptr->max_input_luma_width + MAX_SB_SIZE_MINUS_1) / MAX_SB_SIZE) *
                    ((scs_ptr->max_input_luma_height + MAX_SB_SIZE_MINUS_1) / MAX_SB_SIZE));
    ...
    EB_MALLOC(EbPtr *,
              ...
              sizeof(EbPtr) * scs_ptr->enc_dec_process_init_count,
              EB_N_PTR);
    ...
    for (process_index = 0; process_index < scs_ptr->enc_dec_process_init_count; ++process_index) {
        return_error = eb_vp9_enc_dec_context_ctor(
            ...
            enc_handle_ptr
                ->enc_dec_tasks_producer_fifo_ptr_array[enc_dec_port_lookup(ENCDEC_INPUT_PORT_ENCDEC, process_index)],
    ...
    EB_MALLOC(EbPtr *,
              ...
              sizeof(EbPtr) * scs_ptr->entropy_coding_process_init_count,
              EB_N_PTR);
    ...
    for (process_index = 0; process_index < scs_ptr->entropy_coding_process_init_count; ++process_index) {
        return_error = eb_vp9_entropy_coding_context_ctor(
            ...
            enc_handle_ptr->rate_control_tasks_producer_fifo_ptr_array[rate_control_port_lookup(
                RATE_CONTROL_INPUT_PORT_ENTROPY_CODING, process_index)],
            is16bit);
        if (return_error == EB_ErrorInsufficientResources) {
            return EB_ErrorInsufficientResources;
        }
    ...
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->resource_coordination_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_resource_coordination_kernel,
                    enc_handle_ptr->resource_coordination_context_ptr);

    // Picture Analysis
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->picture_analysis_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->picture_analysis_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->picture_analysis_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->picture_analysis_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_picture_analysis_kernel,
                        enc_handle_ptr->picture_analysis_context_ptr_array[process_index]);
    }

    // Picture Decision
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->picture_decision_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_picture_decision_kernel,
                    enc_handle_ptr->picture_decision_context_ptr);

    // Motion Estimation
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->motion_estimation_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->motion_estimation_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->motion_estimation_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->motion_estimation_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_motion_estimation_kernel,
                        enc_handle_ptr->motion_estimation_context_ptr_array[process_index]);
    }

    // Initial Rate Control
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->initial_rate_control_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_initial_eb_vp9_rate_control_kernel,
                    enc_handle_ptr->initial_rate_control_context_ptr);

    // Source Based Oprations
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->source_based_operations_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->source_based_operations_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->source_based_operations_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->source_based_operations_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_source_based_operations_kernel,
                        enc_handle_ptr->source_based_operations_context_ptr_array[process_index]);
    }

    // Picture Manager
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->picture_manager_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_PictureManagerKernel,
                    enc_handle_ptr->picture_manager_context_ptr);

    // Rate Control
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->rate_control_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_rate_control_kernel,
                    enc_handle_ptr->rate_control_context_ptr);

    // Mode Decision Configuration Process
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->mode_decision_configuration_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->mode_decision_configuration_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->mode_decision_configuration_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->mode_decision_configuration_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_mode_decision_configuration_kernel,
                        enc_handle_ptr->mode_decision_configuration_context_ptr_array[process_index]);
    }

    // EncDec Process
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->enc_dec_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->enc_dec_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->enc_dec_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->enc_dec_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_enc_dec_kernel,
                        enc_handle_ptr->enc_dec_context_ptr_array[process_index]);
    }

    // Entropy Coding Process
    EB_MALLOC(EbHandle *,
              enc_handle_ptr->entropy_coding_thread_handle_array,
              sizeof(EbHandle) * scs_ptr->entropy_coding_process_init_count,
              EB_N_PTR);

    for (process_index = 0; process_index < scs_ptr->entropy_coding_process_init_count; ++process_index) {
        EB_CREATETHREAD(EbHandle,
                        enc_handle_ptr->entropy_coding_thread_handle_array[process_index],
                        sizeof(EbHandle),
                        EB_THREAD,
                        eb_vp9_entropy_coding_kernel,
                        enc_handle_ptr->entropy_coding_context_ptr_array[process_index]);
    }

    // Packetization
    EB_CREATETHREAD(EbHandle,
                    enc_handle_ptr->packetization_thread_handle,
                    sizeof(EbHandle),
                    EB_THREAD,
                    eb_vp9_packetization_kernel,
...
EB_API EbErrorType eb_vp9_deinit_encoder(EbComponentType *svt_enc_component) {
    ...
    EbErrorType       return_error   = EB_ErrorNone;
    int32_t           ptr_index      = 0;
    EbMemoryMapEntry *memory_entry   = (EbMemoryMapEntry *)NULL;
    if (enc_handle_ptr) {
...
#endif
                    break;
                case EB_SEMAPHORE: eb_vp9_destroy_semaphore(memory_entry->ptr); break;
                case EB_THREAD: eb_vp9_destroy_thread(memory_entry->ptr); break;
                case EB_MUTEX: eb_vp9_destroy_mutex(memory_entry->ptr); break;
...
__attribute__((visibility("default")))
#endif
EB_API void eb_vp9_svt_release_out_buffer(EbBufferHeaderType **p_buffer) {
...
#endif
EB_API EbErrorType
eb_vp9_svt_init_handle(EbComponentType **p_handle, // Function to be called in the future for manipulating the component
                       void             *p_app_data,
    ...
    *p_handle = (EbComponentType *)malloc(sizeof(EbComponentType));
    ...
    if (*p_handle != (EbComponentType *)NULL) {
        ...
        if (return_error == EB_ErrorNone) {
            ...
            eb_vp9_deinit_encoder(*p_handle);
...
**********************************/
static EbErrorType eb_svt_enc_component_de_init(EbComponentType *svt_enc_component) {
    EbErrorType return_error = EB_ErrorNone;
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_deinit_handle(EbComponentType *svt_enc_component) {
    EbErrorType return_error = EB_ErrorNone;
...
static void load_default_buffer_configuration_settings(SequenceControlSet *sequence_control_set_ptr) {
    ...
    if (sequence_control_set_ptr->static_config.logical_processors != 0)
        core_count = sequence_control_set_ptr->static_config.logical_processors < core_count
            ? sequence_control_set_ptr->static_config.logical_processors
            : core_count;
#endif
    ...
    sequence_control_set_ptr->input_output_buffer_fifo_init_count = input_pic + SCD_LAD +
        sequence_control_set_ptr->look_ahead_distance;
    ...
    sequence_control_set_ptr->reference_picture_buffer_init_count =
        MAX((uint32_t)(input_pic >> 1), (uint32_t)((1 << sequence_control_set_ptr->hierarchical_levels) + 2)) +
        sequence_control_set_ptr->look_ahead_distance + SCD_LAD;
    sequence_control_set_ptr->pa_reference_picture_buffer_init_count =
        MAX((uint32_t)(input_pic >> 1), (uint32_t)((1 << sequence_control_set_ptr->hierarchical_levels) + 2)) +
        sequence_control_set_ptr->look_ahead_distance + SCD_LAD;
    ...
    sequence_control_set_ptr->total_process_init_count +=
        (sequence_control_set_ptr->picture_analysis_process_init_count = MAX(MIN(15, core_count), core_count / 6));
    sequence_control_set_ptr->total_process_init_count +=
        (sequence_control_set_ptr->motion_estimation_process_init_count = MAX(MIN(20, core_count), core_count / 3));
    sequence_control_set_ptr->total_process_init_count +=
        (sequence_control_set_ptr->source_based_operations_process_init_count = MAX(MIN(3, core_count),
                                                                                    core_count / 12));
    sequence_control_set_ptr->total_process_init_count +=
        (sequence_control_set_ptr->mode_decision_configuration_process_init_count = MAX(MIN(3, core_count),
                                                                                        core_count / 12));
    sequence_control_set_ptr->total_process_init_count += (sequence_control_set_ptr->enc_dec_process_init_count = MAX(
                                                               MIN(40, core_count), core_count));
    sequence_control_set_ptr->total_process_init_count +=
        (sequence_control_set_ptr->entropy_coding_process_init_count = MAX(MIN(3, core_count), core_count / 12));
...
static int32_t compute_default_intra_period(SequenceControlSet *sequence_control_set_ptr) {
    int32_t                   intra_period  = 0;
    EbSvtVp9EncConfiguration *config        = &sequence_control_set_ptr->static_config;
    int32_t                   fps           = config->frame_rate < 1000 ? config->frame_rate : config->frame_rate >> 16;
...
static void set_default_configuration_parameters(SequenceControlSet *sequence_control_set_ptr) {
    // No Cropping Window
...
static uint32_t compute_default_look_ahead(EbSvtVp9EncConfiguration *config, SequenceControlSet *scs_ptr) {
    int32_t lad = 0;
...
static void copy_api_from_app(SequenceControlSet       *sequence_control_set_ptr,
                              EbSvtVp9EncConfiguration *p_component_parameter_structure) {
    sequence_control_set_ptr->max_input_luma_width =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->source_width;
    sequence_control_set_ptr->max_input_luma_height =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->source_height;
    ...
    sequence_control_set_ptr->static_config.intra_period =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->intra_period;
    ...
    sequence_control_set_ptr->static_config.base_layer_switch_mode =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->base_layer_switch_mode;
    sequence_control_set_ptr->static_config.tune = ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->tune;
    sequence_control_set_ptr->static_config.enc_mode =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->enc_mode;
    ...
    // Quantization
    sequence_control_set_ptr->qp = sequence_control_set_ptr->static_config.qp =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->qp;
    sequence_control_set_ptr->static_config.use_qp_file =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->use_qp_file;

    // Loop Filter
    sequence_control_set_ptr->static_config.loop_filter =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->loop_filter;

    // ME Tools
    sequence_control_set_ptr->static_config.use_default_me_hme =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->use_default_me_hme;

    // Default HME/ME settings
    sequence_control_set_ptr->static_config.enable_hme_flag =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->enable_hme_flag;
    sequence_control_set_ptr->static_config.search_area_width =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->search_area_width;
    sequence_control_set_ptr->static_config.search_area_height =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->search_area_height;
    ...
    sequence_control_set_ptr->static_config.rate_control_mode =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->rate_control_mode;
    sequence_control_set_ptr->static_config.vbv_max_rate =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->vbv_max_rate;
    sequence_control_set_ptr->static_config.vbv_buf_size =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->vbv_buf_size;
    sequence_control_set_ptr->static_config.frames_to_be_encoded =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->frames_to_be_encoded;
    sequence_control_set_ptr->frame_rate = sequence_control_set_ptr->static_config.frame_rate =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->frame_rate;
    ...
    sequence_control_set_ptr->encode_context_ptr->available_target_bitrate =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->target_bit_rate;
    ...
    sequence_control_set_ptr->static_config.encoder_bit_depth =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->encoder_bit_depth;

    // Annex A parameters
    sequence_control_set_ptr->static_config.profile =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->profile;
    sequence_control_set_ptr->static_config.level =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->level;

    sequence_control_set_ptr->static_config.injector_frame_rate =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->injector_frame_rate;
    sequence_control_set_ptr->static_config.speed_control_flag =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->speed_control_flag;

    sequence_control_set_ptr->static_config.asm_type =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->asm_type;

    sequence_control_set_ptr->static_config.channel_id =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->channel_id;
    sequence_control_set_ptr->static_config.active_channel_count =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->active_channel_count;

    sequence_control_set_ptr->static_config.logical_processors =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->logical_processors;
    sequence_control_set_ptr->static_config.target_socket =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->target_socket;

    sequence_control_set_ptr->static_config.frame_rate_denominator =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->frame_rate_denominator;
    sequence_control_set_ptr->static_config.frame_rate_numerator =
        ((EbSvtVp9EncConfiguration *)p_component_parameter_structure)->frame_rate_numerator;

    // Set default hierarchical-levels
    if (sequence_control_set_ptr->static_config.tune != 0 &&
    ...
    if (sequence_control_set_ptr->static_config.frame_rate_numerator != 0 &&
        sequence_control_set_ptr->static_config.frame_rate_denominator != 0) {
        sequence_control_set_ptr->frame_rate = sequence_control_set_ptr->static_config.frame_rate =
            (((sequence_control_set_ptr->static_config.frame_rate_numerator << 8) /
              (sequence_control_set_ptr->static_config.frame_rate_denominator))
             << 8);
    }
    ...
    if (sequence_control_set_ptr->static_config.intra_period == -2) {
        sequence_control_set_ptr->intra_period = (sequence_control_set_ptr->static_config.intra_period =
                                                      compute_default_intra_period(sequence_control_set_ptr));
    }

    sequence_control_set_ptr->look_ahead_distance = compute_default_look_ahead(&sequence_control_set_ptr->static_config,
...
static EbErrorType verify_settings(SequenceControlSet *sequence_control_set_ptr) {
    EbErrorType               return_error = EB_ErrorNone;
    ...
    uint32_t input_size = sequence_control_set_ptr->max_input_luma_width *
        sequence_control_set_ptr->max_input_luma_height;
    ...
    if (level_idx < 13) {
        ...
        if (config->level != 0 &&
            ((uint64_t)sequence_control_set_ptr->max_input_luma_width *
                 (uint64_t)sequence_control_set_ptr->max_input_luma_height >
             max_luma_picture_size[level_idx])) {
            SVT_LOG(
                "Error Instance %u: The input luma picture size exceeds the maximum luma picture size allowed for "
                "level %s\n",
                channel_number + 1,
                level_idc);
            return_error = EB_ErrorBadParameter;
        ...
        // Check if the current input video is conformant with the Level constraint
        if (config->level != 0 &&
            ((uint64_t)config->frame_rate * (uint64_t)sequence_control_set_ptr->max_input_luma_width *
                 (uint64_t)sequence_control_set_ptr->max_input_luma_height >
             (max_luma_sample_rate[level_idx] << 16))) {
            SVT_LOG(
                "Error Instance %u: The input luma sample rate exceeds the maximum input sample rate allowed for level "
                "%s\n",
                channel_number + 1,
                level_idc);
            return_error = EB_ErrorBadParameter;
...
* Set Parameter
**********************************/
static void set_param_based_on_input(SequenceControlSet *sequence_control_set_ptr) {
    // Update picture width, and picture height
    if (sequence_control_set_ptr->max_input_luma_width % MIN_CU_SIZE) {
        sequence_control_set_ptr->max_input_pad_right = MIN_CU_SIZE -
            (sequence_control_set_ptr->max_input_luma_width % MIN_CU_SIZE);
        sequence_control_set_ptr->max_input_luma_width = sequence_control_set_ptr->max_input_luma_width +
    ...
    if (sequence_control_set_ptr->max_input_luma_height % MIN_CU_SIZE) {
        sequence_control_set_ptr->max_input_pad_bottom = MIN_CU_SIZE -
            (sequence_control_set_ptr->max_input_luma_height % MIN_CU_SIZE);
        sequence_control_set_ptr->max_input_luma_height = sequence_control_set_ptr->max_input_luma_height +
    ...
    eb_vp9_derive_input_resolution(sequence_control_set_ptr,
...
static void print_lib_params(SequenceControlSet *scs) {
    ...
    if (config->frame_rate_denominator != 0 && config->frame_rate_numerator != 0)
        SVT_LOG("\nSVT [config]: Fps_Numerator / Fps_Denominator / Gop Size\t\t: %d / %d / %d",
                config->frame_rate_numerator > (1 << 16) ? config->frame_rate_numerator >> 16
                                                         : config->frame_rate_numerator,
                config->frame_rate_denominator > (1 << 16) ? config->frame_rate_denominator >> 16
                                                           : config->frame_rate_denominator,
                config->intra_period + 1);
    else
        SVT_LOG("\nSVT [config]: FrameRate / Gop Size\t\t\t\t\t\t: %d / %d ",
                config->frame_rate > 1000 ? config->frame_rate >> 16 : config->frame_rate,
                config->intra_period + 1);
    SVT_LOG("\nSVT [config]: HierarchicalLevels / BaseLayerSwitchMode / PredStructure\t\t: %d / %d / %d ",
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_enc_set_parameter(EbComponentType          *svt_enc_component,
                                                EbSvtVp9EncConfiguration *p_component_parameter_structure) {
    ...
    if (svt_enc_component == (EbComponentType *)NULL) {
        return EB_ErrorBadParameter;
    }
    ...
    copy_api_from_app(scs_ptr, (EbSvtVp9EncConfiguration *)p_component_parameter_structure);
...
************************************************/
static EbErrorType copy_frame_buffer(SequenceControlSet *sequence_control_set_ptr, uint8_t *dst, uint8_t *src) {
    EbSvtVp9EncConfiguration *config       = &sequence_control_set_ptr->static_config;
    EbErrorType               return_error = EB_ErrorNone;
    ...
    if (!is16_bit_input) {
        uint32_t luma_buffer_offset = (input_picture_ptr->stride_y * sequence_control_set_ptr->top_padding +
                                       sequence_control_set_ptr->left_padding)
            << is16_bit_input;
        uint32_t chroma_buffer_offset = (input_picture_ptr->stride_cr * (sequence_control_set_ptr->top_padding >> 1) +
                                         (sequence_control_set_ptr->left_padding >> 1))
            << is16_bit_input;
        ...
        uint16_t luma_width    = (uint16_t)(input_picture_ptr->width - sequence_control_set_ptr->max_input_pad_right)
            << is16_bit_input;
        ...
        for (input_row_index = 0; input_row_index < luma_height; input_row_index++) {
            memcpy(input_picture_ptr->buffer_y + luma_buffer_offset + luma_stride * input_row_index,
        ...
        for (input_row_index = 0; input_row_index < luma_height >> 1; input_row_index++) {
            memcpy(input_picture_ptr->buffer_cb + chroma_buffer_offset + chroma_stride * input_row_index,
        ...
        // V
        for (input_row_index = 0; input_row_index < luma_height >> 1; input_row_index++) {
            memcpy(input_picture_ptr->buffer_cr + chroma_buffer_offset + chroma_stride * input_row_index,
...
static void copy_output_recon_buffer(EbBufferHeaderType *dst, EbBufferHeaderType *src) {
    ...
    dst->pic_type      = src->pic_type;
    if (src->p_buffer)
        memcpy(dst->p_buffer, src->p_buffer, src->n_filled_len);
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_get_recon(EbComponentType *svt_enc_component, EbBufferHeaderType *p_buffer) {
    ...
    if (scs_ptr->static_config.recon_file) {
        eb_vp9_get_full_object_non_blocking((p_enc_comp_data->output_recon_buffer_consumer_fifo_ptr_dbl_array[0])[0],
                                            &eb_wrapper_ptr);
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_enc_send_picture(EbComponentType *svt_enc_component, EbBufferHeaderType *p_buffer) {
    EbEncHandle     *enc_handle_ptr = (EbEncHandle *)svt_enc_component->p_component_private;
    EbObjectWrapper *eb_wrapper_ptr;

    // Take the buffer and put it into our internal queue structure
    eb_vp9_get_empty_object(enc_handle_ptr->input_buffer_producer_fifo_ptr_array[0], &eb_wrapper_ptr);

    if (p_buffer != NULL) {
        copy_input_buffer(enc_handle_ptr->sequence_control_set_instance_array[0]->sequence_control_set_ptr,
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_get_packet(EbComponentType *svt_enc_component, EbBufferHeaderType **p_buffer,
    ...
    if (pic_send_done)
        eb_vp9_get_full_object((p_enc_comp_data->output_stream_buffer_consumer_fifo_ptr_dbl_array[0])[0],
                               &eb_wrapper_ptr);
    else
        eb_vp9_get_full_object_non_blocking((p_enc_comp_data->output_stream_buffer_consumer_fifo_ptr_dbl_array[0])[0],
                                            &eb_wrapper_ptr);

    if (eb_wrapper_ptr) {
        packet = (EbBufferHeaderType *)eb_wrapper_ptr->object_ptr;

        if (packet->flags != EB_BUFFERFLAG_EOS && packet->flags != EB_BUFFERFLAG_SHOW_EXT &&
            packet->flags != (EB_BUFFERFLAG_SHOW_EXT | EB_BUFFERFLAG_EOS) && packet->flags != 0) {
            return_error = EB_ErrorMax;
        }
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_enc_stream_header(EbComponentType     *svt_enc_component,
...
__attribute__((visibility("default")))
#endif
EB_API EbErrorType eb_vp9_svt_enc_eos_nal(EbComponentType *svt_enc_component, EbBufferHeaderType **output_stream_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    UNUSED(svt_enc_component);
    UNUSED(output_stream_ptr);
    return return_error;
...
static EbErrorType init_svt_vp9_encoder_handle(EbComponentType *h_component) {
    ...
    return_error = (EbErrorType)eb_enc_handle_ctor((EbEncHandle **)&(svt_enc_component->p_component_private),
                                                   svt_enc_component);
