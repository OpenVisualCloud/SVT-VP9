...
EbErrorType eb_vp9_me_context_ctor(MeContext **object_dbl_ptr) {
    uint32_t list_index;
    ...
    EB_ALLIGN_MALLOC(uint8_t *,
                     (*object_dbl_ptr)->sb_buffer,
                     sizeof(uint8_t) * MAX_SB_SIZE * (*object_dbl_ptr)->sb_buffer_stride,
                     EB_A_PTR);

    (*object_dbl_ptr)->hme_sb_buffer_stride = (MAX_SB_SIZE + HME_DECIM_FILTER_TAP - 1);
    EB_MALLOC(uint8_t *,
              (*object_dbl_ptr)->hme_sb_buffer,
    ...
    EB_MALLOC(uint8_t *,
              (*object_dbl_ptr)->quarter_sb_buffer,
              sizeof(uint8_t) * (MAX_SB_SIZE >> 1) * (*object_dbl_ptr)->quarter_sb_buffer_stride,
              EB_N_PTR);

    (*object_dbl_ptr)->sixteenth_sb_buffer_stride = (MAX_SB_SIZE >> 2);
    EB_ALLIGN_MALLOC(uint8_t *,
                     (*object_dbl_ptr)->sixteenth_sb_buffer,
                     sizeof(uint8_t) * (MAX_SB_SIZE >> 2) * (*object_dbl_ptr)->sixteenth_sb_buffer_stride,
                     EB_A_PTR);

    (*object_dbl_ptr)->interpolated_stride = MAX_SEARCH_AREA_WIDTH;
    EB_MALLOC(uint8_t *,
              (*object_dbl_ptr)->hme_buffer,
              sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
              EB_N_PTR);
    ...
    for (list_index = 0; list_index < MAX_NUM_OF_REF_PIC_LIST; list_index++) {
        for (ref_pic_index = 0; ref_pic_index < MAX_REF_IDX; ref_pic_index++) {
            EB_MALLOC(uint8_t *,
                      (*object_dbl_ptr)->integer_buffer[list_index][ref_pic_index],
                      sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
                      EB_N_PTR);

            EB_MALLOC(uint8_t *,
                      (*object_dbl_ptr)->posb_buffer[list_index][ref_pic_index],
                      sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
                      EB_N_PTR);

            EB_MALLOC(uint8_t *,
                      (*object_dbl_ptr)->posh_buffer[list_index][ref_pic_index],
                      sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
                      EB_N_PTR);

            EB_MALLOC(uint8_t *,
                      (*object_dbl_ptr)->posj_buffer[list_index][ref_pic_index],
                      sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
                      EB_N_PTR);
        }
    }
    ...
    EB_MALLOC(EbByte,
              (*object_dbl_ptr)->one_d_intermediate_results_buf0,
              sizeof(uint8_t) * MAX_SB_SIZE * MAX_SB_SIZE,
              EB_N_PTR);

    EB_MALLOC(EbByte,
              (*object_dbl_ptr)->one_d_intermediate_results_buf1,
              sizeof(uint8_t) * MAX_SB_SIZE * MAX_SB_SIZE,
              EB_N_PTR);
    ...
    EB_MALLOC(uint8_t *,
              (*object_dbl_ptr)->avctemp_buffer,
              sizeof(uint8_t) * (*object_dbl_ptr)->interpolated_stride * MAX_SEARCH_AREA_HEIGHT,
              EB_N_PTR);

    EB_MALLOC(uint16_t *,
              (*object_dbl_ptr)->p_eight_pos_sad16x16,
              sizeof(uint16_t) * 8 * 16,
              EB_N_PTR); //16= 16 16x16 blocks in a LCU.       8=8search points
