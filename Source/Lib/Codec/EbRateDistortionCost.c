...
EbErrorType get_partition_cost(PictureControlSet *picture_control_set_ptr, EncDecContext *context_ptr, BLOCK_SIZE bsize,
    ...
    if (bsize >= BLOCK_8X8) {
        *partition_cost = RDCOST(
            context_ptr->RDMULT, rd->RDDIV, cpi->partition_cost[partition_context][partition_type], 0);
    } else {
        *partition_cost = 0;
...
EbErrorType inter_full_cost(struct EncDecContext *context_ptr, struct ModeDecisionCandidateBuffer *candidate_buffer_ptr,
                            ...
                            int *cb_coeff_bits, int *cr_coeff_bits, PictureControlSet *picture_control_set_ptr) {
    EbErrorType        return_error = EB_ErrorNone;
    VP9_COMP          *cpi          = picture_control_set_ptr->parent_pcs_ptr->cpi;
    MACROBLOCKD *const xd           = context_ptr->e_mbd;
    RD_OPT *const      rd           = &cpi->rd;
...
EbErrorType intra_full_cost(struct EncDecContext *context_ptr, struct ModeDecisionCandidateBuffer *candidate_buffer_ptr,
                            int *y_distortion, int *cb_distortion, int *cr_distortion, int *y_coeff_bits,
                            int *cb_coeff_bits, int *cr_coeff_bits, PictureControlSet *picture_control_set_ptr) {
    EbErrorType        return_error = EB_ErrorNone;
    VP9_COMP          *cpi          = picture_control_set_ptr->parent_pcs_ptr->cpi;
...
int64_t inter_fast_cost(PictureControlSet *picture_control_set_ptr, int has_uv, int rd_mult_sad,
                        MbModeInfoExt *mbmi_ext, MACROBLOCKD *e_mbd, unsigned int *ref_costs_single,
                        unsigned int *ref_costs_comp, vpx_prob comp_mode_p, ModeDecisionCandidate *candidate_ptr,
...
int64_t intra_fast_cost(PictureControlSet *picture_control_set_ptr, int has_uv, int rd_mult_sad,
                        MbModeInfoExt *mbmi_ext, MACROBLOCKD *e_mbd, unsigned int *ref_costs_single,
                        unsigned int *ref_costs_comp, vpx_prob comp_mode_p, ModeDecisionCandidate *candidate_ptr,
                        uint64_t luma_distortion, uint64_t chroma_distortion) {
    UNUSED(comp_mode_p);
    ...
    int rate_uv_mode = has_uv ? cpi->intra_uv_mode_cost[cpi->common.frame_type][mode][uv_mode] : 0;
