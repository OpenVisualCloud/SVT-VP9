 ...
 ***************************************************************************************************/
EbErrorType eb_vp9_sequence_control_set_ctor(EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr) {
    ...
    *object_dbl_ptr = (EbPtr)sequence_control_set_ptr;
    ...
    EB_MALLOC(AppVideoUsabilityInfo *,
              sequence_control_set_ptr->video_usability_info_ptr,
              sizeof(AppVideoUsabilityInfo),
              EB_N_PTR);

    // Initialize vui parameters
    return_error = eb_video_usability_info_ctor(sequence_control_set_ptr->video_usability_info_ptr);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
    // Initialize picture timing SEI
    eb_picture_timeing_sei_ctor(&sequence_control_set_ptr->pic_timing_sei);

    // Initialize picture timing SEI
    eb_buffering_period_sei_ctor(&sequence_control_set_ptr->buffering_period);

    // Initialize picture timing SEI
    eb_recovery_point_sei_ctor(&sequence_control_set_ptr->recovery_point);

    // Initialize LCU params
    sb_params_ctor(sequence_control_set_ptr);
...
EbErrorType eb_vp9_sequence_control_set_instance_ctor(EbSequenceControlSetInstance **object_dbl_ptr) {
    ...
    return_error = eb_vp9_sequence_control_set_ctor((void **)&(*object_dbl_ptr)->sequence_control_set_ptr,
                                                    (void *)&scs_init_data);
    if (return_error == EB_ErrorInsufficientResources) {
        return EB_ErrorInsufficientResources;
    }
...
extern EbErrorType eb_vp9_sb_params_init(SequenceControlSet *sequence_control_set_ptr) {
    ...
    EB_MALLOC(SbParams *,
              sequence_control_set_ptr->sb_params_array,
              sizeof(SbParams) * picturesb_width * picturesb_height,
              EB_N_PTR);
    ...
    for (sb_index = 0; sb_index < picturesb_width * picturesb_height; ++sb_index) {
        ...
        sequence_control_set_ptr->sb_params_array[sb_index].origin_x =
            sequence_control_set_ptr->sb_params_array[sb_index].horizontal_index * MAX_SB_SIZE;
        sequence_control_set_ptr->sb_params_array[sb_index].origin_y =
            sequence_control_set_ptr->sb_params_array[sb_index].vertical_index * MAX_SB_SIZE;

        sequence_control_set_ptr->sb_params_array[sb_index].width =
            (uint8_t)(((uint32_t)(sequence_control_set_ptr->luma_width -
                                  sequence_control_set_ptr->sb_params_array[sb_index].origin_x) < MAX_SB_SIZE)
                          ? (uint8_t)(sequence_control_set_ptr->luma_width -
                                      sequence_control_set_ptr->sb_params_array[sb_index].origin_x)
                          : (uint8_t)MAX_SB_SIZE);

        sequence_control_set_ptr->sb_params_array[sb_index].height =
            (uint8_t)(((uint32_t)(sequence_control_set_ptr->luma_height -
                                  sequence_control_set_ptr->sb_params_array[sb_index].origin_y) < MAX_SB_SIZE)
                          ? (uint8_t)(sequence_control_set_ptr->luma_height -
                                      sequence_control_set_ptr->sb_params_array[sb_index].origin_y)
                          : (uint8_t)MAX_SB_SIZE);

        sequence_control_set_ptr->sb_params_array[sb_index].is_complete_sb =
            (uint8_t)(((sequence_control_set_ptr->sb_params_array[sb_index].width == MAX_SB_SIZE) &&
                       (sequence_control_set_ptr->sb_params_array[sb_index].height == MAX_SB_SIZE))
                          ? 1
                          : 0);

        sequence_control_set_ptr->sb_params_array[sb_index].is_edge_sb =
            (sequence_control_set_ptr->sb_params_array[sb_index].origin_x < MAX_SB_SIZE) ||
                (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < MAX_SB_SIZE) ||
                (sequence_control_set_ptr->sb_params_array[sb_index].origin_x >
                 sequence_control_set_ptr->luma_width - MAX_SB_SIZE) ||
                (sequence_control_set_ptr->sb_params_array[sb_index].origin_y >
                 sequence_control_set_ptr->luma_height - MAX_SB_SIZE)
            ? 1
            : 0;
        ...
        if (sequence_control_set_ptr->input_resolution <= INPUT_SIZE_576p_RANGE_OR_LOWER) {
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x >=
                                 (sequence_control_set_ptr->luma_width - (3 * MAX_SB_SIZE))) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 2 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x < (3 * MAX_SB_SIZE)) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 2 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_y >=
                                 (sequence_control_set_ptr->luma_height - (2 * MAX_SB_SIZE)))
                ? 1
                : potential_logo_sb;

        } else if (sequence_control_set_ptr->input_resolution < INPUT_SIZE_4K_RANGE) {
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x >=
                                 (sequence_control_set_ptr->luma_width - (7 * MAX_SB_SIZE))) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 4 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x < (7 * MAX_SB_SIZE)) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 4 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_y >=
                                 (sequence_control_set_ptr->luma_height - (4 * MAX_SB_SIZE)))
                ? 1
                : potential_logo_sb;

        } else {
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x >=
                                 (sequence_control_set_ptr->luma_width - (14 * MAX_SB_SIZE))) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 8 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_x < (14 * MAX_SB_SIZE)) &&
                    (sequence_control_set_ptr->sb_params_array[sb_index].origin_y < 8 * MAX_SB_SIZE)
                ? 1
                : potential_logo_sb;
            potential_logo_sb = (sequence_control_set_ptr->sb_params_array[sb_index].origin_y >=
                                 (sequence_control_set_ptr->luma_height - (8 * MAX_SB_SIZE)))
                ? 1
                : potential_logo_sb;
        }
        sequence_control_set_ptr->sb_params_array[sb_index].potential_logo_sb = potential_logo_sb;

        for (raster_scan_block_index = PA_RASTER_SCAN_CU_INDEX_64x64;
             raster_scan_block_index <= PA_RASTER_SCAN_CU_INDEX_8x8_63;
             raster_scan_block_index++) {
            sequence_control_set_ptr->sb_params_array[sb_index].pa_raster_scan_block_validity[raster_scan_block_index] =
                ((sequence_control_set_ptr->sb_params_array[sb_index].origin_x +
                      pa_raster_scan_cu_x[raster_scan_block_index] + pa_raster_scan_cu_size[raster_scan_block_index] >
        ...
        for (md_scan_block_index = 0; md_scan_block_index < EP_BLOCK_MAX_COUNT; md_scan_block_index++) {
            ...
            sequence_control_set_ptr->sb_params_array[sb_index].ep_scan_block_validity[md_scan_block_index] =
                (((sequence_control_set_ptr->sb_params_array[sb_index].origin_x + ep_block_stats_ptr->origin_x +
        ...
        for (md_scan_block_index = EP_BLOCK_MAX_COUNT - 1; md_scan_block_index-- != 0;) {
            ...
            if (sequence_control_set_ptr->sb_params_array[sb_index].ep_scan_block_validity[md_scan_block_index] ==
                ...
                if (ep_block_stats_ptr->bsize > 3) {
                    ...
                    for (uint16_t search_valid_index = md_scan_block_index;
                         search_valid_index < (md_scan_block_index + ep_inter_depth_offset);
                         search_valid_index++) {
                        if (search_valid_index < EP_BLOCK_MAX_COUNT &&
                            sequence_control_set_ptr->sb_params_array[sb_index]
                                    .ec_scan_block_valid_block[search_valid_index] != (uint16_t)~0) {
                            sequence_control_set_ptr->sb_params_array[sb_index]
                                .ec_scan_block_valid_block[md_scan_block_index] =
                                sequence_control_set_ptr->sb_params_array[sb_index]
                                    .ec_scan_block_valid_block[search_valid_index];
                            break;
                        }
                    }
                }
...
extern EbErrorType eb_vp9_derive_input_resolution(SequenceControlSet *sequence_control_set_ptr, uint32_t input_size) {
    ...
    sequence_control_set_ptr->input_resolution = (input_size < INPUT_SIZE_1080i_TH) ? INPUT_SIZE_576p_RANGE_OR_LOWER
