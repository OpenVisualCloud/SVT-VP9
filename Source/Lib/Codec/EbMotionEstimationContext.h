...
// Max Search Area
#define MAX_SEARCH_AREA_WIDTH \
    1350 // This should be a function for the MAX HME L0 * the multiplications per layers and per Hierarchichal structures
#define MAX_SEARCH_AREA_HEIGHT \
    675 // This should be a function for the MAX HME L0 * the multiplications per layers and per Hierarchichal structures
...
typedef struct MeContext { // MV offset (search center)
    int16_t x_mv_offset;
    int16_t y_mv_offset;
...
extern EbErrorType eb_vp9_me_context_ctor(MeContext **object_dbl_ptr);
