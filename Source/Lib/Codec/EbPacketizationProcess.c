...
EbErrorType eb_vp9_packetization_context_ctor(PacketizationContext **context_dbl_ptr,
...
void eb_vp9_update_rc_rate_tables(PictureControlSet  *picture_control_set_ptr,
    ...
    if (sequence_control_set_ptr->static_config.rate_control_mode > 0) {
        ...
        }
        {
            ...
            uint64_t ref_qindex_dequant = (uint64_t)picture_control_set_ptr->parent_pcs_ptr->cpi
                                              ->y_dequant[picture_control_set_ptr->base_qindex][1];
            uint64_t sad_bits_ref_dequant = 0;
            uint64_t weight               = 0;
            {
                if (picture_control_set_ptr->slice_type == I_SLICE) {
                    if (sequence_control_set_ptr->input_resolution < INPUT_SIZE_4K_RANGE) {
                        for (sad_interval_index = 0; sad_interval_index < NUMBER_OF_INTRA_SAD_INTERVALS;
                             sad_interval_index++) {
                            ...
                            if (count[sad_interval_index] > 0) {
                                ...
                                for (qp_index = sequence_control_set_ptr->static_config.min_qp_allowed;
                                     qp_index <= (int32_t)sequence_control_set_ptr->static_config.max_qp_allowed;
                                     qp_index++) {
                                    encode_context_ptr->rate_control_tables_array[qp_index]
                                        .intra_sad_bits_array[picture_control_set_ptr->temporal_layer_index]
                                                             [sad_interval_index] =
                                        (EbBitNumber)(((weight * sad_bits_ref_dequant /
                                                        picture_control_set_ptr->parent_pcs_ptr->cpi
                                                            ->y_dequant[eb_vp9_quantizer_to_qindex(qp_index)][1]) +
                                                       (10 - weight) *
                                                           (uint32_t)encode_context_ptr
                                                               ->rate_control_tables_array[qp_index]
                                                               .intra_sad_bits_array[picture_control_set_ptr
                                    ...
                                    encode_context_ptr->rate_control_tables_array[qp_index]
                                        .intra_sad_bits_array[picture_control_set_ptr->temporal_layer_index]
                                                             [sad_interval_index] = MIN(
                                        (uint16_t)encode_context_ptr->rate_control_tables_array[qp_index]
                                            .intra_sad_bits_array[picture_control_set_ptr->temporal_layer_index]
                                                                 [sad_interval_index],
                                        (uint16_t)((1 << 15) - 1));
                                }
                        ...
                        for (sad_interval_index = 0; sad_interval_index < NUMBER_OF_SAD_INTERVALS;
                            ...
                            if (count[sad_interval_index] > 0) {
                                ...
                                for (qp_index = sequence_control_set_ptr->static_config.min_qp_allowed;
                                     ...
                                     qp_index++) {
                                    encode_context_ptr->rate_control_tables_array[qp_index]
                                        .sad_bits_array[picture_control_set_ptr->temporal_layer_index]
                                                       [sad_interval_index] =
                                        (EbBitNumber)(((weight * sad_bits_ref_dequant /
                                                        picture_control_set_ptr->parent_pcs_ptr->cpi
                                                            ->y_dequant[eb_vp9_quantizer_to_qindex(qp_index)][1]) +
                                                       (10 - weight) *
                                                           (uint32_t)encode_context_ptr
                                                               ->rate_control_tables_array[qp_index]
                                                               .sad_bits_array[picture_control_set_ptr
                                                                                   ->temporal_layer_index]
                                                                              [sad_interval_index] +
                                                       5) /
                                                      10);
...
void *eb_vp9_packetization_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_vp9_get_full_object(context_ptr->entropy_coding_input_fifo_ptr, &entropy_coding_results_wrapper_ptr);
        ...
        picture_control_set_ptr    = (PictureControlSet *)
                                      entropy_coding_results_ptr->picture_control_set_wrapper_ptr->object_ptr;
        ...
        queue_entry_index = picture_control_set_ptr->parent_pcs_ptr->decode_order %
        ...
        output_stream_ptr->dts          = picture_control_set_ptr->parent_pcs_ptr->decode_order -
            (uint64_t)(1 << sequence_control_set_ptr->hierarchical_levels) + 1;
        output_stream_ptr->pic_type      = picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag
                 ? picture_control_set_ptr->parent_pcs_ptr->idr_flag ? EB_IDR_PICTURE : picture_control_set_ptr->slice_type
                 : EB_NON_REF_PICTURE;
        ...
        eb_vp9_get_empty_object(context_ptr->rate_control_tasks_output_fifo_ptr, &rate_control_tasks_wrapper_ptr);
        ...
        rate_control_tasks_ptr->picture_control_set_wrapper_ptr =
            picture_control_set_ptr->picture_parent_control_set_wrapper_ptr;
        ...
        size = 0;
        eb_vp9_reset_bitstream(picture_control_set_ptr->bitstream_ptr->output_bitstream_ptr);

        eb_vp9_pack_bitstream(picture_control_set_ptr,
                              picture_control_set_ptr->parent_pcs_ptr->cpi,
        ...
        queue_entry_ptr->show_existing_frame_index_array[0] =
            picture_control_set_ptr->parent_pcs_ptr->show_existing_frame_index_array[0];
        queue_entry_ptr->show_existing_frame_index_array[1] =
            picture_control_set_ptr->parent_pcs_ptr->show_existing_frame_index_array[1];
        queue_entry_ptr->show_existing_frame_index_array[2] =
            picture_control_set_ptr->parent_pcs_ptr->show_existing_frame_index_array[2];
        queue_entry_ptr->show_existing_frame_index_array[3] =
            picture_control_set_ptr->parent_pcs_ptr->show_existing_frame_index_array[3];
        ...
        while (queue_entry_ptr->output_stream_wrapper_ptr != EB_NULL) {
...

            {
                ...
                if (queue_entry_ptr->show_existing_frame) {
                    if (context_ptr->disp_order_continuity_count ==
                        context_ptr->dpb_disp_order[queue_entry_ptr->show_existing_frame_index_array[0]])
                    ...
                    if (queue_entry_ptr->frame_type == INTER_FRAME && queue_entry_ptr->intra_only == EB_FALSE) {
                        if (queue_entry_ptr->show_existing_frame)
                            SVT_LOG(
                                ...
                                (int)queue_entry_ptr->picture_number,
                                (int)context_ptr->dpb_dec_order[LASTrefIdx],
                                (int)context_ptr->dpb_dec_order[BWDrefIdx],
                                (int)queue_entry_ptr->poc,
                                (int)context_ptr->dpb_disp_order[LASTrefIdx],
                                (int)context_ptr->dpb_disp_order[BWDrefIdx],
                                showTab[queue_entry_ptr->show_frame],
                        ...
                        if (queue_entry_ptr->show_existing_frame)
                            ...
                            SVT_LOG("%i  %i  %c   ----INTRA---- %i frames\n",
                                    (int)queue_entry_ptr->picture_number,
                                    (int)queue_entry_ptr->poc,
                                    (int)showTab[queue_entry_ptr->show_frame],
                                    (int)context_ptr->tot_shown_frames);
                    }
#endif
                }
            ...
            latency = svt_vp9_compute_overall_elapsed_time_ms(queue_entry_ptr->start_time_seconds,
            ...
            if (encode_context_ptr->vbv_max_rate && encode_context_ptr->vbv_buf_size) {
                ...
                buffer_fill_temp                = (int64_t)(buffer_fill_temp +
                                             (encode_context_ptr->vbv_max_rate *
                                              (1.0 / (sequence_control_set_ptr->frame_rate >> RC_PRECISION))));
