...
void avc_style_copy_new(EbByte ref_pic, uint32_t src_stride, EbByte dst, uint32_t dst_stride, uint32_t pu_width,
...
void eb_vp9_avc_style_luma_interpolation_filter_horizontal(EbByte ref_pic, uint32_t src_stride, EbByte dst,
...
void eb_vp9_avc_style_luma_interpolation_filter_vertical(EbByte ref_pic, uint32_t src_stride, EbByte dst,
                                                         uint32_t dst_stride, uint32_t pu_width, uint32_t pu_height,
                                                         EbByte temp_buf, uint32_t frac_pos) {
    const int8_t *if_coeff           = avc_style_luma_if_coeff[frac_pos];
        ...
        for (x = 0; x < pu_width; x++) {
            dst[x] = (uint8_t)CLIP3(
                ...
                (ref_pic[x] * if_coeff[0] + ref_pic[x + if_stride] * if_coeff[1] +
                 ref_pic[x + 2 * if_stride] * if_coeff[2] + ref_pic[x + 3 * if_stride] * if_coeff[3] + if_offset) >>
                    if_shift);
        }
        ref_pic += src_stride;
        dst += dst_stride;
